...
NOTE  
Knowledge graphs can also be used for multi-hop question-answering tasks. See [Construct knowledge graph](https://ragflow.io/docs/construct_knowledge_graph) for details. You may use either approach or both, but ensure you understand the memory, computational, and token costs involved.
...
The following prompt will be applied *recursively* for cluster summarization, with {cluster\_content} serving as an internal parameter. We recommend that you keep it as-is for now. The design will be updated in due course.
