...
Hestami is a unified platform for **HOA/community association management**, **homeowner concierge services**, and **field service operations**. The core architecture is a **modular monolith** built with **SvelteKit 5 with Runes \+ Node**, using **oRPC** for typed APIs, **Zod** (generated from Prisma) for schemas, and **DBOS** for durable workflows.
...
| ---------------------------- | ----------------------------------------------------------- |
| **Property Concierge**       | Service Calls, Vendors, Bidding, Workflow, Communication |
| **CAM Software**             | Governance, Property, Stakeholders, Compliance, Workflow    |
| **Field Service Operations** | Vendors, Work Execution, Commercial Terms, Communication    |

Pillar 1: Property Concierge
...
Pillar 3: Field Service Operations
Core FSO domains:
...
The software offers tools to streamline business workflows, including:

Customer Relationship Management (CRM)

Dispatching and Scheduling (including route optimization)

Call Booking

Work Order Management

Invoicing, Payment Processing, and Accounting Integration
...
* Backend: **SvelteKit 5 with Runes (Bun)** with **oRPC**.
...
NOTE: We ARE NOT using SuperForms. 
...
|----------------|--------------|----------|
| `generated/prisma/` | `bunx prisma generate` | Prisma client, TypeScript enums |
| `generated/zod/` | `bunx prisma generate` (via zod-prisma-types) | Zod schemas for all Prisma models and enums |
| `src/lib/api/types.generated.ts` | `bun run types:generate` | TypeScript types derived from OpenAPI spec |
...
After modifying `prisma/schema.prisma`: run `bunx prisma generate`

After modifying oRPC routes: run `bun run openapi:generate && bun run types:generate`

After any changes: run `bun run check` (must pass with 0 errors)
...
2. **Run `bunx prisma generate`** to update:
...
6. **Regenerate OpenAPI spec**: `bun run openapi:generate`

7. **Regenerate frontend types**: `bun run types:generate` → updates `src/lib/api/types.generated.ts`
...
13. **Run `bun run check`** to verify TypeScript/Svelte types are correct (must pass with 0 errors).
...
* **Importing Prisma types OR types.generated directly in client-side code (CAUSES MEMORY CRASH)**: Never import types from `generated/prisma/client` OR directly from `types.generated.ts` in `.svelte` files, `src/lib/stores/`, or other client-side code. Both are massive files (~44MB Prisma, ~116K lines types.generated) and will cause `bun run check` (svelte-check) to crash with "JavaScript heap out of memory".

**Solution**: Use pre-extracted types from `$lib/api/cam.ts` which re-exports only the specific types needed:

```typescript
// ❌ WRONG - Imports massive Prisma types, crashes svelte-check
import type { Association } from '../../generated/prisma/client.js';

// ❌ ALSO WRONG - Imports entire 116K-line types.generated, crashes svelte-check in .svelte files
...
// ✅ CORRECT - Import pre-extracted types from cam.ts
import type { Organization, Staff, Association } from '$lib/api/cam';
```

**Key principle**: 
- `generated/prisma/` types → Only for server-side code (`*.server.ts`, `$lib/server/**`)
- `types.generated.ts` direct imports → Only in `$lib/api/*.ts` files (not `.svelte`)
- Pre-extracted types from `$lib/api/cam.ts` → For `.svelte` files and client-side code

Type extractions should be added to `$lib/api/cam.ts` and imported from there. This keeps types in sync with the API while avoiding the memory crash.
...
* Passing `bun run check` with 0 errors
