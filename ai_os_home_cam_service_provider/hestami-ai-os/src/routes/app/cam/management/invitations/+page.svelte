<script lang="ts">
	...
	// Create form state
	let newEmail = $state('');
	let newRole = $state(UserRoleValues.OWNER);
...
<svelte:head>
	<title>Member Invitations | CAM</title>
</svelte:head>

<PageContainer>
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold">Member Invitations</h1>
			<p class="mt-1 text-surface-500">Invite homeowners, tenants, and board members to join your association</p>
		</div>
		<button
	...
	<div class="mt-6">
		...
		{:else if invitations.length === 0}
			<EmptyState
				...
				description="Invite members to join your association by clicking the button above."
			/>
		{:else}
...
{#if showCreateModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
		<Card variant="filled" padding="lg" class="w-full max-w-md">
			<h2 class="text-xl font-semibold">Invite New Member</h2>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
		<Card variant="filled" padding="lg" class="w-full max-w-md text-center">
			...
			<p class="mt-4 text-xs text-surface-500">
				This code expires in 72 hours. The member can use it at the "Join Organization" page.
			</p>
