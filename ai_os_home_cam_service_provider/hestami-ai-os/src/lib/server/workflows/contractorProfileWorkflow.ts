...
const WorkflowErrorType = {
	CONTRACTOR_PROFILE_WORKFLOW_ERROR: 'CONTRACTOR_PROFILE_WORKFLOW_ERROR'
} as const;
...
async function createOrUpdateProfile(
	...
	}, { userId, reason: 'CREATE_OR_UPDATE_PROFILE' });

	log.info(`CREATE_OR_UPDATE_PROFILE profile:${profile.id} by user ${userId}`);
	return profile.id;
}
...
async function createOrUpdateLicense(
	...
	const license = await orgTransaction(organizationId, async (tx) => {
		if (licenseId) {
			return tx.contractorLicense.update({
				where: { id: licenseId },
				data: {
			...
			return tx.contractorLicense.create({
				data: {
	...
	}, { userId, reason: 'CREATE_OR_UPDATE_LICENSE' });

	log.info(`CREATE_OR_UPDATE_LICENSE license:${license.id} by user ${userId}`);
	return license.id;
}
...
async function createOrUpdateInsurance(
	...
	const insurance = await orgTransaction(organizationId, async (tx) => {
		if (insuranceId) {
			return tx.contractorInsurance.update({
			...
			return tx.contractorInsurance.create({
				data: {
					contractorProfileId: profileId,
	...
	}, { userId, reason: 'CREATE_OR_UPDATE_INSURANCE' });

	log.info(`CREATE_OR_UPDATE_INSURANCE insurance:${insurance.id} by user ${userId}`);
	return insurance.id;
}
...
async function contractorProfileWorkflow(input: ContractorProfileWorkflowInput): Promise<ContractorProfileWorkflowResult> {
	try {
		...
		await recordSpanError(errorObj, {
			...
			errorType: WorkflowErrorType.CONTRACTOR_PROFILE_WORKFLOW_ERROR
		});
