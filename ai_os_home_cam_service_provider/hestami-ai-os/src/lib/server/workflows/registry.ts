...
export type WorkflowStatus = 'current' | 'deprecated' | 'retired';
...
export function getCurrentWorkflowVersion(workflowName: string): WorkflowVersion | undefined {
	return WORKFLOW_REGISTRY.find(
		(w) => w.name === workflowName && w.status === 'current'
	);
}
...
export function getCurrentWorkflows(): WorkflowVersion[] {
	return WORKFLOW_REGISTRY.filter((w) => w.status === 'current');
}
...
export function getDeprecatedWorkflows(): WorkflowVersion[] {
	return WORKFLOW_REGISTRY.filter((w) => w.status === 'deprecated');
}
...
export function getWorkflowSummary(): {
	...
	return {
		...
		current: WORKFLOW_REGISTRY.filter((w) => w.status === 'current').length,
		deprecated: WORKFLOW_REGISTRY.filter((w) => w.status === 'deprecated').length,
		retired: WORKFLOW_REGISTRY.filter((w) => w.status === 'retired').length,
		byPhase
	};
}
