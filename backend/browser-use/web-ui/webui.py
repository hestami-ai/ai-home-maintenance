...
# Import browser-use core components
...
from src.agent.adapter import BrowserUseAdapter
from src.browser.adapter import BrowserAdapter
from src.utils.agent_state import AgentState
from src.utils import utils

# Keep the controller customization
...
async def run_org_agent(
    ...
    try:
        ...
        if _global_browser_context is None:
            # Ensure page load time parameters have default values if they're None
            min_wait = 0.5 if minimum_wait_page_load_time is None else float(minimum_wait_page_load_time)
            network_idle_wait = 1.0 if wait_for_network_idle_page_load_time is None else float(wait_for_network_idle_page_load_time)
            max_wait = 5.0 if maximum_wait_page_load_time is None else float(maximum_wait_page_load_time)
            vp_expansion = 500 if viewport_expansion is None else int(viewport_expansion)
            
            # Use the default browser-use BrowserContext implementation through our adapter
            from src.browser.adapter import BrowserAdapter
            _global_browser_context = await BrowserAdapter.create_browser_context(
                ...
                viewport_expansion=vp_expansion
            )
            
            # Set trace and recording paths if provided
            if save_trace_path:
                _global_browser_context.trace_path = save_trace_path
            if save_recording_path:
                _global_browser_context.save_recording_path = save_recording_path
...
async def run_custom_agent(
    ...
    try:
        ...
        # Keep using the CustomController as it contains your Gradio UI customizations
        controller = CustomController()

        # Initialize global browser if needed
        # if chrome_cdp not empty string nor None
        if (_global_browser is None) or (cdp_url and cdp_url != "" and cdp_url != None):
            # Use the default browser-use Browser implementation through our adapter
            from src.browser.adapter import BrowserAdapter
            _global_browser = await BrowserAdapter.create_browser(
                headless=headless,
                disable_security=disable_security,
                chrome_cdp=cdp_url,
                window_width=window_w,
        ...
        # collect optional system prompt overrides
        override_system_message = os.getenv("OVERRIDE_SYSTEM_MESSAGE", None)
        extend_system_message = os.getenv("EXTEND_SYSTEM_MESSAGE", None)
        
        # Set default value for use_vision_for_planner if not provided
        if use_vision_for_planner is None:
            use_vision_for_planner = False
        
        if _global_agent is None:
            # Use the default browser-use Agent implementation through our adapter
            from src.agent.adapter import BrowserUseAdapter
            _global_agent = await BrowserUseAdapter.create_agent(
                task=task,
                add_infos=add_infos,
                use_vision=use_vision,
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.TabItem("🌐 Browser Settings", id=3):
                with gr.Group():
                    ...
                    with gr.Row():
                        gr.Markdown("### Page Load Time Settings")
                    with gr.Row():
                        minimum_wait_page_load_time = gr.Number(
                            label="Minimum Wait Page Load Time (s)",
                            value=0.5,
                            precision=1,
                            info="Minimum time to wait before getting page state for LLM input",
                            interactive=True
                        )
                        wait_for_network_idle_page_load_time = gr.Number(
                            label="Wait For Network Idle Time (s)",
                            value=1.0,
                            precision=1,
                            info="Time to wait for network requests to finish before getting page state",
                            interactive=True
                        )
                        maximum_wait_page_load_time = gr.Number(
                            label="Maximum Wait Page Load Time (s)",
                            value=5.0,
                            precision=1,
                            info="Maximum time to wait for page load before proceeding anyway",
                            interactive=True
                        )
                    
                    with gr.Row():
                        viewport_expansion = gr.Number(
                            label="Viewport Expansion (pixels)",
                            value=500,
                            precision=0,
                            info="Expansion in pixels for elements included in the LLM state. -1 for all elements, 0 for only visible elements.",
                            interactive=True
                        )
            ...
            run_button.click(
                ...
                inputs=[
                    ...
                    # Add the new page load time parameters
                    minimum_wait_page_load_time, wait_for_network_idle_page_load_time, maximum_wait_page_load_time,
                    # Add viewport expansion parameter
                    viewport_expansion
                ],
                outputs=[
