...
/**
 * POST endpoint to select a bid for a service request
 * Maps to Django's /api/services/requests/<uuid:request_id>/bids/<uuid:bid_id>/select/ endpoint
 */
export async function POST({ params, request, cookies, url }: RequestEvent) {
  ...
  logApiRequest(
    ...
    request, 
    url, 
    `Select bid (ID: ${params.bidId}) for service request (ID: ${params.id}) from ${clientInfo.isMobile ? 'mobile' : 'web'} client${isMobileSession ? ' (using header session)' : ''}`
  );
  ...
  try {
    ...
    const response = await apiPost(
      cookies,
      `/api/services/requests/${params.id}/bids/${params.bidId}/select/`,
      requestData,
      {},
      url.pathname
    );
