...
/**
 * POST endpoint to track view of a service request
 * Maps to Django's /api/services/requests/<uuid:request_id>/view/ endpoint
 */
export async function POST({ params, request, cookies, url }: RequestEvent) {
  ...
  logApiRequest(
    ...
    request, 
    url, 
    `Track view for service request (ID: ${params.id}) from ${clientInfo.isMobile ? 'mobile' : 'web'} client${isMobileSession ? ' (using header session)' : ''}`
  );
  
  try {
    // Extract any additional data from request
    const viewData = request.headers.get('content-length') !== '0' ? await request.json() : {};
    
    // Forward to Django backend using server-side apiPost
    const response = await apiPost(
      cookies,
      `/api/services/requests/${params.id}/view/`,
      viewData,
      {},
      url.pathname
    );
