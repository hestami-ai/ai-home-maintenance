...
/**
 * POST endpoint to set interest in a service request
 * Maps to Django's /api/services/requests/<uuid:request_id>/interest/ endpoint
 */
export async function POST({ params, request, cookies, url }: RequestEvent) {
  ...
  logApiRequest(
    ...
    request, 
    url, 
    `Set interest for service request (ID: ${params.id}) from ${clientInfo.isMobile ? 'mobile' : 'web'} client${isMobileSession ? ' (using header session)' : ''}`
  );
  
  try {
    // Extract interest data from request
    const interestData = await request.json();
    
    // Forward to Django backend using server-side apiPost
    const response = await apiPost(
      cookies,
      `/api/services/requests/${params.id}/interest/`,
      interestData,
      {},
      url.pathname
    );
