...
/**
 * POST endpoint to start a service
 * Maps to Django's /api/services/requests/<uuid:request_id>/start/ endpoint
 */
export async function POST({ params, request, cookies, url }: RequestEvent) {
  ...
  logApiRequest(
    ...
    request, 
    url, 
    `Start service request (ID: ${params.id}) from ${clientInfo.isMobile ? 'mobile' : 'web'} client${isMobileSession ? ' (using header session)' : ''}`
  );
  ...
  try {
    ...
    const response = await apiPost(
      cookies,
      `/api/services/requests/${params.id}/start/`,
      requestData,
      {},
      url.pathname
    );
