...
CREATE TABLE IF NOT EXISTS gates (
    ...
    gate_id TEXT UNIQUE NOT NULL CHECK(length(gate_id) = 36),
    dialogue_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS intake_conversations (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    dialogue_id TEXT NOT NULL,
    sub_state TEXT NOT NULL CHECK(sub_state IN ('GATHERING','DISCUSSING','SYNTHESIZING','AWAITING_APPROVAL','INTENT_DISCOVERY','PRODUCT_REVIEW','PROPOSING','CLARIFYING','PROPOSING_BUSINESS_DOMAINS','PROPOSING_JOURNEYS','PROPOSING_ENTITIES','PROPOSING_INTEGRATIONS')) DEFAULT 'DISCUSSING',
...
CREATE TABLE intake_conversations_v6 (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    dialogue_id TEXT NOT NULL,
    sub_state TEXT NOT NULL CHECK(sub_state IN ('GATHERING','DISCUSSING','SYNTHESIZING','AWAITING_APPROVAL','INTENT_DISCOVERY','PRODUCT_REVIEW','PROPOSING','CLARIFYING')) DEFAULT 'DISCUSSING',
    turn_count INTEGER NOT NULL DEFAULT 0,
    draft_plan TEXT NOT NULL DEFAULT '{}',
    accumulations TEXT NOT NULL DEFAULT '[]',
    finalized_plan TEXT,
