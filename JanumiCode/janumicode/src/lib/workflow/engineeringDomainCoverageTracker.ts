...
import type {
	EngineeringDomainCoverageEntry,
 ...
 */
export function initializeCoverageMap(): EngineeringDomainCoverageMap {
	const map = {} as EngineeringDomainCoverageMap;
	for (const domain of Object.values(EngineeringDomain)) {
		map[domain] = {
			domain,
			level: EngineeringDomainCoverageLevel.NONE,
			evidence: [],
			turnNumbers: [],
		};
	}
...
export function updateCoverageFromText(
	map: EngineeringDomainCoverageMap,
	text: string,
	turnNumber: number,
): EngineeringDomainCoverageMap {
	...
	for (const domain of Object.values(EngineeringDomain)) {
		...
		if (entry.level === EngineeringDomainCoverageLevel.NONE && cumulativeHits >= PARTIAL_THRESHOLD) {
			entry.level = EngineeringDomainCoverageLevel.PARTIAL;
		}
		if (entry.level === EngineeringDomainCoverageLevel.PARTIAL && cumulativeHits >= ADEQUATE_THRESHOLD) {
			entry.level = EngineeringDomainCoverageLevel.ADEQUATE;
		}
	}
...
export async function updateCoverageFromLLM(
	...
	turnNumber: number,
): Promise<EngineeringDomainCoverageMap> {
...
function applyExtractedCoverage(
	...
	const level = isValidLevel(item.level) ? item.level as EngineeringDomainCoverageLevel : null;
...
export function updateCoverageFromExpert(
	...
	while ((match = pattern.exec(expertText)) !== null) {
		...
		const level = levelStr as EngineeringDomainCoverageLevel;
		const entry = updated[domain];
 ...
 */
export function getCoverageGaps(map: EngineeringDomainCoverageMap): EngineeringDomain[] {
	return Object.values(EngineeringDomain).filter(
		d => map[d].level === EngineeringDomainCoverageLevel.NONE,
	);
}
 ...
 */
export function getPartialDomains(map: EngineeringDomainCoverageMap): EngineeringDomain[] {
	return Object.values(EngineeringDomain).filter(
		d => map[d].level === EngineeringDomainCoverageLevel.PARTIAL,
	);
}
 ...
 */
export function getCoverageSummary(map: EngineeringDomainCoverageMap): {
	adequate: number;
	partial: number;
	none: number;
	percentage: number;
	...
	for (const domain of Object.values(EngineeringDomain)) {
		switch (map[domain].level) {
			...
			case EngineeringDomainCoverageLevel.NONE: none++; break;
		}
	}
...
export function getNextDomain(
	...
	if (currentEngineeringDomain === null) {
		return DOMAIN_SEQUENCE[0];
	}
	const idx = DOMAIN_SEQUENCE.indexOf(currentEngineeringDomain);
...
export function isDomainAdequatelyCovered(
	...
	return map[domain].level === EngineeringDomainCoverageLevel.ADEQUATE;
}
 ...
 */
export function formatCoverageSummaryForPrompt(map: EngineeringDomainCoverageMap): string {
	const lines: string[] = ['Domain Coverage Status:'];
	for (const domain of DOMAIN_SEQUENCE) {
		const entry = map[domain];
		const info = DOMAIN_INFO[domain];
		const icon = entry.level === EngineeringDomainCoverageLevel.ADEQUATE ? '[OK]'
			: entry.level === EngineeringDomainCoverageLevel.PARTIAL ? '[PARTIAL]'
				: '[NONE]';
 ...
 */
export function formatUncoveredDomainsForPrompt(map: EngineeringDomainCoverageMap): string {
	const gaps = getCoverageGaps(map);
	const partials = getPartialDomains(map);
...
export function seedCoverageFromAnalysis(
	...
	for (const item of engineeringDomainAssessment) {
...
/**
 * Resolve a level string to EngineeringDomainCoverageLevel.
 */
function resolveCoverageLevel(raw: string): EngineeringDomainCoverageLevel | null {
	const upper = raw.toUpperCase();
	if (upper === 'ADEQUATE') { return EngineeringDomainCoverageLevel.ADEQUATE; }
	if (upper === 'PARTIAL') { return EngineeringDomainCoverageLevel.PARTIAL; }
	if (upper === 'NONE') { return EngineeringDomainCoverageLevel.NONE; }
	return null;
}

// ==================== PRIVATE HELPERS ====================

function deepCloneMap(map: EngineeringDomainCoverageMap): EngineeringDomainCoverageMap {
	return JSON.parse(JSON.stringify(map)) as EngineeringDomainCoverageMap;
}
...
function isValidLevel(str: string): boolean {
	return Object.values(EngineeringDomainCoverageLevel).includes(str as EngineeringDomainCoverageLevel);
}

function levelRank(level: EngineeringDomainCoverageLevel): number {
	switch (level) {
		case EngineeringDomainCoverageLevel.NONE: return 0;
		case EngineeringDomainCoverageLevel.PARTIAL: return 1;
		case EngineeringDomainCoverageLevel.ADEQUATE: return 2;
	}
}
