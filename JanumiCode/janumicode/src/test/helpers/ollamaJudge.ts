 ...
 *
 * Calls gemma4:26b a second time after a generator probe, asking it to score
 * the generated output against a rubric. Captures the judge's own thinking
 * trace too. Lower temperature than the generator to keep verdicts stable.
 */
...
export interface JudgeRubric {
	name: string;
	criteria: string[];
	/** Optional extra criteria specifically for grading the generator's reasoning trace. */
...
export interface JudgeVerdict {
	...
	issues: string[];
	rationale: string;
	judgeThinking?: string;              // judge's own reasoning trace
	rawText: string;                     // raw judge response (debug)
}
