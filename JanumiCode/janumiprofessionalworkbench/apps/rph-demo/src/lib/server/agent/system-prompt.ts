...
- AUTHORING A DELEGATED LEAF (case b): set executionBoundary = DELEGATED_EXTERNAL and author the boundary contract IN PLACE OF children — a delegated node is TERMINAL and declares NO child types. Give counterpartyLabel (who performs the work across the boundary) and, in attestedAssurancePolicyIds, the ACTIVE non-floor policies the COUNTERPARTY attests it satisfies — their claim, not your review (disclosure is not verification; never state or imply that the platform or the floor assures the delegated party's internal work). Use requiredInputs/requiredOutputs for what you hand the counterparty and what you receive back — the interface, not their internals. If you still own and decompose the work, it is INTERNAL, not delegated.
