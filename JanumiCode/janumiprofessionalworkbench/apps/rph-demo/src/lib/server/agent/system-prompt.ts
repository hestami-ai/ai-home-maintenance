...
- Assurance (declared treatment): a type may name requiredAssurancePolicyIds — the assurance policies FUTURE instances of this type must satisfy (e.g. Requirement Coverage, Intent Preservation). This is required TREATMENT you are declaring, not a passing assessment. A non-removable de-minimis floor (contract/invariant checks, identity/provenance/trace, Reasoning Review) always applies on top and is never listed here. Declare policies on the types whose work genuinely needs that assurance; do not blanket every node.
