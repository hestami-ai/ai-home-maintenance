...
export const PRODUCT_REALIZATION_PWA_ONTOLOGY = {
	...
	pwuTemplates: [
		...
		},
		{
			...
			isRoot: false,
			purpose: 'Define a coherent architecture concern contributing to Architecture Definition.',
			defaultPolicyIds: ['pol_architecture_coverage'],
			sourceSection:
				"AUTHORED 2026-08-05 (standing grant), MINIMAL BY CONSTRUCTION. The corpus NAMES this PWU Type — the PWA Designer and Undertaking Workbench Reference Demonstration lists 'Custom Architecture Concern' among Architecture Definition's permitted children and gives the inherited purpose transcribed VERBATIM above — but this dataset had no row for it, so a kind the workbench seeds five instances of declared no policies and could not be assessed once DOC-004 §5.1 is enforced (REG-F-029). defaultPolicyIds is read off DOC-006 §32.2's trace (concern -> Architecture Coverage Claim -> Architecture Assurance Assessment). DELIBERATELY OMITS inputs, outputArtifactTypes, completionClaims, requiredEvidenceTypes and candidateChildren: every one would be invented, all five are optional on PwuTemplate, and this row exists to close a policy gap — not to describe work the corpus has not described."
		}
	],
	seedPolicies: [
		{
			...
			sourceSection:
				'DOC-004 §15 (POL-INTENT-FIDELITY). TRANSCRIBED from the ratified doc and machine-checked against it by doc004-conformance.test.ts: purpose (§15.2); the 5 criterion descriptions are "15.6 Criteria" verbatim (leading character sentence-cased — the doc\'s numbered lists are inconsistently cased; no word added, removed, or reordered); the 7 finding codes are §15.7 verbatim in document order. Criterion ids AND names are ratified by "15.6 Criteria" ("### ID Name" headings). permittedControlActions transcribed from §15.10. AUTHORED (no ratified source): rationale, evaluatorRole, evaluatedClaimTypes, appliesToPwuKinds, failureSeverity, independenceRequirement where the section has no Independence subsection, each criterion\'s criterionType/evaluationMethod/severityIfNotMet/mayBeNotApplicable, and findingAnnotations (DOC-004 §9.1 mandates FindingDefinition.description + defaultSeverity and ratifies neither for ANY of its 99 codes — an open corpus gap; unannotated codes fall back to the policy\'s failureSeverity and the humanized code). EVIDENCE (REG-E-026, authored 2026-08-05 under sponsor ratification): the 7 items of DOC-004 §15\'s "Required evidence" subsection are TRANSCRIBED verbatim (leading character sentence-cased, the same documented normalization the criteria use) into `requiredEvidence`. THE FIELD IS CHOSEN BY THE CORPUS\'S OWN HEADING — §3.1 declares both requiredEvidence and optionalEvidence, two sections say "Required evidence" and nine say "Evidence"; calling all eleven required would add a word to nine sections that do not use it. DERIVED: id = EV-<section>-<NN> in document order; cardinality = ZERO_OR_MORE where the item is hedged ("where applicable"/"where relevant") OR where its referent is CONTINGENT — it exists only if an optional exchange actually occurred, and no ratified transition guard or mandatory schema field makes its record unavoidable (four items, each named with its reason in doc004-conformance.test.ts) — else AT_LEAST_ONE. THE CONTINGENT CLAUSE IS AUTHORED JUDGEMENT AND THE MOST CONSEQUENTIAL CALL HERE: under a blanket AT_LEAST_ONE, an intent captured from one clear user expression could never reach a SATISFIED fidelity verdict for want of a clarification dialogue that never happened. ⚠ ITS FIRST DRAFT CARRIED A SECOND ARM — "or the system has no object type that can produce it" — WHICH WAS INVALID AND IS WITHDRAWN (adversarial review, 2026-08-05): an Evidence object needs no same-named object type (contentReference is an open record), so the arm would equally have voided EV-15-01, EV-16-05 and EV-16-06, which do gate — it emptied the gate instead of discriminating within it. DOC-002 §6.2\'s intent transition matrix is the discriminator that survives: it makes "known ambiguities recorded" a GUARD on the only path to the assessed state (so EV-16-02 gates) and names stakeholders nowhere (so EV-16-04 does not); requiredForDispositions = SATISFIED_ONLY on §15.9 ("SATISFIED only when ... required evidence is admissible" — named as a condition of the verdict, not of assessing); mayBeWaived = false, fail-closed, because the corpus nowhere states that a requirement may be waived. HONESTLY EMPTY: admissibilityRules = [] (§6.2 states admissibility globally, not per requirement); freshnessRule omitted (optional, and no policy states one). AUTHORED, and the only genuinely new content: each item\'s evidenceType classification and its purpose sentence. SUPERSEDES requiredEvidenceTypes, removed here — a 2-3 member authored summary that nothing read, now stated at 89x the resolution and machine-checked against the document by doc004-conformance.test.ts.',
			findingAnnotations: {
				SOLUTION_SUBSTITUTION: {
			...
			requiredEvidence: [
				{
					id: 'EV-15-01',
					evidenceType: 'SOURCE',
					description: 'Originating expression',
					purpose:
						"The user's own words are the referent every fidelity criterion is measured against; without them IF-01 cannot distinguish representing a need from substituting a preferred solution.",
				...
				},
				{
					id: 'EV-15-02',
					evidenceType: 'SOURCE',
					description: 'Clarification dialogue',
					purpose:
						'Records what was asked and what was answered, so an interpretation can be traced to a user response rather than to the synthesizer’s inference (IF-04).',
				...
				},
				{
					id: 'EV-15-03',
					evidenceType: 'SOURCE',
					description: 'Supplied documents',
					purpose:
						'Material the user provided as authoritative; scope and constraints asserted here bind the intent (IF-02, IF-03).',
					cardinality: 'ZERO_OR_MORE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Proposed Product Intent',
					purpose:
						'The formalization under assessment — the object whose fidelity to the originating expression is the entire question.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'The explicit user constraints IF-03 requires be preserved; without the constraint set, preservation cannot be checked at all.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-15-06',
					evidenceType: 'SOURCE',
					description: 'Recorded user corrections',
					purpose:
						'A correction is the user exercising authority over an interpretation; an intent that contradicts one has drifted from the need (IF-01).',
					cardinality: 'ZERO_OR_MORE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-15-07',
					evidenceType: 'ARTIFACT',
					description: 'Prior intent version where applicable',
					purpose:
						'Where the intent is a revision, its predecessor is what makes an unauthorized change visible — of scope (IF-02) or of a constraint (IF-03).',
					cardinality: 'ZERO_OR_MORE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
		...
		},
		{
			...
			permittedControlActions: ['GATHER_CONTEXT', 'RESHAPE_PWU', 'REQUEST_HUMAN_DECISION'],
			sourceSection:
				'DOC-004 §16 (POL-INTENT-COMPLETENESS). TRANSCRIBED from the ratified doc and machine-checked against it by doc004-conformance.test.ts: purpose (§16.1); the 6 criterion descriptions are "16.3 Claims evaluated" verbatim (leading character sentence-cased — the doc\'s numbered lists are inconsistently cased; no word added, removed, or reordered); the 7 finding codes are §16.5 verbatim in document order. PRECEDENTED DERIVATION: "16.3 Claims evaluated" ratifies neither ids nor names, so ids are minted by ordinal and name = the id. Only §15.6 and §19.5 have a Criteria subsection; the other 10 policies fall back this way. AUTHORED permittedControlActions: this section ratifies NO control actions (only §15.10, §17.8, §19.8 and §23.7 do). RAISED under the sponsor\'s standing grant (2026-07-18; HARMONIZATION-LOG PART 5) to include the control-action FLOOR {REQUEST_HUMAN_DECISION, RESHAPE_PWU} — the intersection of the four ratified control-action sets, which is also the principled universal minimum: a policy that can raise a blocking finding must be able to escalate to a human (REQUEST_HUMAN_DECISION) rather than invent a resolution, and to take the minimal governed corrective action (RESHAPE_PWU). This policy under-declared the floor before; its prior authored value is RETAINED and unioned with it. Still AUTHORED — the floor is derived, not ratified here. AUTHORED (no ratified source): rationale, evaluatorRole, evaluatedClaimTypes, appliesToPwuKinds, failureSeverity, independenceRequirement where the section has no Independence subsection, each criterion\'s criterionType/evaluationMethod/severityIfNotMet/mayBeNotApplicable, and findingAnnotations (DOC-004 §9.1 mandates FindingDefinition.description + defaultSeverity and ratifies neither for ANY of its 99 codes — an open corpus gap; unannotated codes fall back to the policy\'s failureSeverity and the humanized code). EVIDENCE (REG-E-026, authored 2026-08-05 under sponsor ratification): the 6 items of DOC-004 §16\'s "Required evidence" subsection are TRANSCRIBED verbatim (leading character sentence-cased, the same documented normalization the criteria use) into `requiredEvidence`. THE FIELD IS CHOSEN BY THE CORPUS\'S OWN HEADING — §3.1 declares both requiredEvidence and optionalEvidence, two sections say "Required evidence" and nine say "Evidence"; calling all eleven required would add a word to nine sections that do not use it. DERIVED: id = EV-<section>-<NN> in document order; cardinality = ZERO_OR_MORE where the item is hedged ("where applicable"/"where relevant") OR where its referent is CONTINGENT — it exists only if an optional exchange actually occurred, and no ratified transition guard or mandatory schema field makes its record unavoidable (four items, each named with its reason in doc004-conformance.test.ts) — else AT_LEAST_ONE. THE CONTINGENT CLAUSE IS AUTHORED JUDGEMENT AND THE MOST CONSEQUENTIAL CALL HERE: under a blanket AT_LEAST_ONE, an intent captured from one clear user expression could never reach a SATISFIED fidelity verdict for want of a clarification dialogue that never happened. ⚠ ITS FIRST DRAFT CARRIED A SECOND ARM — "or the system has no object type that can produce it" — WHICH WAS INVALID AND IS WITHDRAWN (adversarial review, 2026-08-05): an Evidence object needs no same-named object type (contentReference is an open record), so the arm would equally have voided EV-15-01, EV-16-05 and EV-16-06, which do gate — it emptied the gate instead of discriminating within it. DOC-002 §6.2\'s intent transition matrix is the discriminator that survives: it makes "known ambiguities recorded" a GUARD on the only path to the assessed state (so EV-16-02 gates) and names stakeholders nowhere (so EV-16-04 does not); requiredForDispositions = SATISFIED_ONLY on §15.9 ("SATISFIED only when ... required evidence is admissible" — named as a condition of the verdict, not of assessing); mayBeWaived = false, fail-closed, because the corpus nowhere states that a requirement may be waived. HONESTLY EMPTY: admissibilityRules = [] (§6.2 states admissibility globally, not per requirement); freshnessRule omitted (optional, and no policy states one). AUTHORED, and the only genuinely new content: each item\'s evidenceType classification and its purpose sentence. SUPERSEDES requiredEvidenceTypes, removed here — a 2-3 member authored summary that nothing read, now stated at 89x the resolution and machine-checked against the document by doc004-conformance.test.ts.',
			findingAnnotations: {
				MISSING_DESIRED_OUTCOME: {
					defaultSeverity: 'MATERIAL',
					description:
			...
			requiredEvidence: [
				{
					id: 'EV-16-01',
					evidenceType: 'ARTIFACT',
					description: 'Product Intent',
					purpose:
						'The intent whose completeness is assessed; every claim in §16.3 is evaluated over it.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-16-02',
					evidenceType: 'ARTIFACT',
					description: 'Ambiguity catalog',
					purpose:
						'Claim 6 asks whether major ambiguities have dispositions; an ambiguity absent from the catalog is the FALSE_COMPLETENESS this policy exists to catch.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-16-03',
					evidenceType: 'ARTIFACT',
					description: 'Constraint catalog',
					purpose:
						'Claim 4 requires that mandatory constraints be recorded; the catalog is where that is decidable.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-16-04',
					evidenceType: 'ARTIFACT',
					description: 'Stakeholder and actor catalog',
					purpose:
						'Claim 3 asks whether known stakeholders and actors are represented proportionally.',
					cardinality: 'ZERO_OR_MORE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Explicit exclusions bound the product; without them UNBOUNDED_PRODUCT_SCOPE cannot be distinguished from deliberate breadth (claim 2).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Completeness is proportional to risk, consequence, uncertainty and irreversibility (§16.1), so the profile sets the standard the other claims are judged against.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
		...
		},
		{
			...
			sourceSection:
				'DOC-004 §17 (POL-ASSUMPTION-DISCLOSURE). TRANSCRIBED from the ratified doc and machine-checked against it by doc004-conformance.test.ts: purpose (§17.1); the 6 criterion descriptions are "17.3 Claims evaluated" verbatim (leading character sentence-cased — the doc\'s numbered lists are inconsistently cased; no word added, removed, or reordered); the 7 finding codes are §17.5 verbatim in document order. PRECEDENTED DERIVATION: "17.3 Claims evaluated" ratifies neither ids nor names, so ids are minted by ordinal and name = the id. Only §15.6 and §19.5 have a Criteria subsection; the other 10 policies fall back this way. permittedControlActions transcribed from §17.8. AUTHORED (no ratified source): rationale, evaluatorRole, evaluatedClaimTypes, appliesToPwuKinds, failureSeverity, independenceRequirement where the section has no Independence subsection, each criterion\'s criterionType/evaluationMethod/severityIfNotMet/mayBeNotApplicable, and findingAnnotations (DOC-004 §9.1 mandates FindingDefinition.description + defaultSeverity and ratifies neither for ANY of its 99 codes — an open corpus gap; unannotated codes fall back to the policy\'s failureSeverity and the humanized code). EVIDENCE (REG-E-026, authored 2026-08-05 under sponsor ratification): the 7 items of DOC-004 §17\'s "Evidence" subsection are TRANSCRIBED verbatim (leading character sentence-cased, the same documented normalization the criteria use) into `optionalEvidence`. THE FIELD IS CHOSEN BY THE CORPUS\'S OWN HEADING — §3.1 declares both requiredEvidence and optionalEvidence, two sections say "Required evidence" and nine say "Evidence"; calling all eleven required would add a word to nine sections that do not use it. DERIVED: id = EV-<section>-<NN> in document order; cardinality = ZERO_OR_MORE where the item is hedged ("where applicable"/"where relevant") OR where its referent is CONTINGENT — it exists only if an optional exchange actually occurred, and no ratified transition guard or mandatory schema field makes its record unavoidable (four items, each named with its reason in doc004-conformance.test.ts) — else AT_LEAST_ONE. THE CONTINGENT CLAUSE IS AUTHORED JUDGEMENT AND THE MOST CONSEQUENTIAL CALL HERE: under a blanket AT_LEAST_ONE, an intent captured from one clear user expression could never reach a SATISFIED fidelity verdict for want of a clarification dialogue that never happened. ⚠ ITS FIRST DRAFT CARRIED A SECOND ARM — "or the system has no object type that can produce it" — WHICH WAS INVALID AND IS WITHDRAWN (adversarial review, 2026-08-05): an Evidence object needs no same-named object type (contentReference is an open record), so the arm would equally have voided EV-15-01, EV-16-05 and EV-16-06, which do gate — it emptied the gate instead of discriminating within it. DOC-002 §6.2\'s intent transition matrix is the discriminator that survives: it makes "known ambiguities recorded" a GUARD on the only path to the assessed state (so EV-16-02 gates) and names stakeholders nowhere (so EV-16-04 does not); requiredForDispositions = SATISFIED_ONLY on §15.9 ("SATISFIED only when ... required evidence is admissible" — named as a condition of the verdict, not of assessing); mayBeWaived = false, fail-closed, because the corpus nowhere states that a requirement may be waived. HONESTLY EMPTY: admissibilityRules = [] (§6.2 states admissibility globally, not per requirement); freshnessRule omitted (optional, and no policy states one). AUTHORED, and the only genuinely new content: each item\'s evidenceType classification and its purpose sentence. SUPERSEDES requiredEvidenceTypes, removed here — a 2-3 member authored summary that nothing read, now stated at 89x the resolution and machine-checked against the document by doc004-conformance.test.ts. SCOPE WIDENED 2026-08-06 (REG-F-033 deepening): the kinds below were added because their pwuTemplate rows already DECLARE this policy in defaultPolicyIds while this list did not name them. In every case the PARENT kind was listed and the CHILD was not — this list was authored against the NINE PUBLISHED PWU Types, so the six unpublished templates were outside its field of view, and deepening the tree is what surfaced it. WIDENED RATHER THAN NARROWED deliberately: dropping the policy from the template would silently exempt work from assurance, which is the direction REG-F-024 names as forbidden. Both lists are AUTHORED and state one fact twice; a bidirectional gate now holds them together. Added here: INTENT_DISCOVERY.',
			findingAnnotations: {
				HIDDEN_MATERIAL_ASSUMPTION: {
					defaultSeverity: 'MATERIAL',
					description:
			...
			optionalEvidence: [
				{
					id: 'EV-17-01',
					evidenceType: 'ARTIFACT',
					description: 'Artifact under assessment',
					purpose:
						'The work whose premises are being surfaced; assumptions are read out of it rather than asserted about it.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Execution rationale',
					purpose:
						'The reasoning that produced the artifact is where an unstated premise is most often visible (claim 1).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-17-03',
					evidenceType: 'SOURCE',
					description: 'Prompt and context provenance where relevant',
					purpose:
						'What the agent was given bounds what it could have known; a premise absent from the context was supplied by the model, not by the record (claim 2).',
					cardinality: 'ZERO_OR_MORE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 2 requires assumptions be distinguished from facts; the established fact set is the other side of that distinction.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Constraints',
					purpose:
						'Claim 6 asks that critical assumptions do not silently authorize irreversible work; a premise contradicting an active constraint is the clearest case of an assumption doing exactly that.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'An authorized decision is not an assumption; without the decision record the two are indistinguishable (claim 2).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'A premise attributed to an outside source is verifiable only against that source (claim 5).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
			...
			optionalEvidence: [
				{
					...
					purpose:
						'The authorized outcomes that requirements must cover; coverage is meaningless without the thing being covered (claim 1).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-18-02',
					evidenceType: 'ARTIFACT',
					description: 'Outcome catalog',
					purpose:
						'Claim 1 asks whether each mandatory outcome is represented; the catalog is what enumerates them.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose: 'UNCOVERED_CAPABILITY is decidable only against a declared capability set.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose: 'Claim 2 asks whether each critical journey has applicable requirements.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 4 concerns important failure paths, which live in scenarios rather than in headline outcomes.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Constraints',
					purpose:
						'Claim 3 requires each mandatory constraint create enforceable obligations; CONSTRAINT_WITHOUT_OBLIGATION is decided here.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-18-07',
					evidenceType: 'ARTIFACT',
					description: 'Requirement catalog',
					purpose:
						'The requirement set under assessment — the covering side of every coverage claim.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Coverage is a relation, not a count; the matrix is what makes ORPHAN_REQUIREMENT and UNCOVERED_OUTCOME separately decidable (claim 5).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
		...
		},
		{
			...
			sourceSection:
				'DOC-004 §19 (POL-DECOMPOSITION-COVERAGE). TRANSCRIBED from the ratified doc and machine-checked against it by doc004-conformance.test.ts: purpose (§19.1); the 7 criterion descriptions are "19.5 Criteria" verbatim (leading character sentence-cased — the doc\'s numbered lists are inconsistently cased; no word added, removed, or reordered); the 10 finding codes are §19.6 verbatim in document order. Criterion ids AND names are ratified by "19.5 Criteria" ("### ID Name" headings). permittedControlActions transcribed from §19.8. AUTHORED (no ratified source): rationale, evaluatorRole, evaluatedClaimTypes, appliesToPwuKinds, failureSeverity, independenceRequirement where the section has no Independence subsection, each criterion\'s criterionType/evaluationMethod/severityIfNotMet/mayBeNotApplicable, and findingAnnotations (DOC-004 §9.1 mandates FindingDefinition.description + defaultSeverity and ratifies neither for ANY of its 99 codes — an open corpus gap; unannotated codes fall back to the policy\'s failureSeverity and the humanized code). EVIDENCE (REG-E-026, authored 2026-08-05 under sponsor ratification): the 9 items of DOC-004 §19\'s "Evidence" subsection are TRANSCRIBED verbatim (leading character sentence-cased, the same documented normalization the criteria use) into `optionalEvidence`. THE FIELD IS CHOSEN BY THE CORPUS\'S OWN HEADING — §3.1 declares both requiredEvidence and optionalEvidence, two sections say "Required evidence" and nine say "Evidence"; calling all eleven required would add a word to nine sections that do not use it. DERIVED: id = EV-<section>-<NN> in document order; cardinality = ZERO_OR_MORE where the item is hedged ("where applicable"/"where relevant") OR where its referent is CONTINGENT — it exists only if an optional exchange actually occurred, and no ratified transition guard or mandatory schema field makes its record unavoidable (four items, each named with its reason in doc004-conformance.test.ts) — else AT_LEAST_ONE. THE CONTINGENT CLAUSE IS AUTHORED JUDGEMENT AND THE MOST CONSEQUENTIAL CALL HERE: under a blanket AT_LEAST_ONE, an intent captured from one clear user expression could never reach a SATISFIED fidelity verdict for want of a clarification dialogue that never happened. ⚠ ITS FIRST DRAFT CARRIED A SECOND ARM — "or the system has no object type that can produce it" — WHICH WAS INVALID AND IS WITHDRAWN (adversarial review, 2026-08-05): an Evidence object needs no same-named object type (contentReference is an open record), so the arm would equally have voided EV-15-01, EV-16-05 and EV-16-06, which do gate — it emptied the gate instead of discriminating within it. DOC-002 §6.2\'s intent transition matrix is the discriminator that survives: it makes "known ambiguities recorded" a GUARD on the only path to the assessed state (so EV-16-02 gates) and names stakeholders nowhere (so EV-16-04 does not); requiredForDispositions = SATISFIED_ONLY on §15.9 ("SATISFIED only when ... required evidence is admissible" — named as a condition of the verdict, not of assessing); mayBeWaived = false, fail-closed, because the corpus nowhere states that a requirement may be waived. HONESTLY EMPTY: admissibilityRules = [] (§6.2 states admissibility globally, not per requirement); freshnessRule omitted (optional, and no policy states one). AUTHORED, and the only genuinely new content: each item\'s evidenceType classification and its purpose sentence. SUPERSEDES requiredEvidenceTypes, removed here — a 2-3 member authored summary that nothing read, now stated at 89x the resolution and machine-checked against the document by doc004-conformance.test.ts.',
			findingAnnotations: {
				MISSING_OBLIGATION_ALLOCATION: {
					defaultSeverity: 'BLOCKING',
					description:
			...
			optionalEvidence: [
				{
					id: 'EV-19-01',
					evidenceType: 'ARTIFACT',
					description: 'Parent PWU',
					purpose: 'The work being decomposed; every coverage claim is relative to it.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 6 asks whether the decomposition preserves intent, which is a property of the parent’s intent rather than of any child.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-19-03',
					evidenceType: 'ARTIFACT',
					description: 'Parent obligations',
					purpose:
						'Claim 1 requires every mandatory obligation be allocated, retained, satisfied or waived; the obligation set is that claim’s domain.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Constraints',
					purpose:
						'DC-02 requires applicable constraints be propagated or retained across the decomposition.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'An assumption held by the parent and dropped by every child is an unallocated premise, not merely a missing note.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-19-06',
					evidenceType: 'ARTIFACT',
					description: 'Child PWUs',
					purpose:
						'The covering set; claim 2 asks whether each child has a coherent professional purpose.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'The parent-obligation-to-child mapping — the relation DC-01’s "no mandatory obligation silently disappears" is checked over.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 3 requires sibling dependencies be explicit rather than discovered during recomposition.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-19-09',
					evidenceType: 'ARTIFACT',
					description: 'Recomposition contract',
					purpose:
						'Claim 5 asks whether recomposition is feasible; the contract is where that is asserted and therefore checkable.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
		...
		},
		{
			...
			appliesToPwuKinds: ['ARCHITECTURE_DEFINITION', 'ARCHITECTURE_CONCERN'],
			criteria: [
				{
					id: 'AC-01',
					name: 'AC-01',
			...
			sourceSection:
				'DOC-004 §21 (POL-ARCHITECTURE-COVERAGE). TRANSCRIBED from the ratified doc and machine-checked against it by doc004-conformance.test.ts: purpose (§21.1); the 10 criterion descriptions are "21.3 Claims evaluated" verbatim (leading character sentence-cased — the doc\'s numbered lists are inconsistently cased; no word added, removed, or reordered); the 12 finding codes are §21.5 verbatim in document order. PRECEDENTED DERIVATION: "21.3 Claims evaluated" ratifies neither ids nor names, so ids are minted by ordinal and name = the id. Only §15.6 and §19.5 have a Criteria subsection; the other 10 policies fall back this way. AUTHORED permittedControlActions: this section ratifies NO control actions (only §15.10, §17.8, §19.8 and §23.7 do). RAISED under the sponsor\'s standing grant (2026-07-18; HARMONIZATION-LOG PART 5) to include the control-action FLOOR {REQUEST_HUMAN_DECISION, RESHAPE_PWU} — the intersection of the four ratified control-action sets, which is also the principled universal minimum: a policy that can raise a blocking finding must be able to escalate to a human (REQUEST_HUMAN_DECISION) rather than invent a resolution, and to take the minimal governed corrective action (RESHAPE_PWU). This policy under-declared the floor before; its prior authored value is RETAINED and unioned with it. Still AUTHORED — the floor is derived, not ratified here. AUTHORED (no ratified source): rationale, evaluatorRole, evaluatedClaimTypes, appliesToPwuKinds, failureSeverity, independenceRequirement where the section has no Independence subsection, each criterion\'s criterionType/evaluationMethod/severityIfNotMet/mayBeNotApplicable, and findingAnnotations (DOC-004 §9.1 mandates FindingDefinition.description + defaultSeverity and ratifies neither for ANY of its 99 codes — an open corpus gap; unannotated codes fall back to the policy\'s failureSeverity and the humanized code).\n\nARCHITECTURE_CONCERN ADDED TO appliesToPwuKinds — AUTHORED 2026-08-05 under the standing grant, DERIVED FROM DOC-006 §32.2\'s ratified trace and disclosed as authoring rather than delivery. The corpus routes a concern\'s assurance through THIS policy by name: §32.2 traces \'Scheduling and Dispatch Architecture Concern -> PRODUCES -> Architecture Artifact -> SUPPORTS -> Architecture Coverage Claim -> VERIFIED_BY -> Architecture Assurance Assessment\', and this policy is named \'Architecture Coverage\' and evaluates COVERAGE claims. The concern\'s own corpus purpose is \'contributing to Architecture Definition\', the kind this policy already governed. WHAT IS AUTHORED IS THE MEMBERSHIP; the routing is read off the trace. It closes REG-F-029\'s blocking gap: the corpus NAMES the Architecture Concern type and names no policy for it, so without this the five seeded concerns declare nothing and cannot be assessed at all once DOC-004 §5.1 is enforced. EVIDENCE (REG-E-026, authored 2026-08-05 under sponsor ratification): the 11 items of DOC-004 §21\'s "Evidence" subsection are TRANSCRIBED verbatim (leading character sentence-cased, the same documented normalization the criteria use) into `optionalEvidence`. THE FIELD IS CHOSEN BY THE CORPUS\'S OWN HEADING — §3.1 declares both requiredEvidence and optionalEvidence, two sections say "Required evidence" and nine say "Evidence"; calling all eleven required would add a word to nine sections that do not use it. DERIVED: id = EV-<section>-<NN> in document order; cardinality = ZERO_OR_MORE where the item is hedged ("where applicable"/"where relevant") OR where its referent is CONTINGENT — it exists only if an optional exchange actually occurred, and no ratified transition guard or mandatory schema field makes its record unavoidable (four items, each named with its reason in doc004-conformance.test.ts) — else AT_LEAST_ONE. THE CONTINGENT CLAUSE IS AUTHORED JUDGEMENT AND THE MOST CONSEQUENTIAL CALL HERE: under a blanket AT_LEAST_ONE, an intent captured from one clear user expression could never reach a SATISFIED fidelity verdict for want of a clarification dialogue that never happened. ⚠ ITS FIRST DRAFT CARRIED A SECOND ARM — "or the system has no object type that can produce it" — WHICH WAS INVALID AND IS WITHDRAWN (adversarial review, 2026-08-05): an Evidence object needs no same-named object type (contentReference is an open record), so the arm would equally have voided EV-15-01, EV-16-05 and EV-16-06, which do gate — it emptied the gate instead of discriminating within it. DOC-002 §6.2\'s intent transition matrix is the discriminator that survives: it makes "known ambiguities recorded" a GUARD on the only path to the assessed state (so EV-16-02 gates) and names stakeholders nowhere (so EV-16-04 does not); requiredForDispositions = SATISFIED_ONLY on §15.9 ("SATISFIED only when ... required evidence is admissible" — named as a condition of the verdict, not of assessing); mayBeWaived = false, fail-closed, because the corpus nowhere states that a requirement may be waived. HONESTLY EMPTY: admissibilityRules = [] (§6.2 states admissibility globally, not per requirement); freshnessRule omitted (optional, and no policy states one). AUTHORED, and the only genuinely new content: each item\'s evidenceType classification and its purpose sentence. SUPERSEDES requiredEvidenceTypes, removed here — a 2-3 member authored summary that nothing read, now stated at 89x the resolution and machine-checked against the document by doc004-conformance.test.ts.',
			findingAnnotations: {
				UNCOVERED_REQUIREMENT: {
					defaultSeverity: 'MATERIAL',
					description:
			...
			optionalEvidence: [
				{
					...
					description: 'Approved intent',
					purpose:
						'The product obligations the architecture must provide a coherent structure for (§21.1).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 1 asks whether applicable requirements are allocated to architectural elements.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Journeys cross component boundaries, so they expose an architecture that satisfies every requirement and no path (claim 3).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-21-04',
					evidenceType: 'ARTIFACT',
					description: 'Architecture artifacts',
					purpose: 'The architecture under assessment.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'An architecture choice without its decision record cannot be distinguished from an accident (claim 9).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 4 asks whether interfaces are sufficiently defined; MISSING_INTERFACE_DEFINITION is decided here.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 5 requires data ownership be explicit; AMBIGUOUS_DATA_OWNERSHIP is decided against the model.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose: 'Claim 7 asks whether operational and deployment concerns are proportionate.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 6 requires security boundaries be represented; a boundary is a conclusion of analysis, not an artifact that declares itself.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Allocation is a relation between requirements and components; UNCOVERED_REQUIREMENT is decidable only over it.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-21-11',
					evidenceType: 'ARTIFACT',
					description: 'Assumption catalog',
					purpose: 'Claim 9 requires known architecture risks and assumptions be explicit.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
			...
			optionalEvidence: [
				{
					...
					purpose: 'Claim 3 requires active prior decisions be respected or formally superseded.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-22-02',
					evidenceType: 'SOURCE',
					description: 'Narrative memories',
					purpose:
						'How the work reached its present state — the referent for CHESTERTONS_FENCE_RISK, which cannot be judged from the current state alone.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose: 'Known failures are what KNOWN_FAILURE_RECURRENCE is measured against.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-22-04',
					evidenceType: 'REVIEW',
					description: 'Previous assurance findings produced through validator implementations',
					purpose:
						'A finding raised before and left unaddressed is a recurrence, not a new observation.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-22-05',
					evidenceType: 'SOURCE',
					description: 'Git or repository history',
					purpose: 'The record of what was actually done, as distinct from what was decided.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-22-06',
					evidenceType: 'ARTIFACT',
					description: 'Prior baselines',
					purpose: 'The last authoritative state; divergence is measured from it (claim 4).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'MISSING_DESIGN_RATIONALE is decided here; a fence with no recorded reason is this policy’s central risk.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
		...
		},
		{
			...
			sourceSection:
				'DOC-004 §23 (POL-INTENT-PRESERVATION). TRANSCRIBED from the ratified doc and machine-checked against it by doc004-conformance.test.ts: purpose (§23.1); the 7 criterion descriptions are "23.3 Claims evaluated" verbatim (leading character sentence-cased — the doc\'s numbered lists are inconsistently cased; no word added, removed, or reordered); the 8 finding codes are §23.5 verbatim in document order. PRECEDENTED DERIVATION: "23.3 Claims evaluated" ratifies neither ids nor names, so ids are minted by ordinal and name = the id. Only §15.6 and §19.5 have a Criteria subsection; the other 10 policies fall back this way. permittedControlActions transcribed from §23.7. AUTHORED (no ratified source): rationale, evaluatorRole, evaluatedClaimTypes, appliesToPwuKinds, failureSeverity, independenceRequirement where the section has no Independence subsection, each criterion\'s criterionType/evaluationMethod/severityIfNotMet/mayBeNotApplicable, and findingAnnotations (DOC-004 §9.1 mandates FindingDefinition.description + defaultSeverity and ratifies neither for ANY of its 99 codes — an open corpus gap; unannotated codes fall back to the policy\'s failureSeverity and the humanized code). EVIDENCE (REG-E-026, authored 2026-08-05 under sponsor ratification): the 8 items of DOC-004 §23\'s "Evidence" subsection are TRANSCRIBED verbatim (leading character sentence-cased, the same documented normalization the criteria use) into `optionalEvidence`. THE FIELD IS CHOSEN BY THE CORPUS\'S OWN HEADING — §3.1 declares both requiredEvidence and optionalEvidence, two sections say "Required evidence" and nine say "Evidence"; calling all eleven required would add a word to nine sections that do not use it. DERIVED: id = EV-<section>-<NN> in document order; cardinality = ZERO_OR_MORE where the item is hedged ("where applicable"/"where relevant") OR where its referent is CONTINGENT — it exists only if an optional exchange actually occurred, and no ratified transition guard or mandatory schema field makes its record unavoidable (four items, each named with its reason in doc004-conformance.test.ts) — else AT_LEAST_ONE. THE CONTINGENT CLAUSE IS AUTHORED JUDGEMENT AND THE MOST CONSEQUENTIAL CALL HERE: under a blanket AT_LEAST_ONE, an intent captured from one clear user expression could never reach a SATISFIED fidelity verdict for want of a clarification dialogue that never happened. ⚠ ITS FIRST DRAFT CARRIED A SECOND ARM — "or the system has no object type that can produce it" — WHICH WAS INVALID AND IS WITHDRAWN (adversarial review, 2026-08-05): an Evidence object needs no same-named object type (contentReference is an open record), so the arm would equally have voided EV-15-01, EV-16-05 and EV-16-06, which do gate — it emptied the gate instead of discriminating within it. DOC-002 §6.2\'s intent transition matrix is the discriminator that survives: it makes "known ambiguities recorded" a GUARD on the only path to the assessed state (so EV-16-02 gates) and names stakeholders nowhere (so EV-16-04 does not); requiredForDispositions = SATISFIED_ONLY on §15.9 ("SATISFIED only when ... required evidence is admissible" — named as a condition of the verdict, not of assessing); mayBeWaived = false, fail-closed, because the corpus nowhere states that a requirement may be waived. HONESTLY EMPTY: admissibilityRules = [] (§6.2 states admissibility globally, not per requirement); freshnessRule omitted (optional, and no policy states one). AUTHORED, and the only genuinely new content: each item\'s evidenceType classification and its purpose sentence. SUPERSEDES requiredEvidenceTypes, removed here — a 2-3 member authored summary that nothing read, now stated at 89x the resolution and machine-checked against the document by doc004-conformance.test.ts. SCOPE WIDENED 2026-08-06 (REG-F-033 deepening): the kinds below were added because their pwuTemplate rows already DECLARE this policy in defaultPolicyIds while this list did not name them. In every case the PARENT kind was listed and the CHILD was not — this list was authored against the NINE PUBLISHED PWU Types, so the six unpublished templates were outside its field of view, and deepening the tree is what surfaced it. WIDENED RATHER THAN NARROWED deliberately: dropping the policy from the template would silently exempt work from assurance, which is the direction REG-F-024 names as forbidden. Both lists are AUTHORED and state one fact twice; a bidirectional gate now holds them together. Added here: USER_JOURNEY_DEFINITION, REQUIREMENT_DEFINITION.',
			findingAnnotations: {
				INTENT_EROSION: {
					defaultSeverity: 'MATERIAL',
					description:
			...
			optionalEvidence: [
				{
					...
					description: 'Originating expression',
					purpose:
						'Preservation is fidelity across a transformation, and the origin is one end of it.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					evidenceType: 'ARTIFACT',
					description: 'Approved intent',
					purpose: 'The authorized intent the transformation must remain faithful to.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-23-03',
					evidenceType: 'ARTIFACT',
					description: 'Parent and child PWUs',
					purpose: 'Claim 4 asks whether descendant work continues to serve the parent objective.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Constraints',
					purpose:
						'Claim 3 requires mandatory constraints remain active across the transformation.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-23-05',
					evidenceType: 'ARTIFACT',
					description: 'Transformation artifacts',
					purpose: 'The output whose faithfulness is the question.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Decisions',
					purpose:
						'Claim 2 turns on authority: a scope change with a decision is authorized, and the same change without one is drift.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 1 asks whether desired outcomes remain represented, which is a relation across the transformation rather than a property of either side.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-23-08',
					evidenceType: 'ARTIFACT',
					description: 'Completion claims',
					purpose:
						'Claim 6 asks whether local success contributes to global success; the completion claim is what local success asserts.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
			...
			optionalEvidence: [
				{
					...
					purpose:
						'Claim 5 asks whether regression scope is justified, which is a statement of strategy rather than a result.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose: 'The tests that exist — the covering set for every adequacy claim.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'UNTESTED_REQUIREMENT is a relation between requirements and tests, decidable only over the matrix.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose: 'Claim 7 requires results be current and reproducible where required.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 6 asks whether test environments are sufficiently representative; NONREPRESENTATIVE_TEST_ENVIRONMENT is decided here.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Coverage is a quantity, and §24.6 warns it does not establish fitness — so it is measured and weighed, never concluded from.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-24-07',
					evidenceType: 'OBSERVATION',
					description: 'Defect history',
					purpose:
						'FLAKY_TEST_EVIDENCE is a property of a test over time and is invisible in any single run.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Risk profile',
					purpose:
						'Adequacy is proportional to risk, so the profile sets the standard the other claims are judged against.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
			...
			optionalEvidence: [
				{
					...
					description: 'Approved intent',
					purpose:
						'Claim 3 asks whether the product addresses the originating problem; the intent is what states what that problem was.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					evidenceType: 'ARTIFACT',
					description: 'Journeys',
					purpose: 'Claim 2 asks whether major journeys function coherently.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Requirements',
					purpose:
						'The declared obligations, against which claim 4’s technically-correct-but-unfit case is distinguished.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-25-04',
					evidenceType: 'ARTIFACT',
					description: 'Working product',
					purpose:
						'The product itself; fitness is a property of what was built, not of what was specified.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 1 asks whether intended actors can achieve critical outcomes, which is observable only end to end.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-25-06',
					evidenceType: 'REVIEW',
					description: 'User or human review',
					purpose:
						'Fitness for an actual need is a human judgement; §25.1 makes the approved user need the referent.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose: 'Claim 7 requires critical operational conditions be represented.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose: 'Claim 5 asks whether known limitations are compatible with the declared use.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 6 asks whether residual uncertainty is acceptable, and unresolved findings are that uncertainty.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
			...
			optionalEvidence: [
				{
					...
					purpose: 'What is being promoted; AMBIGUOUS_BASELINE_SCOPE is decided against it.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'The identity of the exact bytes promoted; CANDIDATE_VERSION_MISMATCH is decidable only against them.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-26-03',
					evidenceType: 'REVIEW',
					description: 'Assurance assessments',
					purpose:
						'MISSING_REQUIRED_ASSESSMENT asks which policies judged this candidate; the assessments are that record.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Open findings',
					purpose:
						'§26.6 forbids conditional promotion over unresolved critical findings; OPEN_BLOCKING_FINDING is decided here.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Waivers',
					purpose:
						'INVALID_OR_EXPIRED_WAIVER requires the waiver itself, carrying its authority and expiry (§12.2).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					description: 'Decisions',
					purpose:
						'Promotion requires effective decision authority; MISSING_DECISION_AUTHORITY is decided against the decision record.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 5 requires evidence remain valid; INVALIDATED_EVIDENCE is decided over the package.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					...
					purpose:
						'Claim 8 requires rollback or recovery exist where required; MISSING_RECOVERY_PLAN is decided here.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
				},
				{
					id: 'EV-26-09',
					evidenceType: 'ARTIFACT',
					description: 'Superseded baseline',
					purpose:
						'What this promotion replaces; without it the promotion’s scope and its reversal target are both undefined.',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
