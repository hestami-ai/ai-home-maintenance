	...
	seedPolicies: [
		{
			...
			sourceSection:
				'DOC-004 §15 (POL-INTENT-FIDELITY). TRANSCRIBED from the ratified doc and machine-checked against it by doc004-conformance.test.ts: purpose (§15.2); the 5 criterion descriptions are "15.6 Criteria" verbatim (leading character sentence-cased — the doc\'s numbered lists are inconsistently cased; no word added, removed, or reordered); the 7 finding codes are §15.7 verbatim in document order. Criterion ids AND names are ratified by "15.6 Criteria" ("### ID Name" headings). permittedControlActions transcribed from §15.10. AUTHORED (no ratified source): rationale, evaluatorRole, evaluatedClaimTypes, appliesToPwuKinds, failureSeverity, independenceRequirement where the section has no Independence subsection, each criterion\'s criterionType/evaluationMethod/severityIfNotMet/mayBeNotApplicable, and findingAnnotations (DOC-004 §9.1 mandates FindingDefinition.description + defaultSeverity and ratifies neither for ANY of its 99 codes — an open corpus gap; unannotated codes fall back to the policy\'s failureSeverity and the humanized code). EVIDENCE (REG-E-026, authored 2026-08-05 under sponsor ratification): the 7 items of DOC-004 §15\'s "Required evidence" subsection are TRANSCRIBED verbatim (leading character sentence-cased, the same documented normalization the criteria use) into `requiredEvidence`. THE FIELD IS CHOSEN BY THE CORPUS\'S OWN HEADING — §3.1 declares both requiredEvidence and optionalEvidence, two sections say "Required evidence" and nine say "Evidence"; calling all eleven required would add a word to nine sections that do not use it. DERIVED: id = EV-<section>-<NN> in document order; cardinality = ZERO_OR_MORE where the item is hedged ("where applicable"/"where relevant") OR where its referent is CONTINGENT — exists only if an optional exchange happened, or the system has no object type that can produce it (five items, each named with its reason in doc004-conformance.test.ts) — else AT_LEAST_ONE. THE CONTINGENT CLAUSE IS AUTHORED JUDGEMENT AND THE MOST CONSEQUENTIAL CALL HERE: under a blanket AT_LEAST_ONE, an intent captured from one clear user expression could never reach a SATISFIED fidelity verdict for want of a clarification dialogue that never happened, and POL-INTENT-COMPLETENESS could never be satisfied by ANY undertaking, because no AMBIGUITY or STAKEHOLDER object type exists in this system at all. A requirement whose referent cannot be produced is not a strict gate, it is a dead policy; requiredForDispositions = SATISFIED_ONLY on §15.9 ("SATISFIED only when ... required evidence is admissible" — named as a condition of the verdict, not of assessing); mayBeWaived = false, fail-closed, because the corpus nowhere states that a requirement may be waived. HONESTLY EMPTY: admissibilityRules = [] (§6.2 states admissibility globally, not per requirement); freshnessRule omitted (optional, and no policy states one). AUTHORED, and the only genuinely new content: each item\'s evidenceType classification and its purpose sentence. SUPERSEDES requiredEvidenceTypes, removed here — a 2-3 member authored summary that nothing read, now stated at 89x the resolution and machine-checked against the document by doc004-conformance.test.ts. EVIDENCE (REG-E-026, authored 2026-08-05 under sponsor ratification): the 7 items of DOC-004 §15\'s "Required evidence" subsection are TRANSCRIBED verbatim (leading character sentence-cased, the same documented normalization the criteria use) into `requiredEvidence`. THE FIELD IS CHOSEN BY THE CORPUS\'S OWN HEADING — §3.1 declares both requiredEvidence and optionalEvidence, two sections say "Required evidence" and nine say "Evidence"; calling all eleven required would add a word to nine sections that do not use it. DERIVED: id = EV-<section>-<NN> in document order; cardinality = ZERO_OR_MORE where the item is hedged ("where applicable"/"where relevant") else AT_LEAST_ONE; requiredForDispositions = SATISFIED_ONLY on §15.9 ("SATISFIED only when ... required evidence is admissible" — named as a condition of the verdict, not of assessing); mayBeWaived = false, fail-closed, because the corpus nowhere states that a requirement may be waived. HONESTLY EMPTY: admissibilityRules = [] (§6.2 states admissibility globally, not per requirement); freshnessRule omitted (optional, and no policy states one). AUTHORED, and the only genuinely new content: each item\'s evidenceType classification and its purpose sentence. SUPERSEDES requiredEvidenceTypes, removed here — a 2-3 member authored summary that nothing read, now stated at 89x the resolution and machine-checked against the document by doc004-conformance.test.ts.',
			findingAnnotations: {
				SOLUTION_SUBSTITUTION: {
			...
			requiredEvidence: [
				...
				},
				{
					...
					purpose:
						'Where the intent is a revision, its predecessor is what makes an unauthorized change of scope or constraint visible (IF-02).',
					cardinality: 'ZERO_OR_MORE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
		...
		},
		{
			...
			optionalEvidence: [
				...
				},
				{
					...
					description: 'Constraints',
					purpose:
						'A premise that contradicts an active constraint is an assumption silently authorizing prohibited work (claim 6).',
					cardinality: 'AT_LEAST_ONE',
					admissibilityRules: [],
					requiredForDispositions: 'SATISFIED_ONLY',
					mayBeWaived: false
