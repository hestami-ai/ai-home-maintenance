...
function fixture(): string {
	...
	json(root, 'package.json', { name: 'subject-fixture', private: true, workspaces: ['packages/*'] });
	...
	write(root, 'packages/demo/tsconfig.json', '{ // JSONC\n "compilerOptions": { "strict": true },\n "include": ["src"]\n}\n');
	json(root, 'tsconfig.json', { files: [], include: [] });
	write(root, 'bun.lock', 'fixture lock\n');
	return root;
}

function request(root: string, overrides: Partial<ResolveSubjectRequest> = {}): ResolveSubjectRequest {
	return {
		budgets: { maxBytes: 32 * 1024 * 1024, maxConfigDepth: 32, maxDiagnostics: 100, maxDurationMs: 30_000, maxFiles: 10_000, maxProjects: 100 },
...
function workspaceCapture(files: Readonly<Record<string, string>>, directoryPaths: readonly string[] = []): SubjectCapture {
	return {
		artifacts: [],
		bytesByPath: new Map(Object.entries(files).map(([path, contents]) => [path, new TextEncoder().encode(contents)])),
		diagnostics: [],
		directoryPaths,
...
describe('repository path and artifact policy', () => {
	it('keeps display case, applies injected case rules, and rejects dot/backslash/collisions', () => {
		...
		expect(() => assertNoCanonicalPathCollisions(['Src/A.ts', 'src/a.ts'], false)).toThrow('collision');
	...
	it('fails closed across repository-root, physical-path, glob, and sensitive-state boundaries', () => {
		...
		expect(() => assertSafeExistingPath(root, 'physical-escape')).toThrow('escapes repository root');
		expect(() => assertNoCanonicalPathCollisions(['src/index.ts', 'src/index.ts'], true)).toThrow('Duplicate repository path');
		...
		expect(isRequestedPath('packages/demo/src/index.ts', request(root, {
			filters: { exclude: ['**/index.ts'], include: ['**/*.ts'] }
		}))).toBe(false);
		expect(sensitiveOrLocalStateExclusion('packages/demo/vite.config.ts.timestamp-123')).toMatchObject({ policyId: 'jan-csaa-exclude-local-state/1' });
		expect(sensitiveOrLocalStateExclusion('packages/demo/.env.local')).toMatchObject({ reason: 'Environment/secrets files are never ingested.' });
		expect(sensitiveOrLocalStateExclusion('packages/demo/client-credentials.json')).toMatchObject({ reason: 'Credential or private-key material is never ingested.' });
		expect(sensitiveOrLocalStateExclusion('apps/rph-demo/harness/session.json')).toMatchObject({ primaryClass: 'CACHE' });
	});

	it('classifies tests, generated outputs, framework candidates, and authored tmp files truthfully', () => {
		expect(classifyArtifact('apps/demo/e2e/journey.ts')).toMatchObject({ primaryClass: 'TEST_SOURCE', roles: expect.arrayContaining(['TEST']) });
		expect(classifyArtifact('packages/demo/src/gen/gen-schema.ts')).toMatchObject({ primaryClass: 'GENERATOR_SOURCE', roles: expect.arrayContaining(['GENERATOR']) });
		expect(classifyArtifact('packages/demo/src/schema.ts', '// GENERATED FILE — do not edit.\n')).toMatchObject({ primaryClass: 'GENERATED_SOURCE', roles: expect.arrayContaining(['GENERATED']) });
		expect(classifyArtifact('apps/demo/src/Page.svelte')).toMatchObject({ disposition: 'INVENTORY_ONLY', roles: expect.arrayContaining(['FRAMEWORK_CANDIDATE']) });
		expect(classifyArtifact('vitest.config.ts')).toMatchObject({ primaryClass: 'TOOL_CONFIGURATION', roles: expect.arrayContaining(['COMPILER_CANDIDATE', 'CONFIGURATION']) });
		expect(classifyArtifact('packages/demo/src/foo.tmp')).toMatchObject({ disposition: 'INVENTORY_ONLY', primaryClass: 'OTHER' });
	});
	...
	it('maps unsupported envelopes, invalid roots, root-manifest failures, and exclusion-budget exhaustion exactly', () => {
		...
		const unsupported = {
			...request(root),
			schemaVersion: 'jan-csaa-subject-request/unsupported' as ResolveSubjectRequest['schemaVersion']
		};
		expect(resolveSubject(unsupported)).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'UNSUPPORTED_REQUEST_VERSION', phase: 'CAPTURE' })],
		...
		expect(resolveSubject(request(missingManifest))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'CONFIG_REQUIRED_MISSING', path: 'package.json' })],
			outcome: 'not-found'
		});
		...
		const exclusionBudget = resolveSubject(request(root, {
		...
		expect(exclusionBudget).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED', message: 'Subject capture exceeded its file budget.' })],
			outcome: 'unavailable'
		});
	});
});
...
describe('workspace manifest discovery', () => {
	it('accepts object-form workspace packages and flattens nested export arrays without losing conditions', () => {
		const root = fixture();
		json(root, 'package.json', { name: 'subject-fixture', private: true, workspaces: { packages: ['packages/*'] } });
		json(root, 'packages/demo/package.json', {
			exports: {
				'.': [{ import: './dist/index.js', types: './dist/index.d.ts' }, './dist/fallback.js', null],
		...
		const workspace = resolved(root).subject.workspaces.find((candidate) => candidate.name === '@fixture/demo')!;
		expect(workspace).toMatchObject({ path: 'packages/demo', private: true, workspacePatterns: ['packages/*'] });
		expect(workspace.exports).toHaveLength(6);
		expect(workspace.exports).toEqual(expect.arrayContaining([
	...
	it('fails each malformed, unsupported, missing, and ambiguous workspace-manifest form at its exact boundary', () => {
		...
		const cases: readonly {
			...
			{
				capture: workspaceCapture({ 'package.json': encoded({ workspaces: { packages: ['packages/*', 1] } }) }),
				message: 'Root workspace package patterns must all be strings.',
				outcome: 'incompatible'
			},
			{
				capture: workspaceCapture({ 'package.json': encoded({ workspaces: { packages: ['packages/**'] } }) }),
				message: 'Unsupported workspace pattern',
				outcome: 'incompatible'
			},
			{
			...
			{
				capture: workspaceCapture({ 'package.json': encoded({ workspaces: ['packages/*'] }) }, ['packages/demo']),
...
describe('captured TypeScript project resolution', () => {
	it('uses JSONC, extends arrays/package specifiers, allowJs, include/exclude, and captured closure bytes', () => {
		const root = fixture();
		json(root, 'packages/config/package.json', { name: '@fixture/config', private: true, exports: { './base.json': './base.json' } });
		...
		const project = outcome.subject.projects.find((item) => item.configPath === 'packages/demo/tsconfig.json')!;
		expect(project.rawExtends).toEqual(['@fixture/config/base.json', './other.json']);
		expect(project.rawCompilerOptions).toEqual({});
		expect(project.effectiveCompilerOptions).toMatchObject({ allowJs: true, checkJs: true, strict: true });
		expect(project.fileNames).toEqual(['packages/demo/src/accepted.js', 'packages/demo/src/index.ts']);
		expect(project.configClosure.map((item) => item.path)).toEqual(expect.arrayContaining(['packages/config/base.json', 'packages/config/package.json', 'packages/demo/other.json', 'packages/demo/tsconfig.json']));
		...
		expect(baseArtifact).toMatchObject({ disposition: 'ANALYZED', primaryClass: 'PROJECT_CONFIGURATION', roles: expect.arrayContaining(['CONFIGURATION']) });
		expect(outcome.subject.workspaces.find((workspace) => workspace.name === '@fixture/config')?.exports).toEqual([{ conditions: [], exportName: './base.json', target: './base.json' }]);
	});

	it('resolves bare workspace packages, extensionless relatives, and exact JSON extends into one frozen closure', () => {
		const root = fixture();
		json(root, 'packages/config/package.json', { name: '@fixture/config', private: true, tsconfig: './base.json' });
		...
		const project = resolved(root).subject.projects.find((candidate) => candidate.configPath === 'packages/demo/tsconfig.json')!;
		expect(project.rawExtends).toEqual(['@fixture/config', './local', './exact.json']);
		expect(project.effectiveCompilerOptions).toMatchObject({ allowJs: true, checkJs: true, noImplicitAny: true });
		expect(project.configClosure.map((entry) => entry.path)).toEqual(expect.arrayContaining([
	...
	it('preserves absent versus explicit-empty raw roots and rejects malformed raw declarations', () => {
		...
		expect(resolveSubject(request(root))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'CONFIG_MALFORMED' })] });
	});

	it('keeps root, build, and solution projects distinct and does not union references into roots', () => {
		const root = fixture();
		json(root, 'packages/demo/tsconfig.build.json', { extends: './tsconfig.json', compilerOptions: { declaration: true }, include: ['src/index.ts'] });
		json(root, 'tsconfig.json', { files: [], include: [], references: [{ path: './packages/demo' }] });
		const outcome = resolved(root);
		const solution = outcome.subject.projects.find((item) => item.configPath === 'tsconfig.json')!;
		const build = outcome.subject.projects.find((item) => item.configPath === 'packages/demo/tsconfig.build.json')!;
		expect(solution).toMatchObject({ kind: 'SOLUTION', rootDisposition: 'INTENTIONAL_EMPTY_SOLUTION', fileNames: [], status: 'COMPLETE' });
	...
	it('follows a diamond reference graph as separate projects', () => {
		...
		json(root, 'tsconfig.json', { files: [], references: [{ path: 'packages/a' }, { path: 'packages/b' }] });
		const outcome = resolved(root);
		expect(outcome.subject.projects.filter((item) => ['packages/a/tsconfig.json', 'packages/b/tsconfig.json', 'packages/shared/tsconfig.json'].includes(item.configPath))).toHaveLength(3);
		expect(outcome.subject.projects.find((item) => item.configPath === 'packages/a/tsconfig.json')?.fileNames).toEqual([]);
	});
	...
	it('fails closed for missing/cyclic references and missing explicit compiler roots', () => {
		...
		expect(resolveSubject(request(missing))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'REFERENCE_REQUIRED_MISSING' })] });

		const cyclic = fixture();
		json(cyclic, 'packages/demo/tsconfig.json', { files: [], references: [{ path: '../../tsconfig.json' }] });
		json(cyclic, 'tsconfig.json', { files: [], references: [{ path: './packages/demo' }] });
		expect(resolveSubject(request(cyclic))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'REFERENCE_CYCLE' })] });

		const rootMissing = fixture();
		json(rootMissing, 'packages/demo/tsconfig.json', { files: ['src/absent.ts'] });
		expect(resolveSubject(request(rootMissing))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'CONFIG_REQUIRED_MISSING' })] });
		...
		expect(resolveSubject(request(extendsCycle))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'CONFIG_CLOSURE_CYCLE' })] });
		...
		expect(resolveSubject(request(deep, { budgets: { ...request(deep).budgets, maxConfigDepth: 1 } }))).toMatchObject({ outcome: 'unavailable', diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED' })] });

		const missingPackage = fixture();
		json(missingPackage, 'packages/demo/tsconfig.json', { extends: '@missing/config/base.json', include: ['src'] });
		expect(resolveSubject(request(missingPackage))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'CONFIG_REQUIRED_MISSING' })] });

		const packageDepth = fixture();
		json(packageDepth, 'packages/config/package.json', { name: '@fixture/config', private: true, tsconfig: './a.json' });
		...
		json(packageDepth, 'packages/demo/tsconfig.json', { extends: '@fixture/config', include: ['src'] });
		expect(resolveSubject(request(packageDepth, { budgets: { ...request(packageDepth).budgets, maxConfigDepth: 1 } }))).toMatchObject({ outcome: 'unavailable', diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED' })] });
	});
	...
	it('rejects malformed, escaping, and externally rooted project declarations at their owning boundary', () => {
		...
		expect(resolveSubject(request(malformedExtends))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'CONFIG_MALFORMED', path: 'packages/demo/tsconfig.json' })],
			outcome: 'incompatible'
		});

		const escapingExtends = fixture();
		json(escapingExtends, 'packages/demo/tsconfig.json', { extends: '../../../outside', include: ['src'] });
		expect(resolveSubject(request(escapingExtends))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'PATH_ESCAPE', message: 'Configuration extends target escapes the repository.' })],
			outcome: 'forbidden'
		});

		const nonCanonicalExtends = fixture();
		json(nonCanonicalExtends, 'packages/demo/tsconfig.json', { extends: '..\\base.json', include: ['src'] });
		expect(resolveSubject(request(nonCanonicalExtends))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'PATH_ESCAPE', message: expect.stringContaining('not repository-relative') })],
			outcome: 'forbidden'
		});

		const escapingReference = fixture();
		json(escapingReference, 'packages/demo/tsconfig.json', { files: [], references: [{ path: '../../../outside' }] });
		expect(resolveSubject(request(escapingReference))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'PATH_ESCAPE', message: 'Project reference escapes the repository.' })],
			outcome: 'forbidden'
		});
		...
		expect(resolveSubject(request(externalCompilerOption))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'PATH_ESCAPE', message: expect.stringContaining('external absolute path') })],
			outcome: 'forbidden'
		});

		const malformedOptions = fixture();
		json(malformedOptions, 'packages/demo/tsconfig.json', { compilerOptions: null, include: ['src'] });
		expect(resolveSubject(request(malformedOptions))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'CONFIG_MALFORMED', message: expect.stringContaining('#/compilerOptions') })],
			outcome: 'incompatible'
		});
	});
	...
	it('enforces explicit-project, project-count, reference-depth, diagnostic, and wildcard perimeters', () => {
		...
		expect(resolveSubject(request(missingExplicit, {
			scope: { kind: 'EXPLICIT_PROJECTS', projects: ['packages/demo/missing.json'] }
		}))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'PROJECT_NOT_FOUND', path: 'packages/demo/missing.json' })],
			outcome: 'not-found'
		});

		const projectCount = fixture();
		expect(resolveSubject(request(projectCount, {
			budgets: { ...request(projectCount).budgets, maxProjects: 1 }
		}))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED', message: expect.stringContaining('capturing TypeScript directory queries') })],
			outcome: 'unavailable'
		});

		const referenceDepth = fixture();
		for (const name of ['a', 'b']) {
			json(referenceDepth, `packages/${name}/package.json`, { name: `@fixture/${name}`, private: true });
		...
		expect(resolveSubject(request(referenceDepth, {
			budgets: { ...request(referenceDepth).budgets, maxConfigDepth: 1 },
			scope: { kind: 'EXPLICIT_PROJECTS', projects: ['tsconfig.json'] }
		}))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED', message: expect.stringContaining('Project/config depth') })],
			outcome: 'unavailable'
		});
		...
		expect(resolveSubject(request(diagnostics, {
			budgets: { ...request(diagnostics).budgets, maxDiagnostics: 1 }
		}))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED', message: expect.stringContaining('diagnostics exceeded') })],
			outcome: 'unavailable'
		});
		...
		expect(resolveSubject(request(wildcardTraversal))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'PATH_ESCAPE', message: expect.stringContaining('pattern traverses') })],
			outcome: 'forbidden'
		});
	});
	...
	it('retains filtered compiler roots as explicit partiality and rejects an absent exact closure member', () => {
		...
		const outcome = resolved(filtered, { filters: { exclude: [], include: ['packages/demo/src/kept.ts'] } });
		const project = outcome.subject.projects.find((candidate) => candidate.configPath === 'packages/demo/tsconfig.json')!;
		...
		expect(outcome.diagnostics).toEqual(expect.arrayContaining([
			expect.objectContaining({ code: 'TYPESCRIPT_PROJECT_PARTIAL', path: 'packages/demo/src/filtered.ts', severity: 'WARNING' }),
			expect.objectContaining({ code: 'TYPESCRIPT_PROJECT_PARTIAL', path: 'packages/demo/src/index.ts', severity: 'WARNING' })
		]));
		expect(outcome.subject.excludedArtifacts).toEqual(expect.arrayContaining([
			expect.objectContaining({ path: 'packages/demo/src/filtered.ts', policyId: expect.stringMatching(/^jan-csaa-filter\/1:/u) }),
			expect.objectContaining({ path: 'packages/demo/src/index.ts', policyId: expect.stringMatching(/^jan-csaa-filter\/1:/u) })
		]));

		const missingClosure = fixture();
		json(missingClosure, 'packages/demo/tsconfig.json', { extends: './required-base.json', include: ['src'] });
		expect(resolveSubject(request(missingClosure))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'CONFIG_REQUIRED_MISSING', path: 'packages/demo/required-base.json' })],
			outcome: 'incompatible'
		});
	});
	...
	it('fails malformed configs, permits intentional empty subjects, and rejects hostile include traversal', () => {
		...
		expect(resolveSubject(request(malformed))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'CONFIG_MALFORMED' })] });

		const empty = mkdtempSync(join(tmpdir(), 'csaa-empty-'));
		temporaryRoots.push(empty);
		json(empty, 'package.json', { name: 'empty', private: true });
		expect(resolveSubject(request(empty))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'EMPTY_SUBJECT' })] });
		expect(resolveSubject(request(empty, { expectEmpty: true }))).toMatchObject({ outcome: 'resolved', completeness: 'COMPLETE' });

		const hostile = fixture();
		json(hostile, 'packages/demo/tsconfig.json', { include: ['../../../outside/**/*.ts'] });
		expect(resolveSubject(request(hostile))).toMatchObject({ outcome: 'forbidden', diagnostics: [expect.objectContaining({ code: 'PATH_ESCAPE' })] });
	});
	...
	it('refuses missing, escaping, uncaptured, and unrecorded TypeScript directory evidence', () => {
		...
		const unrecorded = projectFailure(() => discoverProjects({
			...capture,
			typescriptDirectoryRecordings: new Map()
		}, request(root), workspaces));
		expect(unrecorded).toMatchObject({
			diagnostic: expect.objectContaining({ code: 'CONFIG_DIAGNOSTIC', message: expect.stringContaining('unrecorded directory query') }),
			outcome: 'incompatible'
		});

		const noReader = projectFailure(() => recordProjectDirectoryQueries(
			capture,
			request(root),
			workspaces,
		...
		expect(noReader).toMatchObject({
			diagnostic: expect.objectContaining({ code: 'RECONCILIATION_FAILED', message: expect.stringContaining('reader is absent') }),
			outcome: 'incompatible'
		});
		...
		const escaped = projectFailure(() => recordProjectDirectoryQueries(capture, request(root), workspaces, escapingReader));
		expect(escaped).toMatchObject({
			diagnostic: expect.objectContaining({ code: 'PATH_ESCAPE', message: expect.stringContaining('directory result escapes') }),
			outcome: 'forbidden'
		});

		const uncapturedReader = (() => [join(root, 'packages/demo/src/phantom.ts')]) as typeof ts.sys.readDirectory;
		const uncaptured = projectFailure(() => recordProjectDirectoryQueries(capture, request(root), workspaces, uncapturedReader));
		expect(uncaptured).toMatchObject({
			diagnostic: expect.objectContaining({ code: 'SUBJECT_CHANGED_DURING_RESOLUTION', path: 'packages/demo/src/phantom.ts' }),
			outcome: 'unavailable'
		});
	});
	...
	it('binds package-based extends and project discovery to exact captured evidence', () => {
		...
		const absent = projectFailure(() => discoverProjects({ ...capture, bytesByPath: absentBytes }, request(root), workspaces));
		...
		for (const [bytes, message] of [
			...
			[new TextEncoder().encode(JSON.stringify({ name: '@fixture/config', private: true, tsconfig: 42 })), '#/tsconfig must be a string']
		] as const) {
			const changedBytes = new Map(capture.bytesByPath);
			changedBytes.set(manifestPath, bytes);
			const changed = projectFailure(() => discoverProjects({ ...capture, bytesByPath: changedBytes }, request(root), workspaces));
			expect(changed.diagnostic).toMatchObject({ code: 'CONFIG_MALFORMED', path: manifestPath });
			expect(changed.message).toContain(message);
		}

		const unbound = projectFailure(() => discoverProjects({
			...capture,
			artifacts: capture.artifacts.filter((artifact) => artifact.path !== manifestPath)
		}, request(root), workspaces));
		expect(unbound).toMatchObject({
			diagnostic: expect.objectContaining({ code: 'CONFIG_REQUIRED_MISSING', message: expect.stringContaining('not content-bound'), path: manifestPath }),
			outcome: 'incompatible'
		});

		const bounded = projectFailure(() => discoverProjects(capture, request(root, {
			budgets: { ...request(root).budgets, maxProjects: 1 }
		}), workspaces));
		expect(bounded).toMatchObject({
			diagnostic: expect.objectContaining({ code: 'BUDGET_EXCEEDED', message: 'Project count exceeded the request budget.' }),
			outcome: 'unavailable'
		});
	});
});
...
describe('generated context and subject identity', () => {
	function svelteFixture(withGenerated: boolean): string {
		...
		json(root, 'apps/demo/tsconfig.json', { extends: './.svelte-kit/tsconfig.json', compilerOptions: { allowJs: true } });
		if (withGenerated) json(root, 'apps/demo/.svelte-kit/tsconfig.json', { compilerOptions: { noEmit: true }, include: ['../src/**/*.ts', '../src/**/*.svelte', './types/**/$types.d.ts'] });
		return root;
	}
	...
	it('keeps Svelte as framework-only and reports present generated context UNKNOWN/PARTIAL', () => {
		...
		const app = outcome.subject.projects.find((item) => item.configPath === 'apps/demo/tsconfig.json')!;
		...
		expect(outcome.subject.generatedContexts).toEqual([expect.objectContaining({ freshness: 'UNKNOWN', path: 'apps/demo/.svelte-kit/tsconfig.json' })]);
	});
	...
	it('degrades only an absent generated Svelte config and fails a malformed present one', () => {
		...
		if (absent.outcome === 'resolved') expect(absent.diagnostics).toEqual(expect.arrayContaining([expect.objectContaining({ code: 'GENERATED_CONTEXT_ABSENT' })]));
	...
	it('models CURRENT, STALE, and UNKNOWN freshness without changing byte identity semantics', () => {
		...
		expect(assessGeneratedContextFreshness({ generatorInputDigest: 'a', recordedInputDigest: 'a', source: 'record' })).toMatchObject({ freshness: 'CURRENT' });
		expect(assessGeneratedContextFreshness({ generatorInputDigest: 'a', recordedInputDigest: 'b', source: 'record' })).toMatchObject({ freshness: 'STALE' });
		const root = svelteFixture(true);
		const evidenceBase = { path: 'apps/demo/.svelte-kit/tsconfig.json', source: 'verif/generated-context-record.json' };
		const current = resolved(root, { generatedContextEvidence: [{ ...evidenceBase, generatorInputDigest: 'same', recordedInputDigest: 'same' }] }).subject;
		const stale = resolved(root, { generatedContextEvidence: [{ ...evidenceBase, generatorInputDigest: 'new', recordedInputDigest: 'old' }] }).subject;
	...
	it.runIf(process.platform === 'win32')('matches generated-context evidence by canonical Windows path identity', () => {
		const root = svelteFixture(true);
		const subject = resolved(root, { generatedContextEvidence: [{ generatorInputDigest: 'same', path: 'APPS/DEMO/.SVELTE-KIT/TSCONFIG.JSON', recordedInputDigest: 'same', source: 'verif/generated-context-record.json' }] }).subject;
	...
	it('changes identity for material bytes but not mtime, caches, or declared output bytes', () => {
		...
		expect(resolved(root, { outputs }).subject.descriptor.subjectId).toBe(initial.descriptor.subjectId);
		...
		expect(resolved(root, { outputs }).subject.descriptor.subjectId).not.toBe(sourceChanged.descriptor.subjectId);
	});
	...
	it('binds exact output and explicit-project scope policy and retains authored foo.tmp', () => {
		...
		const scoped = resolved(root, { scope: { kind: 'EXPLICIT_PROJECTS', projects: ['packages/demo/tsconfig.json'] } }).subject;
		expect(all.artifacts.some((item) => item.path === 'packages/demo/src/foo.tmp')).toBe(true);
		expect(scoped.artifacts.some((item) => item.path === 'packages/other/src/index.ts')).toBe(false);
		expect(scoped.descriptor.subjectId).not.toBe(all.descriptor.subjectId);
		expect(resolved(root, { outputs: ['verif/a.json'] }).subject.descriptor.subjectId).not.toBe(resolved(root, { outputs: ['verif/b.json'] }).subject.descriptor.subjectId);
	});
	...
	it('isolates explicit-project identity from unrelated source and project-config mutations', () => {
		...
		json(root, 'packages/other/tsconfig.json', { compilerOptions: { strict: true }, include: ['src'] });
		...
		json(root, 'packages/other/tsconfig.json', { compilerOptions: { strict: false }, include: ['src'] });
		json(root, 'packages/other/package.json', { name: '@fixture/other', private: false, version: '2.0.0' });
		const after = resolved(root, { scope }).subject;
		expect(after.descriptor.subjectId).toBe(before.descriptor.subjectId);
		expect(after.artifacts.some((artifact) => artifact.path === 'packages/other/tsconfig.json')).toBe(false);
		expect(after.artifacts.some((artifact) => artifact.path === 'packages/other/package.json')).toBe(false);
	...
	it('canonicalizes duplicate filters and does not count an absent output as a discovered file', () => {
		...
		const duplicate = resolved(root, { filters: { exclude: [], include: ['packages/**', 'packages/**'] } }).subject;
		...
		expect(absentOutput.excludedArtifacts.find((artifact) => artifact.path === 'verif/absent-output.json')?.physicalFileCount).toBe('UNKNOWN');
		expect(absentOutput.population.discovered).toBe(absentOutput.population.included + absentOutput.population.excluded + absentOutput.population.failed);
	});

	it.runIf(process.platform === 'win32')('treats case-equivalent output, filter, and project inputs identically on Windows', () => {
		const root = fixture();
		...
		expect(outcome.subject.projects.map((project) => project.configPath)).toEqual(['packages/demo/tsconfig.json']);
		expect(outcome.subject.artifacts.some((artifact) => artifact.path.toLowerCase() === 'verif/result.json')).toBe(false);
		expect(outcome.subject.artifacts.some((artifact) => artifact.path === 'packages/demo/src/index.ts')).toBe(true);
	});
});
...
describe('capture safety, immutability, freshness, and reconciliation', () => {
	it('redacts unexpected hook failures and enforces the shared resolution deadline between phases', () => {
		...
		const unexpected = resolveSubject(request(root), {
			afterCapture() { throw new Error(`unexpected failure at ${root}`); }
		});
		expect(unexpected).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'READ_FAILED', message: expect.stringContaining('unexpected failure at <runtime>'), phase: 'RESOLVE' })],
			outcome: 'unavailable'
		});
		...
		try {
			const expired = resolveSubject(request(root, {
				budgets: { ...request(root).budgets, maxDurationMs: 10 }
			}), {
				afterCapture() { now += 11; }
			});
			expect(expired).toMatchObject({
				diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED', message: 'Subject resolution exceeded its duration budget.', phase: 'RESOLVE' })],
				outcome: 'unavailable'
			});
		} finally {
			clock.mockRestore();
	...
	it('confines configured, nested, and top-level repository links to physical in-root targets', () => {
		...
		json(configuredRoot, 'package.json', { name: 'configured-link', private: true, workspaces: ['code/*'] });
		symlinkSync(outside, join(configuredRoot, 'code'), process.platform === 'win32' ? 'junction' : 'dir');
		expect(resolveSubject(request(configuredRoot))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'SYMLINK_ESCAPE', message: expect.stringContaining('Configured subject root escapes') })],
			outcome: 'forbidden'
		});

		const nestedEscape = fixture();
		symlinkSync(outside, join(nestedEscape, 'packages/demo/escape'), process.platform === 'win32' ? 'junction' : 'dir');
		expect(resolveSubject(request(nestedEscape))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'SYMLINK_ESCAPE', message: expect.stringContaining('Repository link escapes root') })],
			outcome: 'forbidden'
		});

		const dangling = fixture();
		symlinkSync(join(dangling, 'absent-target'), join(dangling, 'packages/demo/dangling'), process.platform === 'win32' ? 'junction' : 'dir');
		expect(resolveSubject(request(dangling))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'READ_FAILED', message: expect.stringContaining('Cannot resolve repository link') })],
			outcome: 'unavailable'
		});

		const topLevelEscape = fixture();
		symlinkSync(outside, join(topLevelEscape, 'archive-link'), process.platform === 'win32' ? 'junction' : 'dir');
		expect(resolveSubject(request(topLevelEscape))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'SYMLINK_ESCAPE', message: expect.stringContaining('Top-level repository link escapes root') })],
			outcome: 'forbidden'
		});

		const topLevelAlias = fixture();
		write(topLevelAlias, 'node_modules/contained/tool.ts', 'export const tool = true;\n');
		symlinkSync(join(topLevelAlias, 'node_modules/contained'), join(topLevelAlias, 'perimeter-link'), process.platform === 'win32' ? 'junction' : 'dir');
		const aliasOutcome = resolved(topLevelAlias);
		expect(aliasOutcome.subject.excludedArtifacts).toEqual(expect.arrayContaining([
			expect.objectContaining({ path: 'perimeter-link', physicalFileCount: 'UNKNOWN', policyId: 'jan-csaa-exclude-perimeter/1' })
		]));
	});

	it('does not ingest directory or dangling-link masquerades as selected root files', () => {
		const directoryMasquerade = fixture();
		symlinkSync(join(directoryMasquerade, 'packages/demo/src'), join(directoryMasquerade, '.eslintignore-extra'), process.platform === 'win32' ? 'junction' : 'dir');
		const outcome = resolved(directoryMasquerade);
		expect(outcome.subject.artifacts.some((artifact) => artifact.path === '.eslintignore-extra')).toBe(false);

		const danglingMasquerade = fixture();
		symlinkSync(join(danglingMasquerade, 'absent-target'), join(danglingMasquerade, '.eslintignore-extra'), process.platform === 'win32' ? 'junction' : 'dir');
		expect(resolveSubject(request(danglingMasquerade))).toMatchObject({
			diagnostics: [expect.objectContaining({ code: 'SYMLINK_ESCAPE', message: expect.stringContaining('Cannot safely resolve') })],
			outcome: 'forbidden'
		});
	});
	...
	it('retries one concurrent mutation and fails when every attempt changes', () => {
		...
		const retried = resolveSubject(request(root), { afterCapture(attempt) { if (attempt === 1) writeFileSync(source, 'export const value = 2;\n'); } });
		expect(retried).toMatchObject({ outcome: 'resolved' });
		let mutation = 2;
		const failed = resolveSubject(request(root), { afterCapture() { mutation += 1; writeFileSync(source, `export const value = ${mutation};\n`); } });
		expect(failed).toMatchObject({ outcome: 'unavailable', diagnostics: [expect.objectContaining({ code: 'SUBJECT_CHANGED_DURING_RESOLUTION' })] });
	});

	it('enforces one operation-wide time budget', () => {
		const root = fixture();
		const outcome = resolveSubject(request(root, { budgets: { ...request(root).budgets, maxDurationMs: 1 } }), { afterCapture() { const until = Date.now() + 5; while (Date.now() < until) { /* bounded test delay */ } } });
		expect(outcome).toMatchObject({ outcome: 'unavailable', diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED' })] });
		expect(resolveSubject(request(root, { budgets: { ...request(root).budgets, maxBytes: 1 } }))).toMatchObject({ outcome: 'unavailable', diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED' })] });
		expect(resolveSubject(request(root, { budgets: { ...request(root).budgets, maxFiles: 2 } }))).toMatchObject({ outcome: 'unavailable', diagnostics: [expect.objectContaining({ code: 'BUDGET_EXCEEDED' })] });
	});
	...
	it('follows an in-root directory link through the selected workspace without widening the top-level perimeter', () => {
		...
		symlinkSync(join(root, 'vault/safe'), linkedDirectory, process.platform === 'win32' ? 'junction' : 'dir');
		json(root, 'packages/demo/tsconfig.json', { include: ['src', 'linked'] });

		const outcome = resolved(root);
		const project = outcome.subject.projects.find((candidate) => candidate.configPath === 'packages/demo/tsconfig.json')!;
		expect(project.fileNames).toContain('packages/demo/linked/index.ts');
		expect(outcomeArtifact(outcome.subject, 'packages/demo/linked/index.ts')).toMatchObject({ disposition: 'ANALYZED', primaryClass: 'PRODUCTION_SOURCE' });
		expect(outcome.subject.excludedArtifacts).toEqual(expect.arrayContaining([
			expect.objectContaining({ path: 'vault', physicalFileCount: 'UNKNOWN', policyId: 'jan-csaa-exclude-perimeter/1' })
		]));
	});
	...
	it('returns copied bytes, detects post-freeze byte and membership changes, and reconciles exact counts', () => {
		...
		expect(readFrozenSubjectArtifact(subject, 'package.json')).toEqual(new Uint8Array(readFileSync(join(root, 'package.json'))));
		expect(subject.population.discovered).toBe(subject.population.included + subject.population.excluded + subject.population.failed);
		expect(subject.population.included).toBe(subject.population.analyzed + subject.population.inventoryOnly);
	...
	it('rejects an output beneath an escaping symlink and duplicate in-root aliases', () => {
		...
		expect(resolveSubject(request(root, { outputs: ['packages/demo/out/result.json'] }))).toMatchObject({ outcome: 'forbidden', diagnostics: [expect.objectContaining({ code: 'SYMLINK_ESCAPE' })] });

		rmSync(outputParent, { recursive: true, force: true });
		const alias = join(root, 'packages/demo/alias');
		symlinkSync(join(root, 'packages/demo/src'), alias, process.platform === 'win32' ? 'junction' : 'dir');
		expect(resolveSubject(request(root))).toMatchObject({ outcome: 'incompatible', diagnostics: [expect.objectContaining({ code: 'CANONICAL_PATH_COLLISION' })] });
	});

	it.runIf(process.platform !== 'win32' && process.getuid?.() !== 0)('fails an unreadable selected file with a typed diagnostic', () => {
		const root = fixture();
		const source = join(root, 'packages/demo/src/index.ts');
		chmodSync(source, 0o000);
		try {
			expect(resolveSubject(request(root))).toMatchObject({ outcome: 'unavailable', diagnostics: [expect.objectContaining({ code: 'READ_FAILED' })] });
...
describe('live JPWB and inventory projection', () => {
	it('matches the confirmed public TypeScript root populations', () => {
		...
		const counts = new Map(subject.projects.map((project) => [project.configPath, project.fileNames.length]));
		...
		expect(counts.get('scripts/tsconfig.json')).toBe(6);
		expect(counts.get('verif/tsconfig.json')).toBe(40);
		expect(counts.get('apps/rph-demo/tsconfig.json')).toBe(82);
		expect(subject.projects.find((project) => project.configPath === 'apps/rph-demo/tsconfig.json')?.frameworkCandidates).toHaveLength(11);
	}, 30_000);

	it('projects the same subject paths, classes, hashes, roots, and identity into JAN-CSAA-005', () => {
		const subject = projectSubjectForInventory(REPOSITORY_ROOT);
		const inventory = collectInventory({ repositoryRoot: REPOSITORY_ROOT, requireJpwbPopulations: true });
		expect(inventory.subject.subjectId).toBe(subject.descriptor.subjectId);
		expect(inventory.subject.selectedFiles.map(({ bytes, path, sha256 }) => ({ bytes, path, sha256 }))).toEqual(subject.artifacts.map(({ bytes, path, sha256 }) => ({ bytes, path, sha256 })));
		expect(inventory.typescriptProjects.map((project) => [project.path, project.resolvedRootFiles])).toEqual(subject.projects.map((project) => [project.configPath, project.fileNames]));
		expect(inventory.typescriptProjects.map((project) => [project.path, project.exclude, project.files, project.include])).toEqual(subject.projects.map((project) => [project.configPath, project.rawExclude, project.rawFiles, project.rawInclude]));
		expect(inventory.typescriptProjects).toHaveLength(subject.projects.length);
		for (const project of subject.projects) {
			const projected = inventory.typescriptProjects.find((item) => item.path === project.configPath);
			...
			if (project.status === 'PARTIAL') expect(projected?.partialityReasons.length).toBeGreaterThan(0);
		}
		expect(inventory.typescriptProjects.every((project) => project.resolvedRootState === 'RESOLVED_DWP002')).toBe(true);
		const filePreimage = inventory.subject.selectedFiles.map(({ bytes, path, sha256: digest, subjectArtifactClass }) => ({ artifactClass: subjectArtifactClass, bytes, path, sha256: digest }));
		expect(sha256(canonicalJson(filePreimage))).toBe(inventory.subject.fileManifestDigest);
		expect(sha256(canonicalJson(inventory.subject.configurationPreimage))).toBe(inventory.subject.configurationDigest);
		...
		expect(sha256(`JAN-CSAA-SUBJECT\0${'1\0'}${canonicalJson(identityPreimage)}`)).toBe(inventory.subject.subjectId);
		expect(inventory.subject.resolutionCompleteness).toBe('PARTIAL');
		expect(inventory.subject.generatedContexts).toEqual(expect.arrayContaining([expect.objectContaining({ freshness: 'UNKNOWN' })]));
		const app = inventory.typescriptProjects.find((project) => project.path === 'apps/rph-demo/tsconfig.json');
		expect(app).toMatchObject({
			generatedContexts: [expect.objectContaining({ freshness: 'UNKNOWN' })],
			rootDisposition: 'COMPILER_ROOTS',
			status: 'PARTIAL'
		});
		expect(app?.partialityReasons.map((reason) => reason.code)).toEqual(expect.arrayContaining(['FRAMEWORK_CANDIDATES_PRESENT', 'GENERATED_CONTEXT_FRESHNESS_UNKNOWN']));
