...
import { SUBJECT_EXCLUSION_POLICY_IDS, subjectFilterPolicyId, subjectOutputPolicyId } from './policy.js';
...
export function subjectConfigurationPreimage(
	...
	const configurationArtifacts = artifacts.filter((artifact) => ['MANIFEST', 'LOCKFILE', 'TOOL_CONFIGURATION', 'PROJECT_CONFIGURATION', 'GENERATED_CONFIGURATION'].includes(artifact.primaryClass));
	return {
		artifacts: configurationArtifacts.map((artifact) => ({ artifactClass: artifact.primaryClass, bytes: artifact.bytes, path: artifact.path, sha256: artifact.sha256 })),
		generatedContexts: generatedContexts.map(({ consumerProject, path, selectedInput, sha256: digest }) => ({ consumerProject, path, selectedInput, sha256: digest })),
...
function applyExplicitScope(
	...
	const scopedWorkspaces = workspaces.filter((workspace) => [...required].some((path) => path === workspace.path || path.startsWith(`${workspace.path}/`)));
	...
	for (const artifact of capture.artifacts) {
		...
		if (artifact.primaryClass === 'TOOL_CONFIGURATION' && (!artifact.path.includes('/') || scopedWorkspaces.some((workspace) => artifact.path.startsWith(`${workspace.path}/`)))) required.add(artifact.path);
	}
	...
	return {
		capture: {
			...
			excludedArtifacts: [...capture.excludedArtifacts, ...exclusions].sort((left, right) => left.path < right.path ? -1 : 1),
...
export function buildFrozenSubject(inputs: ManifestInputs): FrozenSubject {
	const scoped = applyExplicitScope(inputs.capture, inputs.request, inputs.projects, inputs.workspaces);
	const capture = scoped.capture;
	const artifacts = [...capture.artifacts].sort((left, right) => left.path < right.path ? -1 : 1);
	const excludedArtifacts = [...capture.excludedArtifacts].sort((left, right) => left.path < right.path ? -1 : 1);
	const fileManifestDigest = sha256(canonicalJson(artifacts.map((artifact) => ({ artifactClass: artifact.primaryClass, bytes: artifact.bytes, path: artifact.path, sha256: artifact.sha256 }))));
	const configurationDigest = sha256(canonicalJson(subjectConfigurationPreimage(artifacts, inputs.generatedContexts, inputs.projects, scoped.workspaces)));
	const exclusionPolicyIds = [...new Set([
		...SUBJECT_EXCLUSION_POLICY_IDS.filter((identity) => identity !== 'jan-csaa-exclude-output/1'),
		subjectOutputPolicyId(inputs.request.outputs),
		subjectFilterPolicyId(inputs.request)
	])].sort();
	const perimeter = inputs.request.scope.kind === 'REPOSITORY'
		? [...new Set(artifacts.map((artifact) => artifact.path.includes('/') ? artifact.path.split('/')[0]! : artifact.path))].sort()
	...
	const subjectId = sha256(`JAN-CSAA-SUBJECT\0${SUBJECT_ID_ALGORITHM_VERSION}\0${canonicalJson(identityPreimage)}`);
	const classCounts = new Map<CapturedArtifactRecord['primaryClass'], { physicalFileCount: number | 'UNKNOWN'; recordCount: number }>();
	for (const artifact of excludedArtifacts) {
		const current = classCounts.get(artifact.primaryClass) ?? { physicalFileCount: 0, recordCount: 0 };
		classCounts.set(artifact.primaryClass, {
			physicalFileCount: current.physicalFileCount === 'UNKNOWN' || artifact.physicalFileCount === 'UNKNOWN' ? 'UNKNOWN' : current.physicalFileCount + artifact.physicalFileCount,
	...
	const descriptor: SubjectDescriptor = {
		configurationDigest,
		dirtyState: 'UNKNOWN',
		excludedClasses: [...classCounts].map(([primaryClass, counts]) => ({ ...counts, primaryClass })).sort((left, right) => left.primaryClass < right.primaryClass ? -1 : 1),
		exclusionPolicyIds,
		fileManifestDigest,
	...
	const inventoryOnly = artifacts.filter((artifact) => artifact.disposition === 'INVENTORY_ONLY').length;
	const included = artifacts.length;
	const excluded = excludedArtifacts.reduce((count, artifact) => count + (typeof artifact.physicalFileCount === 'number' ? artifact.physicalFileCount : 0), 0);
	const discovered = capture.discoveredArtifactCount;
	const population = { analyzed, discovered, excluded, failed: 0, included, inventoryOnly, reconciles: discovered === included + excluded && included === analyzed + inventoryOnly };
