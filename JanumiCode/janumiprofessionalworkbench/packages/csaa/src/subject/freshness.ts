import type { FrozenSubject, FrozenSubjectFreshness, ResolveSubjectRequest, SubjectDiagnostic } from '../contracts/subject.js';
...
export function verifyFrozenSubject(subject: FrozenSubject, options: { readonly rootLocator: string }): FrozenSubjectFreshness {
	...
	if (current.subject.descriptor.subjectId === subject.descriptor.subjectId) return { changedPaths: [], diagnostics: [], state: 'CURRENT' };
	const diagnostics: SubjectDiagnostic[] = [{ code: 'SUBJECT_CHANGED_DURING_RESOLUTION', message: 'The frozen subject no longer matches current repository membership and bytes.', path: null, phase: 'FRESHNESS', severity: 'WARNING' }];
