...
function inspectPlainData(
	...
	const pending: Frame[] = [{ exit: false, path: rootPath, value }];
	...
	while (pending.length > 0) {
		...
		if (array) {
			...
			if (keys.length !== current.length + 1 || keys.some((key) => !dense(key)))
				return {
					budget: false,
...
function lookupFact(
	...
	for (const assignment of snapshot.assignments) {
		...
		if (
			...
			continue;
		const elementChildren = directChildren(value, model);
		if (elementChildren.length !== 2) continue;
		let registryNode = elementChildren[0]!;
		let supportedRegistryShape = true;
		while (TRANSPARENT_EXPRESSION_KINDS.has(registryNode.kind)) {
			...
			if (expressionChildren.length !== 1) {
				supportedRegistryShape = false;
				break;
			}
			registryNode = expressionChildren[0]!;
		}
		if (
			!supportedRegistryShape ||
			registryNode.kind !== ts.SyntaxKind.Identifier ||
			registryNode.syntacticIdentifierText !== registryName
		)
			continue;
		const key = elementChildren[1]!;
		let commandNode: SemanticAstNodeRecord;
		let commandTypeNode: SemanticAstNodeRecord;
		try {
			...
			if (
				...
				continue;
			commandNode = keyChildren[0]!;
			commandTypeNode = keyChildren[1]!;
		} catch {
			continue;
		}
		const registryReference = exactReference(registryNode, model, ['SYMBOL_USE']);
		const commandBaseReference = exactReference(commandNode, model, ['SYMBOL_USE']);
		const commandTypeReference = exactReference(commandTypeNode, model, ['MEMBER_NAME']);
		const targetReference = exactReference(target, model, ['DECLARATION_NAME']);
		matches.push({
			assignment,
...
function expectedTopology(
	...
	const retainedCensus = commandDispatchTopologyRetainedCensusReference(subject);
	...
	if (
		orderedStarts.some((start) => start === undefined) ||
	...
	const serializedReferences = [
		commandsLookup.commandTypeReference,
		commandsLookup.registryReference,
		handlersLookup.commandTypeReference,
		handlersLookup.registryReference,
		payloadValidation.calleeReference,
		guard.handlerReference,
		handlerInvocation.calleeReference
	];
	...
	try {
		...
		if (!same(graph.layers, expected.layers))
			add(
				...
				'Layer partitions do not independently reconcile.',
				'$.layers'
			);
		if (
