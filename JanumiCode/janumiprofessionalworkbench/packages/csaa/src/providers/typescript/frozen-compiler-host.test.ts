...
import { CompilerInputCaptureError, CompilerInputJournal, LiveCompilerInputReader, recheckCompilerInputJournal } from './compiler-input-journal.js';
import { createCapturingCompilerEnvironment, createCapturingCompilerHost, createReplayCompilerEnvironment, createReplayCompilerHost, type ExtendedCompilerHost } from './frozen-compiler-host.js';
...
function fixture(): { readonly frozen: FrozenSubject; readonly recipe: ProgramRecipe; readonly root: string } {
...
function withIndexBytes(frozen: FrozenSubject, bytes: Uint8Array): FrozenSubject {
	const changed = {
		...frozen,
		artifacts: frozen.artifacts.map((artifact) => artifact.path === 'src/index.ts' ? { ...artifact, bytes: bytes.byteLength, sha256: sha256(bytes) } : artifact)
	};
	attachFrozenSubjectBytes(changed, new Map(Object.entries(FIXTURE_FILES).map(([path, content]) => [path, path === 'src/index.ts' ? bytes : new TextEncoder().encode(content)])));
	return changed;
}
...
function exercise(host: ExtendedCompilerHost, recipe: ReturnType<typeof materializeProgramRecipe>) {
	...
	const directoryResults = [host.directoryExists!(sourceDirectory), host.directoryExists!(join(sourceDirectory, 'not-frozen'))];
	const program = ts.createProgram({ host, options: recipe.compilerOptions, projectReferences: [...recipe.projectReferences], rootNames: [...recipe.rootNames] });
	...
	].map((diagnostic) => diagnostic.code).sort((left, right) => left - right);
	return {
		directoryResults,
		diagnostics,
		identifiers,
		sources: program.getSourceFiles().map((source) => host.getCanonicalFileName(source.fileName)).sort()
	};
}
...
describe('two-pass frozen CompilerHost', () => {
	it('requires the supplied materialization to be the exact projection of the authoritative ProgramRecipe', () => {
		...
		const unrelated = { ...materialized, rootNames: [materialized.rootNames[1] ?? join(root, 'src/dep.ts')] };
	...
	it('rejects hostile materialized projections and readDirectory arrays without executing traps', () => {
		...
		const hostileOptions = new Proxy({ ...materialized.compilerOptions }, { get: trap, getOwnPropertyDescriptor: trap, getPrototypeOf: trap, ownKeys: trap });
		const firstEnvironment = createCapturingCompilerEnvironment(frozen, root, BUDGETS);
		expectCaptureCode(() => firstEnvironment.createProjectHost(recipe, { ...materialized, compilerOptions: hostileOptions }), 'INVALID_QUERY');
		expect(traps).toBe(0);
		const accessorOptions = { ...materialized.compilerOptions } as Record<string, unknown>;
		Object.defineProperty(accessorOptions, 'strict', { enumerable: true, get() { traps += 1; return true; } });
		const accessorEnvironment = createCapturingCompilerEnvironment(frozen, root, BUDGETS);
		expectCaptureCode(() => accessorEnvironment.createProjectHost(recipe, { ...materialized, compilerOptions: accessorOptions as ts.CompilerOptions }), 'INVALID_QUERY');
		expect(traps).toBe(0);

		const environment = createCapturingCompilerEnvironment(frozen, root, BUDGETS);
		const host = environment.createProjectHost(recipe, materialized);
		const proxiedExtensions = new Proxy(['.ts'], { get: trap, getOwnPropertyDescriptor: trap, getPrototypeOf: trap, ownKeys: trap });
	...
	it('snapshots materialized inputs, freezes the host, and enforces host-side budgets before hostile reflection or path conversion', () => {
		...
		let getters = 0;
		const countEnvironment = createCapturingCompilerEnvironment(frozen, root, { ...BUDGETS, maxDirectoryEntries: 2 });
		const countHost = countEnvironment.createProjectHost(recipe, materializeProgramRecipe(recipe, root));
		expect(countHost.fileExists(join(root, 'src/index.ts'))).toBe(true);
		const oversized = ['.d.ts', '.ts', '.tsx'];
		Object.defineProperty(oversized, '2', { enumerable: true, get() { getters += 1; return '.tsx'; } });
		...
		const keyEnvironment = createCapturingCompilerEnvironment(frozen, root, { ...BUDGETS, maxPathCharacters: 32 });
		const oversizedKey = { ...materializeProgramRecipe(recipe, root), configFilePath: join(root, 'x'.repeat(100)) } as Record<string, unknown>;
		Object.defineProperty(oversizedKey, 'compilerOptions', { enumerable: true, get() { getters += 1; return {}; } });
		expectCaptureCode(() => keyEnvironment.createProjectHost(recipe, oversizedKey as unknown as ReturnType<typeof materializeProgramRecipe>), 'BUDGET_EXCEEDED');
		expect(getters).toBe(0);

		const pathEnvironment = createCapturingCompilerEnvironment(frozen, root, { ...BUDGETS, maxPathCharacters: 32 });
		const pathHost = pathEnvironment.createProjectHost(recipe, materializeProgramRecipe(recipe, root));
		...
		const capacityEnvironment = createCapturingCompilerEnvironment(frozen, root, { ...BUDGETS, maxProjects: 1 });
		capacityEnvironment.createProjectHost(recipe, materializeProgramRecipe(recipe, root));
		let traps = 0;
		const trap = (): never => { traps += 1; throw new TypeError('hostile trap'); };
		const hostileRecipe = new Proxy(recipe, { get: trap, getOwnPropertyDescriptor: trap, getPrototypeOf: trap, ownKeys: trap });
		const hostileMaterialized = new Proxy(materializeProgramRecipe(recipe, root), { get: trap, getOwnPropertyDescriptor: trap, getPrototypeOf: trap, ownKeys: trap });
		expectCaptureCode(() => capacityEnvironment.createProjectHost(hostileRecipe, hostileMaterialized), 'BUDGET_EXCEEDED');
		expect(traps).toBe(0);
	...
	it('exposes deterministic hash/newline callbacks and closes duplicate, failed-evidence, and finalized capture states', () => {
		...
		expectCaptureCode(() => missingEvidence.currentProjectEvidence('missing/tsconfig.json'), 'INVALID_QUERY');
	...
	it('rejects cyclic/non-finite materializations and invalid authoritative recipes before exposing a host', () => {
		...
		const cyclic = { ...materialized, compilerOptions: cyclicOptions } as unknown as ReturnType<typeof materializeProgramRecipe>;
		...
		const nonFinite = { ...materialized, compilerOptions: { ...materialized.compilerOptions, maxNodeModuleJsDepth: Number.POSITIVE_INFINITY } };
		const nonFiniteEnvironment = createCapturingCompilerEnvironment(frozen, root, BUDGETS);
		expectCaptureCode(() => nonFiniteEnvironment.createProjectHost(recipe, nonFinite), 'INVALID_QUERY');
		...
		expectCaptureCode(() => invalidRecipeEnvironment.createProjectHost(invalidRecipe, materialized), 'INVALID_QUERY');
	...
	it('poisons every replay failure seam and refuses all operations after successful finalization', () => {
		...
		expectCaptureCode(() => unconsumedProject.assertProjectConsumed(recipe.configPath), 'UNCONSUMED_QUERY');
		...
		expectCaptureCode(() => unconsumedAll.assertProjectConsumed(recipe.configPath), 'INVALID_CAPTURE');
		...
		expectCaptureCode(() => invalid.createProjectHost(invalidRecipe, materialized), 'INVALID_QUERY');
	...
	it('checks the original deadline before reflecting any readDirectory array', () => {
		...
		const { frozen, recipe, root } = fixture();
		const environment = createCapturingCompilerEnvironment(frozen, root, { ...BUDGETS, maxDurationMs: 100 }, 10_000);
		const host = environment.createProjectHost(recipe, materializeProgramRecipe(recipe, root));
		let getters = 0;
		const extensions = ['.ts'];
		Object.defineProperty(extensions, '0', { enumerable: true, get() { getters += 1; return '.ts'; } });
	...
	it('maps SourceFile names through one pure capture/replay logical-path seam without journal queries', () => {
		...
		const toolchainLogical = host.toLogicalPath(ts.getDefaultLibFilePath(materialized.compilerOptions));
		...
		expect(replayHost.toLogicalPath(ts.getDefaultLibFilePath(materialized.compilerOptions))).toBe(toolchainLogical);
	...
	it('records forbidden host paths and absolute wildcard patterns lexically so later junctions remain observationally absent', () => {
		...
		expect(replayHost.readDirectory(root, ['.ts'], [absoluteExclude], ['.'])).toEqual(directoryResult);
		replay.assertFullyConsumed();
	});
	...
	it('preserves undefined extensions as all files while explicit empty extensions remain none through replay', () => {
		...
		const capture = createCapturingCompilerEnvironment(frozen, root, BUDGETS);
		const host = capture.createProjectHost(recipe, materialized);
		const virtualAll = host.readDirectory(root, undefined, [], ['.']).map((path) => host.toLogicalPath(path));
		const virtualNone = host.readDirectory(root, [], [], ['.']).map((path) => host.toLogicalPath(path));
		const liveRoot = join(root, 'node_modules/mixed');
		const liveAll = host.readDirectory(liveRoot, undefined, [], ['.']).map((path) => host.toLogicalPath(path));
		const liveNone = host.readDirectory(liveRoot, [], [], ['.']).map((path) => host.toLogicalPath(path));
		expect(virtualAll).toEqual(['src/dep.ts', 'src/index.ts', 'tsconfig.json']);
		expect(virtualNone).toEqual([]);
		expect(liveAll).toEqual(['node_modules/mixed/a.d.ts', 'node_modules/mixed/b.d.mts', 'node_modules/mixed/package.json']);
		expect(liveNone).toEqual([]);
		const verified = recheckCompilerInputJournal(capture.finalizeCapture());
		const replay = createReplayCompilerEnvironment(frozen, verified);
		const replayHost = replay.createProjectHost(recipe, materialized);
		expect(replayHost.readDirectory(root, undefined, [], ['.']).map((path) => replayHost.toLogicalPath(path))).toEqual(virtualAll);
		expect(replayHost.readDirectory(root, [], [], ['.'])).toEqual([]);
		expect(replayHost.readDirectory(liveRoot, undefined, [], ['.']).map((path) => replayHost.toLogicalPath(path))).toEqual(liveAll);
	...
	it('binds authoritative project-resolution identity independently of identical materialized inputs', () => {
		...
		const changedRecipe = { ...changedBase, projectResolutionDigest: programRecipeDigest(changedBase) };
		const changedMaterialized = materializeProgramRecipe(changedRecipe, root);
		expect(changedMaterialized).toEqual(materialized);
		rmSync(root, { force: true, recursive: true });
		const replay = createReplayCompilerEnvironment(frozen, verified);
		expectCaptureCode(() => replay.createProjectHost(changedRecipe, changedMaterialized), 'INVALID_CAPTURE');
	...
	it('rejects overlong enumerated children through the host before following their outside targets', () => {
		...
		symlinkSync(outside, join(root, 'node_modules/pkg', `long-${'x'.repeat(80)}`), process.platform === 'win32' ? 'junction' : 'dir');
		const environment = createCapturingCompilerEnvironment(frozen, root, { ...BUDGETS, maxPathCharacters: 32 });
		const host = environment.createProjectHost(recipe, materializeProgramRecipe(recipe, root));
		expectCaptureCode(() => host.getDirectories!(join(root, 'node_modules/pkg')), 'BUDGET_EXCEEDED');
		expectCaptureCode(() => environment.finalizeCapture(), 'INVALID_CAPTURE');
	});
	...
	it('captures, rechecks, and replays a fresh Program after the live repository is removed', () => {
		...
		expect(entries.some((entry) => entry.observation.operation === 'READ_FILE' && entry.observation.result === 'PRESENT')).toBe(true);
		expect(entries.some((entry) => entry.observation.operation === 'USE_CASE_SENSITIVE_FILE_NAMES')).toBe(true);
		const verified = recheckCompilerInputJournal(finalized);
	...
	it('decodes supported BOMs and reports malformed source bytes through a typed failure', () => {
		...
		for (let index = 0; index < bigEndianBody.length; index += 2) [bigEndianBody[index], bigEndianBody[index + 1]] = [bigEndianBody[index + 1]!, bigEndianBody[index]!];
		for (const bytes of [
			new Uint8Array([0xef, 0xbb, 0xbf, ...new TextEncoder().encode(FIXTURE_FILES['src/index.ts'])]),
			...
			const capture = createCapturingCompilerHost(frozen, fixtureValue.root, fixtureValue.recipe, materialized, BUDGETS);
		...
		for (const malformedBytes of [
			...
			const capture = createCapturingCompilerHost(malformed, malformedFixture.root, malformedFixture.recipe, materialized, BUDGETS);
			try {
	...
	it('captures the pinned TypeScript package context for a normal noLib:false Program and replays it without repository I/O', () => {
		const fixtureValue = fixture();
		const toolchainBudgets = { ...BUDGETS, maxContextBytes: 16 * 1024 * 1024, maxContextFileBytes: 4 * 1024 * 1024 };
		const { projectResolutionDigest: _digest, ...recipeWithoutDigest } = fixtureValue.recipe;
		const noLibBase = { ...recipeWithoutDigest, compilerOptions: { ...recipeWithoutDigest.compilerOptions, noLib: false } };
		...
		const capture = createCapturingCompilerHost(fixtureValue.frozen, fixtureValue.root, noLibRecipe, materialized, toolchainBudgets);
		const passA = exercise(capture.host, materialized);
		const finalized = capture.finalizeCapture();
		const entries = finalized.entries;
		const toolchainReads = entries.filter((entry) => entry.observation.operation === 'READ_FILE' && entry.observation.result === 'PRESENT' && entry.observation.logicalPath.startsWith('@toolchain/typescript/'));
		expect(toolchainReads.length).toBeGreaterThan(0);
		expect(toolchainReads.every((entry) => entry.observation.operation === 'READ_FILE' && entry.observation.result === 'PRESENT'
			&& entry.observation.origin === 'TOOLCHAIN_LIBRARY' && entry.observation.byteBudgetClass === 'LIVE_COMPILER_CONTEXT')).toBe(true);
		const verified = recheckCompilerInputJournal(finalized);
		rmSync(fixtureValue.root, { force: true, recursive: true });
		const replay = createReplayCompilerHost(fixtureValue.frozen, noLibRecipe, materialized, verified);
	...
	it('uses one snapshot-wide authority and reproduces exact per-project attribution across two fresh Programs', () => {
		...
		symlinkSync(join(root, 'packages/shared'), join(root, 'node_modules/@fixture/shared'), process.platform === 'win32' ? 'junction' : 'dir');
		const artifacts = Object.entries(files).map(([path, content]) => ({
			bytes: Buffer.byteLength(content),
			canonicalPathKey: path.toLowerCase(),
			disposition: 'ANALYZED' as const,
		...
		const workspace = { exports: [], kind: 'PACKAGE', manifestPath: 'packages/shared/package.json', name: '@fixture/shared', path: 'packages/shared', private: true, provenance: [], workspacePatterns: [] } as FrozenSubject['workspaces'][number];
		const frozen = {
			artifacts,
			descriptor: { subjectId: 'd'.repeat(64) },
			diagnostics: [], excludedArtifacts: [], generatedContexts: [],
			population: { analyzed: artifacts.length, discovered: artifacts.length, excluded: 0, failed: 0, included: artifacts.length, inventoryOnly: 0, reconciles: true },
			projects: [], request: {}, workspaces: [workspace]
		} as unknown as FrozenSubject;
		attachFrozenSubjectBytes(frozen, new Map(Object.entries(files).map(([path, content]) => [path, new TextEncoder().encode(content)])));
		const recipe = (name: 'a' | 'b'): ProgramRecipe => {
			const base = {
				compilerOptions: { configFilePath: `packages/${name}/tsconfig.json`, module: ts.ModuleKind.ESNext, moduleResolution: ts.ModuleResolutionKind.Bundler, noEmit: true, noLib: true, strict: true, target: ts.ScriptTarget.ES2022 },
				configClosureDigest: name.repeat(64), configPath: `packages/${name}/tsconfig.json`, kind: 'PROJECT' as const, projectReferences: [],
				provider: { id: 'typescript' as const, version: TYPESCRIPT_PROVIDER_VERSION }, rootNames: [`packages/${name}/src/index.ts`]
			};
		...
		expect(hostA.toLogicalPath(workspaceAliasFile)).toBe('node_modules/@fixture/shared/src/index.ts');
		const passA = exercise(hostA, materializedA);
		const passB = exercise(hostB, materializedB);
		const finalized = capture.finalizeCapture();
		const entries = finalized.entries;
		const aliasTopology = entries.find((entry) => entry.query.operation === 'REALPATH' && entry.query.logicalPath === 'node_modules/@fixture/shared');
		expect(aliasTopology?.observation).toMatchObject({ invocationCount: 2, resolvedLogicalPath: 'packages/shared', result: 'RESOLVED' });
		const attributions = finalized.projectAttributions;
		expect(attributions).toHaveLength(2);
		expect(attributions.every((attribution) => attribution.queryInvocations.some((record) => record.query.operation === 'REALPATH' && record.query.logicalPath === 'node_modules/@fixture/shared'))).toBe(true);
		const verified = recheckCompilerInputJournal(finalized);
		rmSync(root, { force: true, recursive: true });
		const replay = createReplayCompilerEnvironment(frozen, verified);
		const replayA = replay.createProjectHost(recipeA, materializedA);
		const replayB = replay.createProjectHost(recipeB, materializedB);
		expect(replayA.toLogicalPath(workspaceAliasFile)).toBe('node_modules/@fixture/shared/src/index.ts');
	...
	it('poisons capture and replay atomically when mandatory workspace-alias setup fails after a committed prefix', () => {
		...
		for (const name of ['a', 'b']) {
			...
			symlinkSync(join(fixtureValue.root, `packages/${name}`), join(fixtureValue.root, `node_modules/@fixture/${name}`), process.platform === 'win32' ? 'junction' : 'dir');
		}
		const workspaces = (['a', 'b'] as const).map((name) => ({ exports: [], kind: 'PACKAGE', manifestPath: `packages/${name}/package.json`, name: `@fixture/${name}`, path: `packages/${name}`, private: true, provenance: [], workspacePatterns: [] })) as FrozenSubject['workspaces'];
		const frozen = { ...fixtureValue.frozen, workspaces } as FrozenSubject;
		attachFrozenSubjectBytes(frozen, new Map(Object.entries(FIXTURE_FILES).map(([path, content]) => [path, new TextEncoder().encode(content)])));
		const materialized = materializeProgramRecipe(fixtureValue.recipe, fixtureValue.root);

		const captureEnvironment = createCapturingCompilerEnvironment(frozen, fixtureValue.root, { ...BUDGETS, maxCompilerQueries: 1 });
		expectCaptureCode(() => captureEnvironment.createProjectHost(fixtureValue.recipe, materialized), 'BUDGET_EXCEEDED');
		expectCaptureCode(() => captureEnvironment.finalizeCapture(), 'INVALID_CAPTURE');

		const paths = new FrozenCompilerPathResolver(frozen, fixtureValue.root, ts.sys.useCaseSensitiveFileNames);
		...
		journal.capture({ logicalPath: paths.workspaceAliasRoots()[0]!, operation: 'REALPATH' }, fixtureValue.recipe.configPath);
		...
		expectCaptureCode(() => replayEnvironment.createProjectHost(fixtureValue.recipe, materialized), 'UNRECORDED_QUERY');
