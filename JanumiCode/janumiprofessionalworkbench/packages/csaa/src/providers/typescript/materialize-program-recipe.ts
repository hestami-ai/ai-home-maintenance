...
export { MATERIALIZED_ARRAY_PATH_OPTIONS, MATERIALIZED_SCALAR_PATH_OPTIONS, PRESERVED_NON_PATH_OPTIONS, REJECTED_PATH_OPTIONS } from '../../semantic/program-recipe-policy.js';
...
export class ProgramRecipeMaterializationError extends Error {
	constructor(readonly code: 'INVALID_RECIPE' | 'PATH_ESCAPE' | 'VERSION_MISMATCH', message: string) {
		super(message);
...
function logicalPath(value: unknown, field: string, allowRepositoryRoot = false): string {
	return validateProgramRecipeLogicalPath(value, field, Number.MAX_SAFE_INTEGER, allowRepositoryRoot);
}
...
function canonicalRepositoryRoot(repositoryRoot: unknown): string {
	if (typeof repositoryRoot !== 'string' || !isAbsolute(repositoryRoot)) invalid('Repository root must be an absolute existing directory at the runtime boundary.');
	const rootSegments = repositoryRoot.slice(parse(repositoryRoot).root.length).split(/[\\/]/u);
	if (rootSegments.some((segment) => segment === '.' || segment === '..')) invalid('Repository root must not contain unresolved dot segments.');
	let stats;
	try {
	...
	if (!stats.isDirectory()) invalid('Repository root must be an absolute existing directory at the runtime boundary.');
	try {
...
function canonicalizeCandidate(repositoryRoot: string, lexicalCandidate: string, field: string): string {
	if (!inside(repositoryRoot, lexicalCandidate)) throw new ProgramRecipeMaterializationError('PATH_ESCAPE', `${field} escapes the repository root.`);
	...
	while (existingAncestor !== repositoryRoot) {
		try {
			...
			if (code !== 'ENOENT' && code !== 'ENOTDIR') invalid(`${field} could not be inspected for path confinement.`);
	...
	if (!inside(repositoryRoot, canonicalCandidate)) throw new ProgramRecipeMaterializationError('PATH_ESCAPE', `${field} escapes the repository root through filesystem indirection.`);
...
function materializePath(repositoryRoot: string, value: unknown, field: string): string {
	...
	const lexicalCandidate = logical === '.' ? repositoryRoot : resolve(repositoryRoot, ...logical.split('/'));
...
function validatePathSubstitutions(
	...
	for (const [pattern, substitutions] of Object.entries(paths)) {
		for (const [index, substitution] of substitutions.entries()) {
			...
			if (substitution.length === 0 || substitution.includes('\\') || substitution.startsWith('/') || /^[A-Za-z]:/u.test(substitution)
				|| substitution.split('/').some((segment) => segment === '')) invalid(`${field} must be a non-absolute slash-form TypeScript path substitution.`);
...
function materializeProgramRecipeUnsafe(recipeValue: unknown, repositoryRootValue: unknown): MaterializedProgramRecipe {
	const repositoryRoot = canonicalRepositoryRoot(repositoryRootValue);
	if (ts.version !== TYPESCRIPT_PROVIDER_VERSION) {
		throw new ProgramRecipeMaterializationError('VERSION_MISMATCH', `ProgramRecipe requires TypeScript ${TYPESCRIPT_PROVIDER_VERSION}; runtime is ${ts.version}.`);
	}
	...
	for (const [key, value] of Object.entries(recipe.compilerOptions)) {
		...
		if (MATERIALIZED_ARRAY_PATH_OPTIONS.has(key)) {
			...
			compilerOptions[key] = paths.map((entry, index) => materializePath(repositoryRoot, entry, `compilerOptions.${key}[${index}]`));
			continue;
		}
		compilerOptions[key] = value;
	}
	if (recipe.compilerOptions.paths !== undefined) {
		const effectiveBaseLogical = typeof recipe.compilerOptions.baseUrl === 'string'
		...
		const effectiveBase = materializePath(repositoryRoot, effectiveBaseLogical, 'compilerOptions.paths effective base');
		validatePathSubstitutions(recipe.compilerOptions.paths as Readonly<Record<string, readonly string[]>>, repositoryRoot, effectiveBase);
		if (recipe.compilerOptions.baseUrl === undefined && recipe.compilerOptions.pathsBasePath === undefined) compilerOptions.pathsBasePath = effectiveBase;
	}
	...
	return {
		...
		projectReferences: recipe.projectReferences.map((path) => ({ path: materializePath(repositoryRoot, path, 'projectReferences') })),
		rootNames: recipe.rootNames.map((path) => materializePath(repositoryRoot, path, 'rootNames'))
	};
}

export function materializeProgramRecipe(recipe: ProgramRecipe, repositoryRoot: string): MaterializedProgramRecipe {
	try {
		...
		if (error instanceof ProgramRecipePolicyError) throw new ProgramRecipeMaterializationError(error.code, error.message);
		throw new ProgramRecipeMaterializationError('INVALID_RECIPE', 'ProgramRecipe could not be read as inert, valid wire data.');
	}
}
