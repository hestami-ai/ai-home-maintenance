...
export const ARROW_COMMAND_CENSUS_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-arrow-command-census-progress-transport/0.1.0' as const;
export const ARROW_COMMAND_CENSUS_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const ARROW_COMMAND_CENSUS_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface ArrowCommandCensusProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_ARROW_COMMAND_CENSUS_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof ARROW_COMMAND_CENSUS_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface ArrowCommandCensusProgressJsonlWriter {
	emit(event: ArrowCommandCensusReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface ArrowCommandCensusProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createArrowCommandCensusProgressJsonlWriter(
	options: ArrowCommandCensusProgressJsonlWriterOptions
): ArrowCommandCensusProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		ARROW_COMMAND_CENSUS_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, ARROW_COMMAND_CENSUS_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
	...
	const truncate = (reason: ArrowCommandCensusProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: ArrowCommandCensusProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
			...
			protocolRole: 'PRELIMINARY_ARROW_COMMAND_CENSUS_REPORT_TELEMETRY_TRANSPORT',
			reason,
			schemaVersion: ARROW_COMMAND_CENSUS_PROGRESS_TRANSPORT_SCHEMA_VERSION,
			state: 'TRUNCATED'
		});
