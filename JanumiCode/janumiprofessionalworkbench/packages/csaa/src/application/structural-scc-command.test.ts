...
import { STRUCTURAL_SCC_PROGRESS_TRANSPORT_SCHEMA_VERSION } from './structural-scc-progress-jsonl.js';

const REPOSITORY_ROOT = fileURLToPath(new URL('../../../../', import.meta.url));
const SCRIPT = fileURLToPath(
	new URL('../../../../scripts/csaa-structural-scc.ts', import.meta.url)
);
...
describe('structural SCC command adapter', () => {
	it.each([
		{ args: [] },
		{ args: ['--unknown'] },
		{ args: ['--request'] },
			...
			progress.every(
				(event) =>
					...
					JSON.stringify(event.nonclaims) ===
						JSON.stringify(STRUCTURAL_SCC_REPORT_PROGRESS_NONCLAIMS)
			)
		).toBe(true);
	...
	it('delivers one accepted partial report and the complete stage transcript', () => {
		const result = run(
			['--stdin'],
			JSON.stringify({
				budgets: STRUCTURAL_SCC_REPORT_SAFETY_CEILINGS,
		...
		expect(JSON.parse(result.stdout)).toMatchObject({
			...
			outcome: 'partial',
			result: {
				capability: { id: 'JAN-CSAA-CAP-027', status: 'PARTIAL' },
		...
		expect(result.stdout).not.toContain(REPOSITORY_ROOT.replaceAll('\\', '/'));
		expect(result.stderr).not.toContain(REPOSITORY_ROOT);
		expect(result.stderr).not.toContain(REPOSITORY_ROOT.replaceAll('\\', '/'));
	}, 120_000);
	...
	it('bounds request-file reads before parsing', () => {
		const root = mkdtempSync(join(tmpdir(), 'csaa-scc-command-request-'));
