...
export interface StructuralModuleReachabilityProgressTransportTruncation {
	...
	readonly schemaVersion: typeof STRUCTURAL_MODULE_REACHABILITY_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface StructuralModuleReachabilityProgressJsonlWriter {
	emit(event: StructuralModuleReachabilityReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface StructuralModuleReachabilityProgressJsonlWriterOptions {
...
export function createStructuralModuleReachabilityProgressJsonlWriter(
	options: StructuralModuleReachabilityProgressJsonlWriterOptions
): StructuralModuleReachabilityProgressJsonlWriter {
	...
	const truncate = (
		reason: StructuralModuleReachabilityProgressTransportTruncation['reason']
	): void => {
		if (truncated) return;
		truncated = true;
		const marker: StructuralModuleReachabilityProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
			...
			reason,
			schemaVersion: STRUCTURAL_MODULE_REACHABILITY_PROGRESS_TRANSPORT_SCHEMA_VERSION,
			state: 'TRUNCATED'
		});
