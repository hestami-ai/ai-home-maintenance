...
} from './structural-module-reachability-progress-jsonl.js';

function event(detailCode: string): StructuralModuleReachabilityReportProgressEvent {
	return {
		...
		observations: [],
		operationVersion: 'jan-csaa-report-structural-module-reachability/0.1.0',
		phase: 'REQUEST_BIND',
		protocolRole: 'PRELIMINARY_CAP_027_REPORT_TELEMETRY',
		reportIdentityEffect: 'EXCLUDED_FROM_REPORT_IDENTITY',
		schemaVersion: 'jan-csaa-structural-module-reachability-report-progress/0.1.0',
		sequence: 1,
		stage: 'REQUEST',
		state: 'STARTED',
...
describe('structural reachability progress JSONL transport', () => {
	it('reserves one bounded marker when the event limit is reached', () => {
		const lines: string[] = [];
		const writer = createStructuralModuleReachabilityProgressJsonlWriter({
			maxBytes: 16_384,
			maxEvents: 3,
