...
export const GUARD_CLASSIFICATION_OVERLAY_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-guard-classification-overlay-progress-transport/0.1.0' as const;
export const GUARD_CLASSIFICATION_OVERLAY_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const GUARD_CLASSIFICATION_OVERLAY_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface GuardClassificationOverlayProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_TYPESCRIPT_GUARD_CLASSIFICATION_OVERLAY_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof GUARD_CLASSIFICATION_OVERLAY_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface GuardClassificationOverlayProgressJsonlWriter {
	emit(event: GuardClassificationOverlayReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface GuardClassificationOverlayProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createGuardClassificationOverlayProgressJsonlWriter(
	options: GuardClassificationOverlayProgressJsonlWriterOptions
): GuardClassificationOverlayProgressJsonlWriter {
	...
	const truncate = (
		reason: GuardClassificationOverlayProgressTransportTruncation['reason']
	): void => {
		if (truncated) return;
		truncated = true;
		const marker: GuardClassificationOverlayProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
