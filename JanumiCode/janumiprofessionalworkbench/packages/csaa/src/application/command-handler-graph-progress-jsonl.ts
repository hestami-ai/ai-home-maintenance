...
export const COMMAND_HANDLER_GRAPH_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-command-handler-graph-progress-transport/0.1.0' as const;
export const COMMAND_HANDLER_GRAPH_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const COMMAND_HANDLER_GRAPH_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface CommandHandlerGraphProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_TYPESCRIPT_COMMAND_HANDLER_GRAPH_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof COMMAND_HANDLER_GRAPH_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface CommandHandlerGraphProgressJsonlWriter {
	emit(event: CommandHandlerGraphReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface CommandHandlerGraphProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createCommandHandlerGraphProgressJsonlWriter(
	options: CommandHandlerGraphProgressJsonlWriterOptions
): CommandHandlerGraphProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		COMMAND_HANDLER_GRAPH_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, COMMAND_HANDLER_GRAPH_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
	...
	const truncate = (reason: CommandHandlerGraphProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: CommandHandlerGraphProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
			...
			protocolRole: 'PRELIMINARY_TYPESCRIPT_COMMAND_HANDLER_GRAPH_REPORT_TELEMETRY_TRANSPORT',
			reason,
			schemaVersion: COMMAND_HANDLER_GRAPH_PROGRESS_TRANSPORT_SCHEMA_VERSION,
			state: 'TRUNCATED'
		});
