...
export interface SemanticSourceQueryProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_SEMANTIC_SOURCE_QUERY_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof SEMANTIC_SOURCE_QUERY_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface SemanticSourceQueryProgressJsonlWriter {
	emit(event: SemanticSourceQueryReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface SemanticSourceQueryProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createSemanticSourceQueryProgressJsonlWriter(
	options: SemanticSourceQueryProgressJsonlWriterOptions
): SemanticSourceQueryProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		SEMANTIC_SOURCE_QUERY_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, SEMANTIC_SOURCE_QUERY_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
	...
	const truncate = (reason: SemanticSourceQueryProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: SemanticSourceQueryProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
