...
describe('guard-enforcement-ledger report', () => {
	it('rejects malformed request records, budgets, paths, and project arrays without invoking production work', async () => {
		...
		Object.defineProperty(topLevelNonEnumerable, 'schemaVersion', {
			enumerable: false,
			value: GUARD_ENFORCEMENT_LEDGER_REPORT_REQUEST_SCHEMA_VERSION
		});
		for (const malformed of [new Date(), topLevelNonEnumerable]) {
			expect(admitGuardEnforcementLedgerReportRequest(malformed)).toMatchObject({
		...
		for (const [overrides, code, state] of [
			...
			[
				{
					budgets: {
						...
						maxResultBytes: GUARD_ENFORCEMENT_LEDGER_REPORT_SAFETY_CEILINGS.maxResultBytes + 1
					}
				},
			...
			expect(
				admitGuardEnforcementLedgerReportRequest({ ...acceptedRequest(), ...overrides })
		...
		const publicOutcome = await runGuardEnforcementLedgerReport(null, { repositoryRoot: root });
		expect(publicOutcome).toMatchObject({
			code: 'REQUEST_SHAPE_INVALID',
			outcome: 'unavailable',
			stage: 'REQUEST'
		});
		expect(guardEnforcementLedgerReportExitCode(publicOutcome)).toBe(2);
	});
	...
	it('classifies artifact-set refusal, incompatibility, and producer failure independently', async () => {
		for (const [diagnosticCode, state] of [
			...
			const outcome = await runGuardEnforcementLedgerReportWithDependencies(
				...
				dependencies({
					buildArtifactSet: (() => ({
						...
						outcome: 'unavailable'
					})) as typeof buildGuardEnforcementLedgerArtifactSet
				})
			);
			expect(outcome).toMatchObject({
			...
			expect(guardEnforcementLedgerReportExitCode(outcome)).toBe(
	...
	it('fails closed at repository, path, progress, currentness, serialization, and internal boundaries', async () => {
		const nullOptions = await runGuardEnforcementLedgerReport(null, null as never);
		expect(nullOptions).toMatchObject({ code: 'REQUEST_SHAPE_INVALID', stage: 'REQUEST' });

		const missingRoot = await runGuardEnforcementLedgerReportWithDependencies(
			acceptedRequest(),
		...
		for (const path of ['fixture', 'fixture/missing.json']) {
			const invalidProject = await runGuardEnforcementLedgerReportWithDependencies(
		...
		const malformedProgress = await runGuardEnforcementLedgerReportWithDependencies(
			acceptedRequest(),
		...
		const diagnosticSubject = await runGuardEnforcementLedgerReportWithDependencies(
			acceptedRequest(),
			{ repositoryRoot: root },
			dependencies({
				resolveSubject: (() => ({
		...
		const unavailableFreshness = await runGuardEnforcementLedgerReportWithDependencies(
			acceptedRequest(),
			{ repositoryRoot: root },
			dependencies({
				verifySubject: (() => {
		...
		const serializationFailure = await runGuardEnforcementLedgerReportWithDependencies(
			acceptedRequest(),
			{ repositoryRoot: root },
			dependencies({
				verifySubject: (() => ({
		...
		const internalFailure = await runGuardEnforcementLedgerReportWithDependencies(
			acceptedRequest(),
			{ repositoryRoot: root },
			dependencies({
				resolveSubject: (() => {
