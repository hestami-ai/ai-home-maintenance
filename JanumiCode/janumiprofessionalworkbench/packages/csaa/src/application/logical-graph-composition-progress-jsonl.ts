...
export const LOGICAL_GRAPH_COMPOSITION_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-logical-graph-composition-progress-transport/0.1.0' as const;
export const LOGICAL_GRAPH_COMPOSITION_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const LOGICAL_GRAPH_COMPOSITION_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface LogicalGraphCompositionProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_TYPESCRIPT_LOGICAL_GRAPH_COMPOSITION_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof LOGICAL_GRAPH_COMPOSITION_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface LogicalGraphCompositionProgressJsonlWriter {
	emit(event: LogicalGraphCompositionReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface LogicalGraphCompositionProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createLogicalGraphCompositionProgressJsonlWriter(
	options: LogicalGraphCompositionProgressJsonlWriterOptions
): LogicalGraphCompositionProgressJsonlWriter {
	...
	const truncate = (reason: LogicalGraphCompositionProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: LogicalGraphCompositionProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
