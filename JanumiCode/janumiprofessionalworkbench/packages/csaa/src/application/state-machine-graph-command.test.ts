...
import { STATE_MACHINE_GRAPH_PROGRESS_TRANSPORT_SCHEMA_VERSION } from './state-machine-graph-progress-jsonl.js';
import { STATE_MACHINE_GRAPH_REPORT_PROGRESS_SCHEMA_VERSION } from './run-state-machine-graph-report.js';

const REPOSITORY_ROOT = fileURLToPath(new URL('../../../../', import.meta.url));
const SCRIPT = fileURLToPath(
	new URL('../../../../scripts/csaa-state-machine-graph.ts', import.meta.url)
);
const PACKAGE_SCRIPT = 'csaa:analyze:state-machine-graph';
...
function acceptedRequest(): Record<string, unknown> {
	return {
		...
		source: {
			logicalPath: 'packages/csaa/command-subjects/state-machine-graph/transitions.data.ts',
			projectConfigPath: 'packages/csaa/command-subjects/state-machine-graph/tsconfig.json'
		},
		subjectProjectConfigPaths: ['packages/csaa/command-subjects/state-machine-graph/tsconfig.json']
	};
}
...
describe('state-machine-graph command adapter', () => {
	it('keeps the silent package entry point machine-framed and preserves refusal exit codes', () => {
		...
		const budgets = accepted.budgets as typeof STATE_MACHINE_GRAPH_REPORT_SAFETY_CEILINGS;
			...
			JSON.stringify({
				...
				budgets: {
					...
					stateMachineGraph: {
						...
						maxNodes: STATE_MACHINE_GRAPH_REPORT_SAFETY_CEILINGS.stateMachineGraph.maxNodes + 1
					}
				}
			})
		);
	...
	it(
		'delivers one admitted full graph and a bounded six-stage transcript',
		{ timeout: 180_000 },
		() => {
			...
			expect(terminal.result.evidence.stateMachineGraph.nodes.length).toBeGreaterThan(0);
			...
			expect(stages.filter((event) => event.state === 'STARTED')).toHaveLength(6);
			expect(stages.filter((event) => event.state === 'COMPLETED')).toHaveLength(6);
	...
	it('contains closed pipes and bounds stream and regular-file request reads', async () => {
		...
		const root = mkdtempSync(join(tmpdir(), 'csaa-state-machine-graph-command-'));
