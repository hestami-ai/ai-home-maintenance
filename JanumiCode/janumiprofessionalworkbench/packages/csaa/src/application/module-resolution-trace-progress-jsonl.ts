...
export const MODULE_RESOLUTION_TRACE_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-module-resolution-trace-progress-transport/0.1.0' as const;
export const MODULE_RESOLUTION_TRACE_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const MODULE_RESOLUTION_TRACE_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface ModuleResolutionTraceProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_CAP_011_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof MODULE_RESOLUTION_TRACE_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface ModuleResolutionTraceProgressJsonlWriter {
	emit(event: ModuleResolutionTraceReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface ModuleResolutionTraceProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createModuleResolutionTraceProgressJsonlWriter(
	options: ModuleResolutionTraceProgressJsonlWriterOptions
): ModuleResolutionTraceProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		MODULE_RESOLUTION_TRACE_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, MODULE_RESOLUTION_TRACE_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
	...
	const truncate = (reason: ModuleResolutionTraceProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: ModuleResolutionTraceProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
			...
			protocolRole: 'PRELIMINARY_CAP_011_REPORT_TELEMETRY_TRANSPORT',
			reason,
			schemaVersion: MODULE_RESOLUTION_TRACE_PROGRESS_TRANSPORT_SCHEMA_VERSION,
			state: 'TRUNCATED'
		});
