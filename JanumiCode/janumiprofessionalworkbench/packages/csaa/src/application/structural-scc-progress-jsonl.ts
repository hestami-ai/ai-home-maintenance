...
export const STRUCTURAL_SCC_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-structural-scc-progress-transport/0.1.0' as const;
export const STRUCTURAL_SCC_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const STRUCTURAL_SCC_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 2_048;

export interface StructuralSccProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_CAP_027_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof STRUCTURAL_SCC_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface StructuralSccProgressJsonlWriter {
	emit(event: StructuralSccReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface StructuralSccProgressJsonlWriterOptions {
	/** Testable equal-or-stricter bound; production uses the fixed exported maximum. */
	readonly maxBytes?: number;
	/** Total stderr JSONL records including a possible truncation marker. */
	readonly maxEvents?: number;
	/** Node-style `false` means accepted with backpressure; rejected thenables are contained. */
...
export function createStructuralSccProgressJsonlWriter(
	options: StructuralSccProgressJsonlWriterOptions
): StructuralSccProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		STRUCTURAL_SCC_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, STRUCTURAL_SCC_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
		...
		try {
			...
			if (result !== undefined)
				void Promise.resolve(result).catch(() => {
					// Asynchronous transport rejections are contained outside terminal report delivery.
					truncated = true;
				});
	...
	const truncate = (reason: StructuralSccProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: StructuralSccProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
