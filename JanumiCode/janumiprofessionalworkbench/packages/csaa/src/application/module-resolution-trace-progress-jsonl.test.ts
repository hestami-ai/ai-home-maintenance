...
import {
	createModuleResolutionTraceProgressJsonlWriter,
	MODULE_RESOLUTION_TRACE_PROGRESS_TRANSPORT_SCHEMA_VERSION
} from './module-resolution-trace-progress-jsonl.js';

function event(detailCode: string): ModuleResolutionTraceReportProgressEvent {
	return {
		...
		observations: [],
		operationVersion: 'jan-csaa-report-module-resolution-trace/0.1.0',
		phase: 'REQUEST_BIND',
		protocolRole: 'PRELIMINARY_CAP_011_REPORT_TELEMETRY',
		reportIdentityEffect: 'EXCLUDED_FROM_REPORT_IDENTITY',
		schemaVersion: 'jan-csaa-module-resolution-trace-report-progress/0.1.0',
		sequence: 1,
		stage: 'REQUEST',
		state: 'STARTED',
...
describe('module-resolution-trace progress JSONL transport', () => {
	it('reserves one bounded marker when the event limit is reached', () => {
		const lines: string[] = [];
		const writer = createModuleResolutionTraceProgressJsonlWriter({
			maxBytes: 65_536,
			maxEvents: 3,
	...
	it('uses a bounded marker for byte exhaustion and unserializable events', () => {
		...
		const byteWriter = createModuleResolutionTraceProgressJsonlWriter({
			maxBytes: 32_768,
			maxEvents: 10,
		...
		const serializationWriter = createModuleResolutionTraceProgressJsonlWriter({
	...
	it('isolates throwing, rejecting, and backpressured sinks', async () => {
		const throwing = createModuleResolutionTraceProgressJsonlWriter({
			write: () => {
		...
		const backpressured = createModuleResolutionTraceProgressJsonlWriter({
			write: (line) => {
		...
		const rejecting = createModuleResolutionTraceProgressJsonlWriter({
