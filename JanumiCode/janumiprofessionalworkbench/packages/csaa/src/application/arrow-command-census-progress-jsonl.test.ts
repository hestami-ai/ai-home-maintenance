...
import {
	...
	createArrowCommandCensusProgressJsonlWriter
} from './arrow-command-census-progress-jsonl.js';
import {
...
function event(detailCode: string): ArrowCommandCensusReportProgressEvent {
	return {
...
describe('arrow-command-census progress JSONL transport', () => {
	it('reserves one bounded marker when the event limit is reached', () => {
		const lines: string[] = [];
		const writer = createArrowCommandCensusProgressJsonlWriter({
			maxBytes: 65_536,
			maxEvents: 3,
	...
	it('uses bounded markers for byte exhaustion and unserializable events', () => {
		...
		const byteWriter = createArrowCommandCensusProgressJsonlWriter({
			maxBytes: 32_768,
			maxEvents: 10,
		...
		const serializationWriter = createArrowCommandCensusProgressJsonlWriter({
			write: (line) => serializationLines.push(line)
		});
		serializationWriter.emit({
			...event('HOSTILE'),
			observations: new Proxy([], {})
		} as ArrowCommandCensusReportProgressEvent);
	...
	it('isolates throwing, rejecting, and backpressured sinks', async () => {
		const throwing = createArrowCommandCensusProgressJsonlWriter({
			write: () => {
		...
		const backpressured = createArrowCommandCensusProgressJsonlWriter({
			write: (line) => {
		...
		const rejecting = createArrowCommandCensusProgressJsonlWriter({
