...
export const DECLARATION_CONTEXT_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-declaration-context-progress-transport/0.1.0' as const;
export const DECLARATION_CONTEXT_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const DECLARATION_CONTEXT_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface DeclarationContextProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_CAP_013_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof DECLARATION_CONTEXT_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface DeclarationContextProgressJsonlWriter {
	emit(event: DeclarationContextReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface DeclarationContextProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createDeclarationContextProgressJsonlWriter(
	options: DeclarationContextProgressJsonlWriterOptions
): DeclarationContextProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		DECLARATION_CONTEXT_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, DECLARATION_CONTEXT_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
	...
	const truncate = (reason: DeclarationContextProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: DeclarationContextProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
			...
			protocolRole: 'PRELIMINARY_CAP_013_REPORT_TELEMETRY_TRANSPORT',
			reason,
			schemaVersion: DECLARATION_CONTEXT_PROGRESS_TRANSPORT_SCHEMA_VERSION,
			state: 'TRUNCATED'
		});
