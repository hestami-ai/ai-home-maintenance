...
export const MODULE_DEPENDENCY_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-module-dependency-progress-transport/0.1.0' as const;
export const MODULE_DEPENDENCY_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const MODULE_DEPENDENCY_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface ModuleDependencyProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_TYPESCRIPT_MODULE_DEPENDENCY_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof MODULE_DEPENDENCY_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface ModuleDependencyProgressJsonlWriter {
	emit(event: ModuleDependencyReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface ModuleDependencyProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createModuleDependencyProgressJsonlWriter(
	options: ModuleDependencyProgressJsonlWriterOptions
): ModuleDependencyProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		MODULE_DEPENDENCY_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, MODULE_DEPENDENCY_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
	...
	const truncate = (reason: ModuleDependencyProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: ModuleDependencyProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
