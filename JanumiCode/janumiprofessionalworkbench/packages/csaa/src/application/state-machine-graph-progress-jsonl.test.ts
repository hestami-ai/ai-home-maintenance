...
import {
	...
	createStateMachineGraphProgressJsonlWriter
} from './state-machine-graph-progress-jsonl.js';
import {
...
function event(detailCode: string): StateMachineGraphReportProgressEvent {
	return {
		...
		observations: [],
		operationVersion: 'jan-csaa-report-state-machine-graph/0.1.0',
		phase: 'REQUEST_BIND',
		protocolRole: 'PRELIMINARY_TYPESCRIPT_STATE_MACHINE_GRAPH_REPORT_TELEMETRY',
		reportIdentityEffect: 'EXCLUDED_FROM_REPORT_IDENTITY',
		schemaVersion: 'jan-csaa-state-machine-graph-report-progress/0.1.0',
		sequence: 1,
		stage: 'REQUEST',
		state: 'STARTED',
...
describe('state-machine-graph progress JSONL transport', () => {
	it('reserves one bounded marker when the event limit is reached', () => {
		const lines: string[] = [];
		const writer = createStateMachineGraphProgressJsonlWriter({
			maxBytes: 65_536,
			maxEvents: 3,
	...
	it('uses bounded markers for byte exhaustion and unserializable events', () => {
		...
		const byteWriter = createStateMachineGraphProgressJsonlWriter({
			maxBytes: 32_768,
			maxEvents: 10,
		...
		const serializationWriter = createStateMachineGraphProgressJsonlWriter({
			write: (line) => serializationLines.push(line)
		});
		serializationWriter.emit({
			...event('HOSTILE'),
			observations: new Proxy([], {})
		} as StateMachineGraphReportProgressEvent);
	...
	it('isolates throwing, rejecting, and backpressured sinks', async () => {
		const throwing = createStateMachineGraphProgressJsonlWriter({
			write: () => {
		...
		const backpressured = createStateMachineGraphProgressJsonlWriter({
			write: (line) => {
		...
		const rejecting = createStateMachineGraphProgressJsonlWriter({
