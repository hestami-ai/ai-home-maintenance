...
export const COMMAND_EVENT_CONTRACT_OVERLAY_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-command-event-contract-overlay-progress-transport/0.1.0' as const;
export const COMMAND_EVENT_CONTRACT_OVERLAY_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const COMMAND_EVENT_CONTRACT_OVERLAY_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface CommandEventContractOverlayProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_TYPESCRIPT_COMMAND_EVENT_CONTRACT_OVERLAY_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof COMMAND_EVENT_CONTRACT_OVERLAY_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface CommandEventContractOverlayProgressJsonlWriter {
	emit(event: CommandEventContractOverlayReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface CommandEventContractOverlayProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createCommandEventContractOverlayProgressJsonlWriter(
	options: CommandEventContractOverlayProgressJsonlWriterOptions
): CommandEventContractOverlayProgressJsonlWriter {
	...
	const truncate = (
		reason: CommandEventContractOverlayProgressTransportTruncation['reason']
	): void => {
		if (truncated) return;
		truncated = true;
		const marker: CommandEventContractOverlayProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
