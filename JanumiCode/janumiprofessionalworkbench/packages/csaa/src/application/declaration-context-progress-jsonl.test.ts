...
import {
	createDeclarationContextProgressJsonlWriter,
	DECLARATION_CONTEXT_PROGRESS_TRANSPORT_SCHEMA_VERSION
} from './declaration-context-progress-jsonl.js';

function event(detailCode: string): DeclarationContextReportProgressEvent {
	return {
		...
		observations: [],
		operationVersion: 'jan-csaa-report-declaration-context/0.1.0',
		phase: 'REQUEST_BIND',
		protocolRole: 'PRELIMINARY_CAP_013_REPORT_TELEMETRY',
		reportIdentityEffect: 'EXCLUDED_FROM_REPORT_IDENTITY',
		schemaVersion: 'jan-csaa-declaration-context-report-progress/0.1.0',
		sequence: 1,
		stage: 'REQUEST',
		state: 'STARTED',
...
describe('declaration-context progress JSONL transport', () => {
	it('reserves one bounded marker when the event limit is reached', () => {
		const lines: string[] = [];
		const writer = createDeclarationContextProgressJsonlWriter({
			maxBytes: 65_536,
			maxEvents: 3,
	...
	it('uses a bounded marker for byte exhaustion and unserializable events', () => {
		...
		const byteWriter = createDeclarationContextProgressJsonlWriter({
			maxBytes: 32_768,
			maxEvents: 10,
		...
		const serializationWriter = createDeclarationContextProgressJsonlWriter({
	...
	it('isolates throwing, rejecting, and backpressured sinks', async () => {
		const throwing = createDeclarationContextProgressJsonlWriter({
			write: () => {
		...
		const backpressured = createDeclarationContextProgressJsonlWriter({
			write: (line) => {
		...
		const rejecting = createDeclarationContextProgressJsonlWriter({
