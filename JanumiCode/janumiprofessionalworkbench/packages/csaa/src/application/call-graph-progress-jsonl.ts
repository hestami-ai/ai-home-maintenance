...
export const CALL_GRAPH_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-call-graph-progress-transport/0.1.0' as const;
export const CALL_GRAPH_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const CALL_GRAPH_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 16 * 1024;

export interface CallGraphProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_TYPESCRIPT_CALL_GRAPH_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof CALL_GRAPH_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface CallGraphProgressJsonlWriter {
	emit(event: CallGraphReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface CallGraphProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createCallGraphProgressJsonlWriter(
	options: CallGraphProgressJsonlWriterOptions
): CallGraphProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		CALL_GRAPH_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, CALL_GRAPH_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
	...
	const truncate = (reason: CallGraphProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: CallGraphProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
