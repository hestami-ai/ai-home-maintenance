...
export const PROJECT_CONTEXT_PROGRESS_TRANSPORT_SCHEMA_VERSION =
	'jan-csaa-project-context-progress-transport/0.1.0' as const;
export const PROJECT_CONTEXT_PROGRESS_MAX_BYTES = 8 * 1024 * 1024;
export const PROJECT_CONTEXT_PROGRESS_MAX_EVENTS = 2_048;

const TRUNCATION_MARKER_RESERVE_BYTES = 2_048;

export interface ProjectContextProgressTransportTruncation {
	...
	readonly protocolRole: 'PRELIMINARY_CAP_010_REPORT_TELEMETRY_TRANSPORT';
	readonly reason: 'BACKPRESSURE' | 'BYTE_LIMIT' | 'EVENT_LIMIT' | 'SERIALIZATION_FAILED';
	readonly schemaVersion: typeof PROJECT_CONTEXT_PROGRESS_TRANSPORT_SCHEMA_VERSION;
	readonly state: 'TRUNCATED';
}

export interface ProjectContextProgressJsonlWriter {
	emit(event: ProjectContextReportProgressEvent): void;
	stats(): {
		readonly emittedBytes: number;
		readonly emittedEvents: number;
...
export interface ProjectContextProgressJsonlWriterOptions {
	readonly maxBytes?: number;
	readonly maxEvents?: number;
...
export function createProjectContextProgressJsonlWriter(
	options: ProjectContextProgressJsonlWriterOptions
): ProjectContextProgressJsonlWriter {
	const maxBytes = boundedLimit(
		options.maxBytes,
		PROJECT_CONTEXT_PROGRESS_MAX_BYTES,
		TRUNCATION_MARKER_RESERVE_BYTES * 2
	);
	const maxEvents = boundedLimit(options.maxEvents, PROJECT_CONTEXT_PROGRESS_MAX_EVENTS, 2);
	let emittedBytes = 0;
	let emittedEvents = 0;
	let truncated = false;
	...
	const truncate = (reason: ProjectContextProgressTransportTruncation['reason']): void => {
		if (truncated) return;
		truncated = true;
		const marker: ProjectContextProgressTransportTruncation = Object.freeze({
			emittedBytes,
			emittedEvents,
