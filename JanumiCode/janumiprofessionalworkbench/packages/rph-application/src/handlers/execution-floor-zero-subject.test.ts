...
describe('JAN-EXECREM WP-11 / F-01 limb B — the zero-subject floor', () => {
	...
	it('an AI step that ships content while naming NOTHING is REFUSED', () => {
		...
		expect(r.status, 'this exact input returned ACCEPTED | state=SUCCEEDED before WP-11').toBe('REJECTED');
		...
		expect(succeededEvents(), 'the unassessed result must not reach the governed stream').toHaveLength(0);
	...
	it('a NON-AI step with zero subjects is untouched by this rule, content or not', () => {
		...
		expect(
			dispatch('SkipExecutionStep', { stepId: AI_STEP, mandatory: false }, PLAN, 'EXECUTION_PLAN', HUMAN).status
		).toBe('ACCEPTED');
		expect(dispatch('StartExecutionStep', { stepId: HUMAN_STEP }, PLAN, 'EXECUTION_PLAN', HUMAN).status).toBe(
			'ACCEPTED'
		);
		const r = dispatch(
			'CompleteExecutionStep',
