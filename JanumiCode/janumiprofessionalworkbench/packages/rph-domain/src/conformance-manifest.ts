...
const COVERAGE_BY_PREFIX: Readonly<Record<string, Coverage>> = {
	...
	'RPH-FIX': {
		...
		note: 'FIVE OF SIX ARE ASSERTED BY NAME, across TWO oracles because the rules are of two kinds. FOUR are properties of the §26 event SEQUENCE, in `runConformance` (packages/rph-engine/src/replay.ts), green in replay.test.ts: FIX-001 (contiguous seq — reference integrity), FIX-002 (every one of the 72 trace events resolves to a registered EVENTS contract), FIX-003a/b/c (both baselines promoted; the trace ends AUTHORITATIVE + PwuBaselined; a Behavior PWU SATISFIED but not baselined), FIX-006 (the deferred offline capability stays represented as a refined intent constraint). FIX-004 is a property of the OBJECT GRAPH and is asserted in `runGraphConformance` (graph-conformance.ts) over the store `driveReferenceUndertaking` builds — every MANDATORY obligation sourced from the Architecture PWU is allocated to a child of its decomposition, or explicitly retained (DOC-003 §6 DEC-3 permits retention, and a bare "all allocated" reading would wrongly refuse it). IT WOULD HAVE PASSED VACUOUSLY: every PWU was proposed with `obligationIds: []`, so the drive now AUTHORS the three obligations corpus §25 Test 3 names as traceability targets, and the check treats an empty population as a FAILURE rather than a pass. GENUINELY OUTSTANDING — FIX-005 alone (Multi-Tenancy Constraint -> Architecture PWU -> Tenant Isolation Artifact -> Claim -> Assessment). It needs the drive to author that chain; all four references exist in the ratified model (`constraintIds`, `producingPwuId`, `subjectObjectIds`, the assessment the drive already runs), so it is authoring, not a missing capability.'
	},
	'RPH-E2E': {
		status: 'DEFERRED',
