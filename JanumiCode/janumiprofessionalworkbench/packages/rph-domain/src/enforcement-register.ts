...
export const ENFORCEMENT_REGISTER: Readonly<Record<RegisteredRuleId, EnforcementDisposition>> = {
	...
	'RPH-GOV-005': {
		...
		enforcedAt:
			'⚠ MOVED 2026-08-20 (REG-F-202). This rule USED to be enforced INVERSELY, through the floor gate: no handler refused "this waiver is out of scope"; `waiverDischargesFloorPolicy` simply failed to discharge and `pwaFloorGate` then refused for a floor reason. ASR-3 makes the de minimis floor UNCONDITIONAL, so that whole discharge apparatus was deleted and the inverse enforcement went with it. THE RULE IS NOT WEAKER — it moved to a DIRECT refusal, which is stronger than an inverse one. DECISION: packages/rph-application/src/handlers/waiver-authorization.ts — resolveWaiverAuthorization, which refuses in its own voice on decisionType (an APPROVAL is not a WAIVER), on OBJECT (a waiver naming another object does not authorize this one) and on the VERSION pin. THE REFUSAL ITSELF is rejectUnauthorizedWaiver in packages/rph-application/src/handlers/pwu.ts, reached from ChangePwuState when the cited decision does not authorize the waive. The kernel predicate waiverCovers (packages/rph-domain/src/governance.ts) no longer has a production caller and is proven only by its own unit test — recorded rather than left implicit.',
		refusalCode: 'RPH_INVARIANT_VIOLATION',
	...
	'RPH-GOV-003': {
		...
		declaredMutations: [
			...
			'~~THE INTEGRITY CAVEAT, and it is REG-F-014\'s second instance: the versions this arm compares against are CALLER-SUPPLIED at approval time. `proposeDecision` PINS them from the store, and `approveDecision`\'s extraMutate then OVERWRITES that pin with the payload. So the binding this rule enforces is exact against a number the approver chose. The refusal is real; what it is exact ABOUT is not verified.~~ CLOSED 2026-08-03 (REG-F-017), struck rather than deleted. The pin is now IMMUTABLE — set at proposal from the store, never written by approval — so this arm compares against the versions the approver actually REVIEWED. An approval STATING anything else is refused.',
			'THIS ROW\'S OWN PROBE WAS BUILT ON THAT DEFECT, AND IS NOT ANY MORE (REG-F-017, filed and fixed 2026-08-03). `promotionProbe(\'stale-version\')` used to make the decision bind v2 while the subject sat at v1 by having the approver STATE v2 — the caller-supplied-fact bug, used as a fixture device. Measured at the time: of 132 ApproveDecision dispatches, 13 diverged from the store, and 2 of those were this probe and its sibling test.',
