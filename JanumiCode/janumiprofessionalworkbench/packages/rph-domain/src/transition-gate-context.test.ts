...
/** A BRANCH fan-out of `width` guarded arms off one settled source — the shape whose cost F-34 measured. */
function fanOutPlan(width: number): GatePlan {
	const steps: GateStep[] = [step('s0', 'SUCCEEDED', { stepType: 'BRANCH' })];
...
describe('JAN-EXECREM WP-2 — the guard evaluator is called at most once per guarded edge (F-34)', () => {
	...
	it('scales LINEARLY in the fan-out width, not quadratically (the shape of the defect, not a timing)', () => {
		...
		startableStepIds(fanOutPlan(10), narrow.evaluate);
		startableStepIds(fanOutPlan(40), wide.evaluate);
...
describe('JAN-EXECREM WP-2 — the seam is semantically inert', () => {
	...
	it('inEdgeDisposition agrees edge-by-edge with the barrier the gate actually uses', () => {
		const plan = fanOutPlan(4);
		const guard = guardTrueFor('t2'); // arm 2 wins first-match; every other arm is neutralized
		for (const t of plan.transitions ?? []) {
			...
			if (d === 'SATISFIED') expect(startable, `${target} via ${t.id}`).toBe(true);
			if (d === 'NEUTRALIZED') expect(startable, `${target} via ${t.id}`).toBe(false);
		}
	});

	it('BRANCH first-match still follows AUTHORED edge order (the index preserves it)', () => {
		const plan = fanOutPlan(3);
		...
		expect(startableStepIds(plan, guard)).toEqual(['s2']);
	});
