...
import { createProjectContextProgressJsonlWriter } from '../packages/csaa/src/application/project-context-progress-jsonl.js';
import {
	projectContextReportExitCode,
	runProjectContextReport
} from '../packages/csaa/src/application/run-project-context-report.js';
...
try {
	...
	const progressWriter = createProjectContextProgressJsonlWriter({
	...
	process.exitCode = projectContextReportExitCode(outcome);
} catch (error) {
