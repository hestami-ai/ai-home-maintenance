...
**Change authority and procedure:** `REG-D-018` permits authors to revise this Draft. Proposed eligibility is governed by `CSAA-002-REQ-098` and `CSAA-002-REQ-474`; affected re-review of a material Proposed change is governed by `CSAA-002-REQ-099`. Normative status or a material Normative change requires an exact-version/digest `JPWB-REG-005` sponsor act

**Review and evidence companions:** Requirement ledger above; revision-bound examples are grounded by `JAN-CSAA-005`; [Wave 1 Manifest Gap Record](<records/JAN-CSAA-W1 - Draft Authoring Initiation and Manifest Gap Record.md>); self-review and independent-review records are not yet authored
...
Analyzer ≠ Analysis Rule Profile ≠ Assurance Policy or Profile
Analyzer execution ≠ assurance-policy satisfaction ≠ Assessment
Analyzer output ≠ semantic authority ≠ canonical Evidence ≠ Decision
...
A Static Semantic Snapshot Identity identifies source/configuration-derived facts. An Execution Evidence Set Identity identifies append-only test, coverage, build, or trace observations correlated to a static snapshot. New execution evidence creates a successor evidence set, not a mutation of historical static facts.
...
## 7. Artifact, contract, build, and source-origin model

### 7.1 Artifact classifications
...
A Program/Source File context is one source occurrence interpreted under one TypeScript Project. The same bytes analyzed under different configurations, compiler versions, conditional exports, or resolver environments are distinct semantic contexts.

### 8.2 Syntax

AST Nodes have a syntax kind, parent, ordered child position, source range, source artifact occurrence, project context, parser/toolchain, and parse health. Cross-revision node matching is a lineage inference, not stable node identity.
...
| Import/export graph | Occurrences, module requests, resolved targets, re-exports, dynamic imports | Resolver conditions and unresolved outcomes retained |
| Package/dependency graph | Packages, manifests, lockfile components, declared/resolved/observed relations | Edge kinds never collapsed |
...
| Data-flow/taint graph | Definition/use, assignment, call/return, property access, capture, source/transform/sanitizer/sink | Bounded analysis never called complete information flow |
...
Runtime Build Identity, Runtime Environment Identity, Workload Selection, Execution, Trace Schema, Trace Collector, Trace, Span, and Event are distinct.
...
The catalog contains 125 objects. Each row inherits every §4.1 profile facet, including required and optional metadata, authority/epistemic limits, all four explicit state treatments, serialization ownership, and invariants. Its supplement adds mandatory item-specific identity/metadata and a narrower limit.
...
| Advisory | Workspace/components | `O-REF` | Advisory source, advisory ID/version, affected-version expression, observation cutoff | External source retains advisory authority |
...
| Authored Source Artifact | Artifacts | `O-ART` | Snapshot path/URI, content digest, authored classification and language/framework role | Generated content is never silently classified as authored |
| Generated Artifact | Artifacts | `O-ART` | Output occurrence/digest, generator/version, inputs, configuration, invocation/basis and origin mapping | `INV-006` and `INV-012` apply |
| Virtual Artifact | Artifacts | `O-ART` | Tool-scoped virtual URI/ID, content identity, producer/version, authored origin and mapping | Non-materialization remains explicit |
| Declaration Artifact | Artifacts | `O-ART` | Artifact occurrence/digest, declaration role, producer/origin and consumer project | Declaration authority and provenance are explicit |
| Configuration Artifact | Artifacts | `O-ART` | Artifact occurrence/digest, configuration kind, consumer and extension context | Presence does not prove effective use |
| Test Artifact | Artifacts | `O-ART` | Artifact occurrence/digest, test framework/role and target context | Test source is not a Test Run |
| External/Vendor Artifact | Artifacts | `O-ART` | External source/package coordinates, artifact path/digest, licensing/source provenance | External classification and trust boundary remain explicit |
...
| Generated Contract Artifact | Contracts/builds | `O-ART` | Artifact digest, governing source/version, generator/version, generation basis and fidelity evidence | Generated output is a bound derivative, not second authority |
...
| Program/Source File Context | Syntax | `O-SEM` | Program/checker identity, Source Artifact occurrence, language mode and parse context | Same file under another project is another context |
...
| Source Location | Syntax | `O-SEM` | Artifact occurrence, encoding, range, authored/generated/emitted role and mapping | Location role and mapping quality remain explicit |
...
| Inferred Runtime Dependency | Dependencies/calls | `O-REL` | Source/Execution Artifact, candidate component/module, inference method/input and limitations | Never presented as observed or compiler-confirmed |
| Observed Runtime Load | Dependencies/calls | `O-REL` | Execution, loader event, loaded module/component/artifact and observation time | Captured load does not establish all possible loads |
...
| Test Run | Tests | `O-EXE` | Static subject, Test Target, runner/config/version, environment, Test Selection, time boundary | Run aggregates but does not erase attempts |
...
| Coverage Observation | Coverage | `O-EXE` | Execution/Test Run, instrumentation, target build, selection/workload, region, denominator, granularity, metric and provider | `INV-004` applies |
...
| Execution | Runtime | `O-EXE` | Runtime Build Identity, Runtime Environment Identity, Workload Selection/input, time boundary and Instrumentation Configuration | Required binding closes the former `REQ-075` gap |
...
| Trace Collector | Runtime | `O-EXE` | Collector identity/version, configuration, clock/context behavior and target Execution | Collector limits bound trace claims |
| Trace | Runtime | `O-EXE` | Execution, trace ID within collector/schema context, time boundary, root and completeness limits | Captured trace is not whole-program behavior |
...
| Event | Runtime | `O-EXE` | Trace/Span or Execution, event identity/ordinal, time and schema-typed payload reference | Event occurrence alone proves only the captured event |
...
| Query Result Binding | Analysis | `O-ANA` | Query reference/version, exact semantic snapshot, optional execution evidence set, execution identity and result coverage | Result never floats across snapshots |
...
| `REL-015` | package-containment | Workspace/package | `R-STR` | Package → contained Artifact/Project; containment kind | Not filesystem proximity alone |
...
| `REL-017` | package-export | Workspace/package | `R-RES` | Package Manifest/export key/conditions → target Artifact | Conditions retained |
...
| `REL-024` | project-include | Project/config | `R-RES` | TypeScript Project/Variant → included Artifact; rule/basis | Effective file set |
| `REL-025` | project-exclude | Project/config | `R-RES` | TypeScript Project/Variant → excluded Artifact/pattern; rule/basis | Exclusion not absence |
...
| `REL-028` | artifact-classification | Artifact/origin | `R-STR` | artifact occurrence → authored/generated/virtual/declaration/config/test/external class | `INV-012` |
| `REL-029` | generator-input | Artifact/origin | `R-MAP` | Generated Artifact/generation → input Artifact; input role | Input version retained |
| `REL-030` | generation | Artifact/origin | `R-MAP` | generator/configuration/invocation → Generated Artifact | Producer and basis required |
| `REL-031` | virtual-origin | Artifact/origin | `R-MAP` | Virtual Artifact → authored/generated origin | Mapping quality explicit |
| `REL-032` | emission | Artifact/origin | `R-MAP` | source/module/build execution → emitted Build Artifact/module | Source and emitted distinct |
| `REL-033` | bundling | Artifact/origin | `R-MAP` | input artifacts/modules → bundled Build Artifact | Many-to-one mapping retained |
| `REL-034` | source-map-mapping | Artifact/origin | `R-MAP` | generated/emitted range → authored source range; Source Map/quality | `INV-006` |
...
| `REL-056` | import-occurrence | Module | `R-RES` | importing Module/Artifact → Import Occurrence | Syntax occurrence only |
| `REL-057` | export-occurrence | Module | `R-RES` | exporting Module/Artifact → Export Occurrence | Syntax occurrence only |
...
| `REL-061` | resolved-module-target | Module | `R-RES` | importer + specifier → target Module/Artifact | `INV-010` |
...
| `REL-066` | observed-runtime-load | Dependency | `R-EXE` | Execution → loaded module/component/artifact | Captured path only |
...
| `REL-070` | observed-call-target | Call | `R-EXE` | Call Site/runtime mapping → observed Callable Target; Execution | Observed execution only |
...
| `REL-092` | build-execution-artifact | Build | `R-MAP` | Build Execution → Build/Execution Artifact | Output digest required |
...
| `REL-096` | test-run-subject | Test | `R-EXE` | Test Run → Repository/Static Semantic Snapshot | Exact subject |
| `REL-097` | test-run-build | Test | `R-EXE` | Test Run → Test Target/Build/Execution Artifact | Target mode retained |
...
| `REL-103` | coverage-observation-run | Coverage | `R-EXE` | Coverage Observation → Test Run/Execution | `INV-004` |
...
| `REL-105` | coverage-build | Coverage | `R-EXE` | Coverage Observation → target Build/Execution Artifact | Exact target |
...
| `REL-110` | trace-execution | Trace | `R-EXE` | Trace → Execution | `INV-005` |
| `REL-111` | trace-build | Trace | `R-EXE` | Trace → Runtime Build Identity/Execution Artifact | Exact build |
...
| `CSAA-002-REQ-025` | An Advisory observation SHALL bind component identity. |
| `CSAA-002-REQ-186` | An Advisory observation SHALL bind component version. |
| `CSAA-002-REQ-187` | An Advisory observation SHALL bind advisory source. |
| `CSAA-002-REQ-188` | An Advisory observation SHALL bind advisory version. |
| `CSAA-002-REQ-189` | An Advisory observation SHALL bind observation time. |
| `CSAA-002-REQ-190` | An Advisory observation SHALL bind correlation method. |
...
| `CSAA-002-REQ-193` | A Vulnerability Observation SHALL bind advisory source. |
...
| `CSAA-002-REQ-253` | Program context SHALL be defined. |
...
| `CSAA-002-REQ-073` | A Coverage Observation SHALL bind its execution. |
...
| `CSAA-002-REQ-075` | A Runtime Execution SHALL bind its build. |
| `CSAA-002-REQ-380` | A Runtime Execution SHALL bind its environment. |
| `CSAA-002-REQ-381` | A Runtime Execution SHALL bind its input. |
| `CSAA-002-REQ-382` | A Runtime Execution SHALL bind its workload. |
| `CSAA-002-REQ-383` | A Runtime Execution SHALL bind its time boundary. |
| `CSAA-002-REQ-384` | A Runtime Execution SHALL bind its instrumentation. |
| `CSAA-002-REQ-076` | A Trace SHALL bind its Trace Schema. |
| `CSAA-002-REQ-385` | A Trace SHALL bind its Trace Collector. |
| `CSAA-002-REQ-386` | A Trace SHALL bind its execution. |
| `CSAA-002-REQ-387` | A Trace SHALL bind its environment. |
| `CSAA-002-REQ-388` | A Trace SHALL bind its workload. |
...
| `CSAA-002-REQ-392` | A Span SHALL bind its execution. |
| `CSAA-002-REQ-393` | A Span SHALL bind its environment. |
| `CSAA-002-REQ-394` | A Span SHALL bind its workload. |
...
| `CSAA-002-REQ-398` | An Event SHALL bind its execution. |
| `CSAA-002-REQ-399` | An Event SHALL bind its environment. |
| `CSAA-002-REQ-400` | An Event SHALL bind its workload. |
...
| `CSAA-002-REQ-078` | A test addition SHALL create an append-only Execution Evidence Set revision. |
| `CSAA-002-REQ-404` | A coverage addition SHALL create an append-only Execution Evidence Set revision. |
| `CSAA-002-REQ-405` | A trace addition SHALL create an append-only Execution Evidence Set revision. |
...
| `CSAA-002-REQ-433` | Finding history SHALL remain append-oriented. |
| `CSAA-002-REQ-434` | Disposition history SHALL remain append-oriented. |
| `CSAA-002-REQ-435` | Suppression history SHALL remain append-oriented. |
...
| `CSAA-002-REQ-094` | Analyzer output SHALL NOT exercise governance. |
...
| `CSAA-002-REQ-128` | Analyzer output SHALL NOT mutate source. |
| `CSAA-002-REQ-129` | Analyzer output SHALL NOT become an independent source of authority. |

### 18.9 Cross-graph closure

| Requirement ID | Atomic requirement |
| --- | --- |
| `CSAA-002-REQ-100` | No semantic fact SHALL exist without subject identity. |
| `CSAA-002-REQ-455` | No semantic fact SHALL exist without provenance. |
| `CSAA-002-REQ-101` | No cross-revision edge SHALL be presented as a same-revision fact. |
| `CSAA-002-REQ-102` | No provider-normalized fact SHALL lose its raw-provider trace. |
| `CSAA-002-REQ-103` | No coverage observation SHALL detach from its execution artifact. |
| `CSAA-002-REQ-456` | No coverage observation SHALL detach from its instrumentation. |
| `CSAA-002-REQ-457` | No coverage observation SHALL detach from its denominator. |
| `CSAA-002-REQ-458` | No coverage observation SHALL detach from its granularity. |
| `CSAA-002-REQ-459` | No coverage observation SHALL detach from its selection. |
| `CSAA-002-REQ-104` | No runtime observation SHALL detach from its execution artifact. |
| `CSAA-002-REQ-460` | No runtime observation SHALL detach from its trace schema. |
| `CSAA-002-REQ-461` | No runtime observation SHALL detach from its trace collector. |
| `CSAA-002-REQ-462` | No runtime observation SHALL detach from its environment. |
| `CSAA-002-REQ-463` | No runtime observation SHALL detach from its workload. |
| `CSAA-002-REQ-464` | No runtime observation SHALL detach from its time boundary. |
| `CSAA-002-REQ-105` | No transformed-code coverage mapping SHALL exist without source-origin mapping. |
| `CSAA-002-REQ-465` | No transformed-code trace mapping SHALL exist without source-origin mapping. |
| `CSAA-002-REQ-106` | No inferred edge SHALL be presented as compiler-confirmed. |
| `CSAA-002-REQ-107` | No graph-completeness claim SHALL exist without a declared coverage basis. |
| `CSAA-002-REQ-108` | No finding-state change SHALL erase prior Engineering Evidence Records. |
| `CSAA-002-REQ-109` | No resolved module edge SHALL omit resolver context. |
| `CSAA-002-REQ-466` | No resolved module edge SHALL omit target artifact class. |
| `CSAA-002-REQ-110` | No architecture-rule conformance claim SHALL omit recognized authority. |
| `CSAA-002-REQ-467` | No architecture-rule conformance claim SHALL omit authority version. |
| `CSAA-002-REQ-468` | No architecture-rule conformance claim SHALL omit scope. |
| `CSAA-002-REQ-469` | No architecture-rule conformance claim SHALL omit an applicable exception reference or an explicit no-exception state. |
| `CSAA-002-REQ-111` | No generated node SHALL masquerade as authored. |
| `CSAA-002-REQ-470` | No virtual node SHALL masquerade as authored. |
| `CSAA-002-REQ-112` | No combined static-and-dynamic evidence SHALL omit Static Semantic Snapshot Identity. |
| `CSAA-002-REQ-471` | No combined static-and-dynamic evidence SHALL omit Execution Evidence Set Identity. |
...
| `CSAA-002-REQ-099` | A material Proposed change SHALL receive affected re-review. |
...
| `CSAA-002-REQ-540` | Analyzer SHALL remain distinct from Analysis Rule Profile. |
| `CSAA-002-REQ-541` | Analysis Rule Profile SHALL remain distinct from Assurance Policy or Profile. |
| `CSAA-002-REQ-542` | Analyzer execution SHALL remain distinct from assurance-policy satisfaction. |
| `CSAA-002-REQ-543` | Assurance-policy satisfaction SHALL remain distinct from Assessment. |
| `CSAA-002-REQ-544` | Analyzer output SHALL remain distinct from semantic authority. |
...
| --- | --- | --- |
| Object-catalog coverage | Mechanical count and one-to-one comparison of 125 §14 rows to `JAN-CSAA-000` §10.2 | `JAN-CSAA-008` may enforce schema coverage later |
...
| Revision and project identity | Adversarial examples using current inventory | Fixture/contracts/tests later `006`–`008` |
...
| No-false-green and epistemic dimensions | Adversarial review | `JAN-CSAA-008` |
| Invalidation and history | Scenario review | `JAN-CSAA-008`/`009` |

No later-wave artifact is created by this allocation.

---

## 20. Revision-bound JPWB examples

[JAN-CSAA-005@0.1.0](<JAN-CSAA-005 - JPWB TypeScript Repository Semantic Inventory and Conformance Mapping.md>) records facts for the exact historical subject bound to parent commit `e673fb5c2e186fb0873d3720036e5e8d7b00038a`, observation time `2026-07-26T11:52:31.6050250-04:00`, and [JAN-CSAA-005-EVIDENCE-001@0.1.0](<records/JAN-CSAA-005 - Preparation Evidence Snapshot.md>). It is non-authoritative and `STALE_FOR_CURRENT_REPOSITORY`. These examples are informative, do not establish current repository facts, and are used only to test the model:
...
- 125-object and 137-relationship catalogs with explicit complete semantic-profile mappings;
- epistemic dimensions, lifecycle, invalidation, and fourteen cross-graph invariants;
- 549-row independently dispositionable stable candidate requirement catalog comprising 119 retained prior IDs below 131 and 430 continuous, non-reused new IDs from `CSAA-002-REQ-131` through `CSAA-002-REQ-560`, with later verification allocations;
- open-fork safe defaults.

Remaining before Proposed:
