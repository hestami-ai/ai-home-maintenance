...
- Item 9 (re-derivation at the point of writing): NOT a corpus distillation — merged from REG-005 REG-F-278, whose measured basis is three figures wrong in two days, each produced inside an agent's synthesis and restated without being re-derived. ⚠ Distinct from item 8 and NOT an amendment to it: item 8 guards DECAY (true when written, false later) and its remedy — a dated reading — applied to these failures yields a DATED FALSE FIGURE; item 9 guards TRANSCRIPTION (false when written). Disclosed widening: REG-F-278 says "written into canon"; the item binds the wider surface §10's preamble already governs.
- Item 10 (an annotation never moves the lines it annotates): NOT a corpus distillation — merged from REG-005 REG-F-277 as refined by REG-F-281. Measured basis: annotating an append-only artifact by INSERTING lines shifts every line citation below the insertion, permanently, because the artifact cannot correct them; extending the bullet's existing last line adds no line. Two constraints travel with it, from scripts/tracker/ingest.ts:109, whose status regex runs to END OF LINE.
