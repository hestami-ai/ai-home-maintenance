...
version: 1.5.0 — B5 amendments: REG-D-011 (requirement ledger); REG-D-012 (tests-of-the-tests, §7.6 oracle integrity, changed-module grain); REG-D-014 (Cycle-000 batch under delegated authority: §9.1 SPEC-gap trigger, §9.2 PROPOSED REFINEMENT type, §8.1 tiebreak scope, §7.4 interim mutation default, §7.6 judgment grain + proposed-oracle status + citation fix, §2.1 working-references load rung); REG-F-273/274/275 (§10 item 8: mutable-value citation discipline — merged under the sponsor's election of DOC-004 over REG-005 §1, following the REG-D-014 delegated-authority recording idiom); REG-F-277/278/281 (§10 item 9: re-derivation at the point of writing; §10 item 10: annotation form — an annotation must not move existing line numbers)
date: 2026-07-16
governs: |
...
When the task implements or modifies governed ground, the agent additionally loads **the governing JPWB-SPEC-nnn deep reference specification for that work area** before implementing. The canon is the overview tier; the SPEC carries the enumerated obligation surface — per-field contracts, guards, transitions, error semantics, conformance fixtures — that the implementation will be diffed against. Implementation rigor tracks the visible obligation surface: an enumerated specification converts economy into countable omission. Accordingly, **the absence of a SPEC for governed ground is a finding to file (§9), never license for economy** — the agent does not substitute its own minimal reading of the overview for a missing specification; it proposes a spec commission or escalates, and proceeds only on ground a SPEC or the canon actually covers. Non-example: ungoverned ground — scratch tooling, program-internal scripts, presentation code with no semantic authority — needs no SPEC; this rule governs work on canonical professional-work semantics.
...
8. **Never assert a mutable value as a standing fact.** Item 5 forbids restating another artifact's content; this is its state-level form. The object is any property a later act can change — a ledger row's score, arm, or filing status; a count or census figure; a status word; a claim about what an artifact does or does not yet contain — and the scope is any artifact, **including the one the text is being written into**, whose own state is exactly as perishable as a ledger row's. Cite what a later revision cannot move: the **mechanism, and the file and field that carry it** — *"a different mechanism: the dead type table"* — never the bookkeeping — *"is scored `UNENFORCED_DEAD_PREDICATE`"*. An id alone does not discharge this where the record it names can itself be superseded; name the id **and** the mechanism. Where a mutable value is genuinely load-bearing for the distinction being drawn, it is citable only as a **dated reading of the record that owns it** — *"`filing: NEAR_MISS` as at `<commit>` (`invariant-verdicts.ndjson`)"* — so that a later reader meets a measurement to re-check rather than a property to trust; the present perfect (*"has never been…"*) is the tense in which a missing date hides. This is DOC-003 PER-11's occurrence/record distinction applied to prose. The obligation binds hardest where a correction can only be appended — a register entry, a domain event, a comment shipped in a ratified artifact — because there the record that owns the value stays reconcilable and the copy never becomes so, and nothing will ever true the two up. **Why, and it is measured rather than assumed:** the correcting hand reaches bookkeeping and leaves substance to rot — one register entry's status line was rewritten in place twice in a single day, by the commit that authored a false census figure alongside it, and that figure was still standing nineteen days later; and an audit record is read *at revisions*, where §7.1's remedy — repair the stale text in the same change that moves the thing described — reaches no reader who arrived before the repair. **Non-example:** quoting is not asserting. Reproducing another record's clause in order to correct or examine it — which is how a superseding entry states what it supersedes — asserts nothing of its own, and neither does reporting what a record said at a stated reading. This rule reaches the sentence that *relies* on a mutable value, not the one that *examines* it; an agent that responds by citing no records at all has over-applied it in the §11.3 manner. (Merged from REG-F-273, as corrected by REG-F-274 and widened by REG-F-275.)
9. **Re-derive a figure at the point of writing, or do not write it.** Item 8 governs how a mutable value may be *cited*; this governs where a figure came from before the sentence carrying it was written, and it reaches figures no later act can change — a count over a closed corpus, a date range over a fixed set of commits, a withdrawn denominator. The object is any count, range, date, ordinal, or population size that will stand in normative text the agent authors, and the scope is the act of writing it down: the figure is derived again at that moment, from the primary artifact that owns it, by the hand that writes it. A figure arriving from another agent's synthesis, from an orchestrator's relay, from a handoff summary, or from this agent's own prose one revision ago is a **relay**, and a relay is derived again before it is written or it is not written. Where the derivation was not performed, the boundary is stated — the figure is marked as not re-derived rather than reprinted as though it had been — and where two derivations disagree, what is published is the range and the rule that splits them, never a chosen number. **Item 8's remedy does not detect this defect:** a dated reading tells a reader *when* to re-check a value, and dating a figure that was already false when it was measured yields a dated false figure wearing the appearance of a checked one. This is DOC-003 ASR-2's re-derivation gate — *the service re-derives; it never rubber-stamps* — turned on the agent's own prose. **Why, and it is measured rather than assumed:** figures carried into this canon have gone wrong in one repeating form — produced inside an agent's synthesis, restated downstream, and never derived again — and each was caught by re-deriving from the primary artifact, none by re-reading the prose; `REG-F-278` owns those instances and the tally of them, which is why no tally is restated here. **Non-example:** quoting is not deriving. Reproducing an earlier figure in order to correct or examine it — which is how a superseding entry states what it supersedes — and reporting what a record said at a stated reading produce no figure of their own and need none. This binds the figure written into normative text, not the figures an agent derives to navigate, triage, or form a hypothesis, and not the correctness of a derivation once performed: a wrong derivation is an ordinary defect, not a breach of this item. An agent that answers it by publishing no figures at all has over-applied it in the §11.3 manner. (Merged from REG-F-278.)
10. **An annotation never moves the lines it annotates.** Item 5 routes content to the artifact that owns it; this governs where a note is attached once it is in the right artifact. The object is any text added alongside text that already exists — a supersession pointer, a correction marker, a dated re-reading, a still-owed note — and the scope is any artifact whose existing lines are cited by line number and which corrects only by appending, so that neither the lines that move nor the citations recorded inside it can be trued up afterwards: the append-only record CON-000 AX-7 requires, above all, and equally a ratified artifact or a data row a test anchors on. **Extend the last existing line of the passage being annotated where that line can carry the note; use an appendix at the end of the artifact only where it cannot.** Inserting a line shifts every line citation below it, and where correction is append-only that staleness is permanent; extending a line shifts nothing. Three things send a note to the appendix instead, and each is a property of the note or of the line rather than a preference. **First, legibility** — a line already long enough that extending it would bury the note is a line that cannot carry it. **Second, the consumers that parse the line**: an extension may not be attached to a line carrying a field a consumer reads to the end of that line, because the note is then swallowed into that field's value, and the note introduces no status token of its own, because an entry carries exactly one live status and a second makes its current status a matter of interpretation rather than lookup; for JPWB-REG-005 those consumers are `scripts/tracker/ingest.ts` and `verif/register-status.test.ts`, and the field and the exact patterns are theirs, not this document's (item 6). **Third, and it is the limb that is easiest to miss: a note that reproduces the words a destructive edit removed goes to the appendix, never into the entry that lost them** — a content-survival gate treats a lost word reappearing in a baseline-existing entry as evidence the word survived, so co-locating the quotation consumes the very evidence the annotation exists to preserve. **Why, and it is measured rather than assumed:** the insert-a-line form was authored, applied to JPWB-REG-005, driven and reverted, on two independent measurements — it moved a population of live line citations that register can never repair while gaining no content, and it degraded the instrument watching for the edits it was annotating; `REG-F-277` owns both, and `REG-F-281` owns the measurement behind the extension form. **Non-example:** this does not reach a new entry appended after all existing text, which moves nothing and is the ordinary way to file; it does not disfavor the appendix, which is the sanctioned fallback and not a defeat; it does not reach an artifact whose citers can be corrected — a working reference, or a source file no record cites by line; and it does not reach the retire-by-striking convention, whose replacement status must be a new line because a struck line is read as struck in whole, an insertion that carries the same cost and that this item neither licenses nor resolves. Nor is it licence to overwrite: an extension adds to what a line says and never replaces it. (Merged from REG-F-277, as refined by REG-F-281.)
