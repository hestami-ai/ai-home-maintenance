...
- **ALREADY NEUTRALIZED, THOUGH NOT BY THIS ENTRY.** REG-F-012's hashability gate (`command-bus.ts` step 0c) now refuses an uncanonicalizable payload before any handler runs, so the crash is a **classified refusal** as of the same commit. What remains is the divergence itself: the contracts still promise to accept values the engine cannot store, and a caller learns this from a runtime refusal rather than from the schema.
- **Disposition:** Recorded, not fixed, with the cost stated rather than the work quietly deferred. `messages.ts` / `objects.ts` are **generated** from `packages/rph-contracts/vocab/*.json` by `bun run gen`, so the fix is a vocab change plus a regeneration — and the committed generated files are known to be **stale against their generators**, so a regeneration lands a large unrelated diff that must be isolated in its own commit to stay reviewable. Doing it inside a finding-closure commit would bury the change this commit is actually accountable for. It is a contained, mechanical change with a clear gate (`check-types` + the full suite), and it should be its own increment.
- **Merge target:** Repository — `packages/rph-contracts/vocab/*.json` (the number type → an integer-constrained one), then `bun run gen`. The `contentHash` header is unchanged and correct; it stated the requirement all along. **Status:** OPEN — the crash is closed, the contract gap is not.
