...
- **Merge target:** Repository (`command-bus.ts`; `conformance-manifest.ts` for the miscitation). `packages/rph-contracts/src/envelopes.ts` is unchanged — `DomainCommandSchema` is correct; what is missing is anything asking it. **Status:** OPEN — measured and disclosed, remediation unscheduled.
