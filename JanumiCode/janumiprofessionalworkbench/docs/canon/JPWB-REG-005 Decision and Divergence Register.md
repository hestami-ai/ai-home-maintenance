...
- **STILL OPEN — the other four instances of the same shape**, none of which this fix touches: `approveDecision`'s `extraMutate` overwriting the store-pinned `subjectSemanticVersions` with the caller's payload (recorded on `RPH-GOV-003`'s row); `ReshapePwu.triggeringObjectId`; `detectedConflicts`; and `authorityDecisionId`, which is never resolved to a real Decision. The survey that produced the 139-dispatch measurement is reusable for each.
...
- **Disposition:** **Recorded, not fixed** — deliberately, and the reason is the finding itself. A first fix was written, measured, and withdrawn on its own merits; the correct one changes a governance authority path and re-bases four register probes, and it deserves to be done on purpose rather than squeezed onto the end of the survey that found it. **The survey is the reusable part** and is recorded above so the next attempt need not re-derive it.
- **Merge target:** Repository — `handlers/governance.ts` (`approveDecision`'s `extraMutate`), `baseline-stale-decision-version.test.ts`, and `execrem-wp16-enforcement-observed.test.ts`'s `promotionProbe`. Canon is unchanged and correct: DOC-003 ASR-15 states the version binding, and RPH-GOV-003 states the refusal. **Status:** OPEN. This is REG-F-014's second instance; three more stand (`ReshapePwu.triggeringObjectId`, `detectedConflicts`, `authorityDecisionId`).
