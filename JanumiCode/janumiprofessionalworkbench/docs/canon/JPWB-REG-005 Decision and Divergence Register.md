...
- **Merge target:** Repository (`handlers/governance.ts` refusal codes; the affected probes). `packages/rph-contracts/src/errors.ts` is unchanged — the codes are correctly ratified; what is missing is their use. **Status:** OPEN — groups 1 and 2 disposed, group 3 unscheduled.
...
- **Disposition:** Recorded, not fixed. The minimal fix is `proposeDecision` binding `authority` the way `requestWaiver` already does, or refusing when `p.authority.actorId !== command.issuedBy.actorId` without a delegation record — but it is a **behaviour change that will refuse commands the engine accepts today**, so it takes the REG-F-011/REG-F-012 treatment: a caller and fixture survey first, then a red-first test written against the CONTRACT (an agent cannot cause a decision to become EFFECTIVE on an authority it does not hold) rather than against a chosen message. A delegation record, if one is wanted, is a new ratified shape and a sponsor question — canon says authority may be *delegated*, and this repository has no object for that.
- **Merge target:** Repository — `handlers/governance.ts` (`proposeDecision`), plus whatever the provenance-field survey turns up. JPWB-DOC-003 ASR-15 is unchanged and correct: canon already states the rule. **Status:** OPEN.
...
- **THE OPEN PART, and it is the reason this is filed rather than closed.** The repair is one file. What found it was dispositioning the rule that file was named for — so the general question stands: **how many other test files arrange less than they claim, and pass because the null arrangement produces the same refusal as the guarded one?** The detector is cheap and mechanical — a fixture that throws instead of ignoring a dispatch result — and this programme has now shipped **two** distinct instances of evidence-that-cannot-fail (three controls in JAN-VERIF, this file). A sweep for unasserted arranging dispatches across the handler test corpus is the honest follow-up.
- **Merge target:** Repository — done for `floor-waiver-scope.test.ts` and `__tests__/floor-fixtures.ts`; **OPEN** for the corpus-wide sweep for unasserted arranging dispatches. Canon is unchanged and correct: DOC-003 ASR-14 states the rule, and production honours it. **Status:** OPEN (sweep).
