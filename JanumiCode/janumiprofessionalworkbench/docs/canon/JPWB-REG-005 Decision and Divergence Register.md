...
- **Disposition:** ~~Recorded, not fixed.~~ **FIXED 2026-08-04 for two of PER-5's three dimensions.** The prescription below was followed as written and is kept unstruck because it was right.
  ...
  - **TWO OF THREE DIMENSIONS, and the third is named rather than approximated.** `commandType` and `targetAggregateId` are now compared before returning `DUPLICATE`. The third — *same command, same target, different **payload*** — still replays. The receipt's `resultHash` is `contentHash(nextState)`, a hash of the RESULTING OBJECT rather than of the payload, so closing it needs a stored payload hash (a persistence schema change); deriving it from `resultHash` would mean executing the command first, which is precisely what idempotency exists to avoid. **OPEN**, with its cost stated.
...
- **Merge target:** Repository — `command-bus.ts` (the comparison) and `packages/rph-application/src/idempotency-key-reuse.test.ts`. JPWB-DOC-003 §9 PER-5 is unchanged and correct. **Status:** OPEN, NARROWED — the payload dimension owes a stored payload hash.
