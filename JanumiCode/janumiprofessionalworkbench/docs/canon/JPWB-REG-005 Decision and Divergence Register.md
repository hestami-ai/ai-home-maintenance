...
- **THE FIVE INSTANCES, and where each now stands.** The instrumented-survey technique that measured the first was reused on each; it is the reusable part of this entry.
  ...
  4. **`ReshapePwu.triggeringObjectId` — OPEN, unsurveyed.** Same shape: an optional caller-supplied reference written into `PwuReshapingStarted` and never loaded. Lower stakes than the other four — it is provenance on an event rather than a basis for dropping an obligation — but the same remedy applies (resolve it, or refuse an id that names nothing).
