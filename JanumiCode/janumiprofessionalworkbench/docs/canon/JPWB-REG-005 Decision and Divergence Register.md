...
- **Merge target:** Repository (handler + `decomposition-revise-conformance.test.ts` + the vocabulary provenance, all landed); JPWB-DOC-003 DEC-2/3/4 carry the semantic requirement unchanged. **Status:** OPEN AND UNSCHEDULED — B7 component discharged and the event payload now engine-enforced; the DOCS_STRONGER remediation (wire the existing M9 conservation kernel into the revise path) is held by no milestone.
...
- **Disposition:** **Disclosed and gated now; remediation is repository work.** The gap is recorded on the `RPH-EVD-003` row of `packages/rph-domain/src/enforcement-register.ts` with an `OBSERVED_ADMISSION` guard, so it cannot outlive the condition it discloses: wire the missing limb and the observation goes RED and the row must be re-dispositioned as ENFORCED with a refusal probe. Remediation itself needs no ratification decision — canon already states the rule and DOC-003 is OPERATIVE — but it is **not merely a null-check fix**: `EvidenceForAdmissibility` must gain a `producedBy` (or equivalent) field before the predicate can express the obligation, which is REG-F-005's impoverishment pattern one type over.
- **Merge target:** Repository (`assurance-rules.ts` predicate + input type; `evd-disclosure-observed.test.ts` re-dispositioned on fix). JPWB-DOC-003 §7 carries the requirement unchanged. **Status:** OPEN — disclosed, gated, and unscheduled.
