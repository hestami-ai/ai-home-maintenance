...
## Amendment pass 4 (2026-07-24, sponsor-directed: REG-D-011)
DOC-004 v1.1.0: requirement ledger added — §3.3 intake extracts every applicable SHALL/SHALL NOT mapped to site+test (with ungoverned-ground non-example); §6.3 handoff closes the ledger (every entry implemented+verified or dispositioned; completion claimed against the closed ledger, never the diff). REG-005 gained REG-D-011, which also records the Codex methodology-chat transplant into deep-spec commission template v2 as complete (survivorship check satisfied; the chat retires with the corpus). Source: Documentation Generation Methodology Discussion/Initial Chat.md §12.
...
## Amendment pass 7 (2026-07-24, REG-D-015: the sponsor's chain-of-thought ruling in sponsor voice)
DOC-003 v1.2.0: PER-12 rewritten per the sponsor's restatement — retain WHERE AVAILABLE (provider/config-dependent; no procurement obligation); quarantined from every assurance tier; OFFLINE DIAGNOSIS is the sole permitted use; WHY now carries the sponsor's own rationale (trace-conclusion decoupling cuts both ways); operative-default clause retired. REG-005: REG-D-015 records the ruling with its two-pushback history; REG-Q-027 MERGED; REG-E-003 CLOSED; REG-E-001 sponsor evidence recorded (recursively-composable/reductionism concern), thesis amendment pending sponsor confirmation.
