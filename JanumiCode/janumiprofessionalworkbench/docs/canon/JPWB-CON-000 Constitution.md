...
version: 1.3.0 — B5 amendments: REG-D-012 (B2 authority-relative "sponsor"); REG-D-014 (B1 program-working-references class); REG-D-016 (§3 thesis sponsor-confirmed verbatim — decomposable/recomposable pairing, complexity clause, two-axis gloss)
date: 2026-07-16
governs:
...
The Janumi Professional Workbench externalizes professional cognition into explicit, recursively decomposable and recomposable, continuously reconciled, governed representations, so that humans and AI can reason together while intent, evidence, authority, and coherence are preserved — managing professional work as the complex whole it is, not the merely complicated sum of its parts. *(Sponsor-confirmed verbatim, REG-D-016.)*

The pairing is deliberate, asymmetric, and is where this constitution takes its stance on complexity. Complicated work decomposes losslessly — understand every part and the whole is recovered. Complex work fails that test: decompose it, understand every part, recompose it, and something is still missing — that remainder *is* the complexity, and professional work is complex in exactly this sense. Decomposition loses intent, constraints, assumptions, and cross-cutting obligations (§2); every decomposition therefore creates an explicit obligation to recompose the whole (AX-4), and coherence is continuously re-earned, never assembled once. Complexity is also temporal: in a complex system the present state does not explain itself, and futures are credible rather than arbitrary. The canon's history machinery exists to answer the two questions no snapshot can — *how did we get to this point* (the governed stream, AX-7), and *where can we credibly go from here* (Narrative Memory, reconciliation, and the evidence that makes "credibly" mean something).
...
**B1 — The canon.** The recognized corpus is exactly: JPWB-CON-000, JPWB-DOC-001, JPWB-DOC-002, JPWB-DOC-003, JPWB-DOC-004, JPWB-REG-005; the **JPWB-SPEC-nnn series** of deep reference specifications — one per subsystem, each individually ratified per REG-005, layer Semantic Model, settledness HYPOTHESIS, subordinate to JPWB-DOC-003 by concern; **program working references** — design documents, roadmaps, and program-scoped specifications authored under a registered sponsor grant, each grant a REG-005 entry naming scope and standing, holding HYPOTHESIS-grade authority within their program's scope and subordinate to every canon artifact by concern; plus the repository's generated contracts, schemas, migrations, and conformance tests as shape authority. **the SOURCE CORPORA** from which the canon artifacts were distilled — `docs/Recursive Professional Harness/`, `docs/Constitution Discussion/`, and the Coding Agent Guide — holding authority for DETAIL (state values, field semantics, transition matrices, worked scenarios, and the depth canon compresses), subordinate to canon by concern per B3, and never themselves a substitute for a ratifying act. Nothing else governs. A document not in this registry — whatever its title or voice — is historical material. ⚠ AMENDED 2026-08-09 (REG-D-034, sponsor ruling): the source corpora were previously OUTSIDE this registry and therefore historical material. They are admitted because canon is a DISTILLATION of them and measurably drops load-bearing detail — four of six questions escalated to the sponsor as *"canon does not define this"* were answered in these documents (REG-F-093). Admission grants authority for detail ONLY; principle remains canon's by concern. Shape authority means the repository's *reference artifacts* — schemas, generated contracts, migrations, conformance fixtures — never the implementation itself: the implementation is the experiment (B4, level 3) and is never its own shape authority. A shape reference that no type check or conformance test enforces asserts a status nothing performs (B7).

**B2 — Status is conferred, not authored.** An artifact's authority derives from its ratification record in JPWB-REG-005, never from its title, voice, or fluency. Every artifact carries a status block; the block is part of the artifact. Ratification is a sponsor act; an agent's best-judgment resolution is a proposal logged for confirmation, not a conferral. Throughout the canon, *sponsor* denotes the accountable authority under the governing authority model — presently a single human sponsor; in multi-party deployments the same clauses resolve through that deployment's authority model (roles, delegation, separation of duties) without textual change. [ELICITATION: confirm that the M0 build-agent RATIFIED entries are proposals pending sponsor confirmation, not conferrals.]

**B3 — Precedence is by concern.** Vision, values, and first principles → CON-000. Reasoning, judgment, and operation → DOC-001. Naming and meaning of terms → DOC-002. Semantic structure and invariants → DOC-003. Agent conduct and process → DOC-004. Exact shapes → the repository. Open questions and rulings → REG-005. On conflict between artifacts, the artifact that owns the concern controls; on conflict within a concern, the higher settledness class controls; residual conflicts are SEMANTIC_CONFLICT findings (per the JPWB-DOC-004 divergence protocol) filed in REG-005. **Canon's authority over a principle is DERIVATIVE AND CONDITIONAL (REG-D-034, sponsor ruling).** Canon governs where it faithfully distils the source its provenance cites. Where a canon clause DIVERGES from that source, canon governs **only if the divergence carries a ratifying act naming it**; a divergence with no such act makes the canon clause **DEFECTIVE, not governing**, and the source controls until the divergence is ruled. **WHY:** REG-D-010 conferred authority on a *distillation*, and an unfaithful distillation is not the artifact that was ratified. **AND THE DIRECTION IS NOT NEUTRAL** — of the divergences measured in the 87-clause fidelity audit, the ratified one (PER-12 / REG-D-015) was a deliberate amendment, while every unratified one WEAKENED A PROHIBITION (ASR-14, ASR-16). A rule preferring canon on silent conflict would systematically launder constraints away. **Canon's SILENCE on a detail is never evidence of the corpus's silence** (REG-F-093).
...
**B6 — The convergence clause.** During the convergence phase — now, until closure — the canon is the sole semantic authority, and the code is the first experiment being brought into conformance. ⚠ **SCOPE CLARIFIED 2026-08-09 (REG-D-034): "sole semantic authority" is stated AGAINST THE CODE, and does not exclude the B1 source corpora.** They are not a competing authority but the material canon distils; B3 governs the relationship, and B8 permits reading them to verify that distillation. Read otherwise, this clause would re-impose by implication the retirement REG-D-034 reversed. Dual run never means dual semantic authority. The code was written without benefit of this canon; divergence is expected and is evidence, not scandal. The docs-win presumption is a property of this phase, not a permanent fact; settledness is thereafter earned bottom-up through real-world operation. Convergence closes subsystem by subsystem: a subsystem exits convergence when its specification's content is fully performed by enforced reference artifacts and conformance tests; authority for that ground then rests with the codebase — including its executable specification — and the prose specification retires by sponsor act under B8's discipline. Retirement is discharge, not disposal: it is the ceremony of authority transfer, and the survivorship audit is its verification.
...
**B8 — Retirement, and the provenance exception.** Retirement is a sponsor act, recorded in REG-005, and applies **only to documents an act individually names**. A retired document has no authority and must not be consulted as authority; reading it requires treating it as historical evidence only. **⚠ EXCEPT FOR PROVENANCE VERIFICATION, WHICH IS ALWAYS PERMITTED:** consulting any document — retired or otherwise — **to check whether a canon clause faithfully distils it** is not consulting it as authority. **WHY:** canon's own provenance sidecars cite documents inside `retired/`, so a rule forbidding this would make canon unauditable, and unauditable is precisely how ASR-14 and ASR-16 came to weaken prohibitions their sources state categorically (REG-F-094). ⚠ **AMENDED 2026-08-09 (REG-D-034, sponsor ruling). REG-D-006 IS REVERSED.** This clause previously retired *"the pre-canon corpus (~200k+ words under `docs/`)"* wholesale upon ratification. That retirement was **never executed** — six weeks after ratification, `retired/` subdirectories existed while 16 and 13 top-level documents remained agent-visible with no declared status — and the material it would have silenced is the material that answered four of six escalated questions. The source corpora are **not retired**; they are operative per B1.
