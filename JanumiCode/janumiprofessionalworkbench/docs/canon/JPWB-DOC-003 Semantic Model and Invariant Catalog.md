...
version: 1.3.0 — B5 amendments: REG-D-014 (ASR-3, ASR-4, §11 hygiene → 1.1.1); REG-D-015 (PER-12 sponsor-ruled CoT retention → 1.2.0); REG-D-016 (Narrative Memory gains its defining question — the temporal-complexity pairing)
date: 2026-07-16
governs: |
...
| **Narrative Memory** | Interpretive, provenance-linked account of evolution; never a replacement for canonical Events, objects, or Evidence. Exists to answer *where can we credibly go from here*; the governed stream answers *how did we get to this point* (sponsor-voiced purpose, REG-D-016). |
...
**SCOPE:** governs material transformations. A result is material by default when it creates or changes professional content, supplies a downstream actor, proposes a Claim or Evidence, changes decomposition, contributes to a Decision/Baseline package, or supports a governed transition. Each independently downstream-consumable result is its own transformation boundary unless an explicit grouping records every covered subject and version and its rationale; one PWU-level review never substitutes for exact per-output assessment. Grouping is legitimate only where grouped outputs share a producing Attempt or a policy-permitted equivalence class; a grouped Assessment answers each mandatory criterion for each covered subject and version, and the grouping rationale is subject to the ASR-2 re-derivation gate. Batching heterogeneous outputs to dilute per-subject scrutiny is not grouping — it is a floor violation. **NON-EXAMPLE:** a transformation established as losslessly semantically equivalent by a *versioned rule* (not the producer's own say-so) is nonmaterial and needs no Reasoning Review; ambiguity resolves to material.

**ASR-4 · Reasoning Review is a contracted policy concern, not a second prompt.** Reasoning Review is a versioned Assurance Policy implemented by a replaceable Validator. It binds the exact subject, input/context versions, producing attempt, policy versions, and available evidence; evaluates observable derivational integrity (unsupported assumptions, invalid or circular inference, scope/authority confusion, contradiction, premature convergence, completeness shortcuts, unacknowledged uncertainty, evidence misuse or omission); records considered/rejected/missing evidence, findings, limitations, and uncertainty through the normal contracts; and prohibits same-invocation self-review — at minimum a distinct evaluator invocation, role, and review context whose actual identities and lineage are recorded. It reviews professional rationale summaries, outputs, and authorized tool-call records. The Assessment is a durable, inspectable record (PER-9); a disposition asserted without its retrievable record is a B7 anti-vacuity violation.
...
**⚠ REPAIRED 2026-08-09 (ROADMAP-corpus-precedence P-4, under CON-000 B3 as amended by REG-D-034).** This clause read *"may exceed ordinary product authority and be non-waivable"*. Its source states the prohibition categorically — *"Critical integrity failures cannot be waived by ordinary product authority"* (Assurance Policy Catalog and Validator Contract L1513-1515, via `extract-doc004-b.md`) — and glosses it *"waiver authority is tiered; some failures exceed product-level authority"*. The distillation had turned a categorical bar into a possibility, leaving waiver by ordinary product authority available whenever the *"may"* did not apply, **and had also dropped the tiering that explains why the bar is on the AUTHORITY rather than on waiver as such**. No ratifying act named the divergence (`ratifyingActsFor('ASR-14')` = `[]`), so under B3 the clause was DEFECTIVE and the source controlled. Restored to the source's strength.
...
**ASR-16 · Baseline promotion is a governance event with exact identity.** Promotion requires: exact item identities, semantic versions, and content hashes where applicable; the promoted version exactly matching the reviewed version; required claims, admitted evidence, assessments, and dispositions; no unresolved critical finding, and blocking findings resolved or authorized through a policy-permitted scoped waiver; effective promotion authority; accepted residual risk; and declared purpose/scope with rollback/recovery where required. An authoritative baseline is immutable — change creates a successor with a supersession trace, and the predecessor stays queryable.
...
**⚠ REPAIRED 2026-08-09 (ROADMAP-corpus-precedence P-4, under CON-000 B3 as amended by REG-D-034).** This clause read *"no unresolved blocking **or critical** finding except through a policy-permitted scoped waiver"* — collapsing two conditions the source keeps apart and **extending the waiver exception to CRITICAL findings, which the source does not permit**. The source reads *"Promotion conditions: no unresolved critical finding; blocking findings resolved or authorized through waiver"* (RPH-DOC-003 promotion gates, via `extract-doc003-a.md`), and the blocking list independently names *"unresolved critical observation"* as a promotion blocker (`extract-doc003-b.md`). Canon permitted what its own first-cited source forbids. No ratifying act named the divergence, so under B3 the source controlled. The waiver exception now attaches to BLOCKING findings only.
...
**PER-12 · Volunteered model reasoning is retained where available, quarantined from assurance, and usable only for offline diagnosis.** (Sponsor-ruled — REG-D-015.) Chain-of-thought and other volunteered model reasoning is redacted at the trust boundary and **retained where available** as a typed Artifact bound to its producing Attempt under the applicable retention, security, and access policy. Availability is provider- and configuration-dependent — many hosted frontier models expose no trace, local models generally do, some harnesses only under special configuration; there is no obligation to solicit or procure a trace. It is never admitted as Evidence, never forwarded to another actor, never logged, never projected, and **never an input to any tier of assurance policy or Assurance Engineering**; its presence in an evaluator's context is a hidden-context independence violation (ASR-13). Its one permitted use is **offline diagnosis**: engineering inspection under the applicable access policy, outside the assurance, execution, and governance planes. It is purgeable at retention expiry (PER-8).
**WHY:** two truths that pull in opposite directions, both sponsor-affirmed: a reasoning trace is not reliably tied to the conclusion the model actually reached — which is exactly why it can never be evidence-grade or contaminate evaluation — and yet it is the highest-leverage diagnostic signal available when behavior must be explained after the fact. Retention-without-forwarding serves the second truth without violating the first.
**SCOPE:** governs volunteered reasoning as data at rest and in flow. **NON-EXAMPLE:** this rule governs retention and flow of reasoning, never its generation — enabling model thinking is legal (the §9.7 lesson, JPWB-DOC-004); professional rationale summaries offered through governed contracts are ordinary content, not reasoning traces; and offline diagnostic *reading* of a retained trace is not "forwarding" — forwarding means placing it in an actor's or evaluator's operating context.
...
The following meaning-level ambiguities in the source corpus are deliberately **not** resolved here. Items 1–5 are filed in JPWB-REG-005 with a safe default; items 6 and 7 are resolved in place below (their onetime register shadows, REG-Q-043/044, are CLOSED citing this section — REG-D-014). Do not invent resolutions locally.
