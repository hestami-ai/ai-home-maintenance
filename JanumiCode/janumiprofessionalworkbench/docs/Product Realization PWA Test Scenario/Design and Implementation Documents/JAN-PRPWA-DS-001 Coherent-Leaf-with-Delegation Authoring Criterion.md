...
- **D-1a (The three planes — definition).** A PWA is read on three distinct, non-interchangeable planes; the agnostic core SHALL teach all three and keep them distinct:
  ...
  The **V-model correspondence axis** (left-decomposition ↔ right-verification) is a **deferred 4th plane** (§11), NOT one of the three. *This trio is distinct from §5.2's platform/content three-way classification and from STD-1-I(d)'s floor triad; the agnostic core SHALL NOT conflate them.*
