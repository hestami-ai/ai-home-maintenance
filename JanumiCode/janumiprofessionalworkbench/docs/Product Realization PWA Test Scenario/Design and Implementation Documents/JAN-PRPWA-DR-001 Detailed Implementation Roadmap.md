...
open_decisions: ["F-11 RULED (2026-07-21): the three planes are taught EXPLICITLY (not left implicit) in system-prompt.ts §THE THREE PLANES; D-1a non-conflation completed — the trio is held distinct from the platform/content trichotomy AND from STD-1-I(d)'s three-policy floor triad ('three PLANES, not three policies')"]
...
open_decisions: ["D-C RULED (Option 1, broker-parity): attestedAssurancePolicyIds validated in the broker like requiredAssurancePolicyIds", "Option 3 handler-hardening RULED → DELIVERED post-wave (F-7 closure, commit 9be1b58f): assurance-policy-reference validation moved to the domain write boundary (§15)"]
...
conformance_state: CONFORMANT  # SPEC-3/STD-2/STD-3/R-9/R-10(authoring)/INV-1/F-1/F-3 realized as specified; INV-5 untouched (attestation = counterparty claim, never authoring self-cert). Option-3 systemic handler-hardening DELIVERED post-wave (F-7 closure, commit 9be1b58f; §15).
```
...
open_decisions: ["RULED by delivery: >1-distinct-output advisory scope confirmed for this wave (INV-4b, F-9); template-keyed arms DEFERRED with SPEC-4/R-12 (§2/§15)"]
...
open_decisions: ["RULED: define_from_template stays INTERNAL-only (all templates INTERNAL; boundary is set via define/edit, not template application)"]
...
open_decisions: ["D-E RULED: the under-decomposition advisory's human read is satisfied by the agent surface (review_composition, DWP-03); no net-new advisory panel this wave (disclosed, PB-3)", "Node-card DELEGATED marker RULED: delegation is represented in the rail (attestation-substitute row), not a header badge; the optional §11.7.4 header marker was not taken this wave"]
...
open_decisions: ["D-F RULED: the delegated Reasoning-Review slot reads '… · substituted by counterparty attestation (required at Undertaking time)' — never SATISFIED", "RULED: requiredAssurancePolicyIds is NOT projected onto PwaGraphNode (unneeded; the card rail shows declared policies from PwuTypeNode)"]
...
open_decisions: ["RULED: projected-ASPLE oracle format = assertion-set (required work-area kinds + expected-leaf kinds + forbidden-platform kinds) — robust to mock-plan wording vs a brittle full expected-tree"]
...
- **Per-DWP decisions (all RULED — reconciled 2026-07-21):** D-E advisory human surface → agent surface suffices (DWP-05); D-F delegated Reasoning-Review label → "substituted by counterparty attestation" (DWP-06); define_from_template boundary override → INTERNAL-only (DWP-04); DWP-07 oracle format → assertion-set. **Still deferred (dispositioned, NOT open):** the two template-keyed advisory-lint arms, with SPEC-4/R-12 (DWP-03, §2).
...
## 20. Post-wave follow-up dispositions — deferred items, pre-existing debt, F-11 (sponsor-directed 2026-07-21)

Sponsor directive (2026-07-21): *address all the deferred items, the pre-existing debt, and the untouched DWP-01 F-11.* Each was scoped against DS-001 + the live code before any edit. The governing principle (DS-001 §299/§10; STD-4 `JAN-ENGC-001`): implementers SHALL follow a ratified deferral until the sponsor overrides that specific ruling, and SHALL surface friction rather than fabricate. Outcome: **three BUILT** (no ratified bar; correct + non-hollow), **six HELD** (fabricating would violate the corpus — each with a concrete un-defer path). No item left silently dangling.

### BUILT (landed → gated → green: check-types 21/21 · test 21/21 tasks · lint 0 · boundary 0)

- **F-11 (three-planes wording) — CLOSED.** SPEC-1/D-1a require the agnostic core to teach the three planes AND keep the trio distinct. Verified the prompt already teaches all three explicitly (DWP-01 delivered it; the "implicit vs explicit" decision was resolved, only unreconciled). The one real residual: the trio was held distinct from the platform/content trichotomy but NOT from STD-1-I(d)'s three-policy floor triad. Closed with a precise clause ("three PLANES, not three policies. Do not conflate the trios") + a locking test assertion. Files: `system-prompt.ts`, `system-prompt.test.ts`.
- **Fixture C (import-and-project, PB-4 calibration limb) — DELIVERED.** The projected scorer silently DROPPED any kind matching none of the oracle's three lists — so a foreign/imported element read as "no finding". Added the third import-and-project bucket: `CalibrationScore.outOfModelKinds` (+ `ProjectedOracle.structuralKinds` so the domain root stays in-model and Fixture A stays clean), and a Fixture C test proving the three-way classification (platform-leak / missing-area / out-of-model) + the closed silent-drop gap. Reuses `scoreAgainstOracle` exactly like A/B. The full import PLAYBOOK/UI stays deferred (R-8/PB-4); only the calibration fixture is built. Files: `calibration.ts`, `calibration.test.ts` (8→11 tests).
- **F-13 (triplicated floor labels) — CONSOLIDATED where safe + DRIFT-GUARDED where not.** The label/id duplication is PARTLY justified: `catalog.ts` `ASSURANCE_FLOOR` is a deliberately browser-safe copy because rph-assurance is browser-UNSAFE (node:crypto; `.dependency-cruiser.cjs projections-browser-safe`), so physical consolidation there would break browser-safety. Actions: (a) consolidated `assurance.ts` onto the canonical `@janumipwb/rph-assurance FLOOR_POLICY_IDS` — the "avoid cross-package dep" comment was stale (floor-gate.ts already takes that acyclic edge); (b) added a Node-only **drift-guard test** (`catalog.floor-consistency.test.ts`, rph-assurance as a test-only devDep) pinning the two justified label authorities (store `FLOOR_POLICY_DEFINITIONS` names ↔ browser `ASSURANCE_FLOOR` labels) so they can never SILENTLY drift. **Un-defer full physical consolidation:** relocate floor id+label identity to the `rph-contracts` foundation (browser-safe, universally depended-on) — a separate architectural increment; until then the guard is the correct discipline for justified duplication.

### HELD — fabricating would violate the corpus; each with its un-defer trigger

- **ValidatePwa assurance-assignment limb — HELD (§0.3).** Skipped under a deontic bar, not neglect: `requiredAssurancePolicyIds` is a bare `string[]` lacking §11.7.4 rail-1's trigger/materiality/subject/Claim/Evidence/independence; the two rules that would close it are stated nowhere in the corpus and are contradicted by the seeded PWA itself (5/9 types carry only the floor); `evaluateApplicability` exists but is deliberately dead (harmonization finding [41]). Forcing it deepens the hollow-governed-layer problem. **Un-skip trigger:** a sponsor-ratified §16-item-9 assurance-assignment CONTRACT (the wire shape is explicitly unresolved), then route through `evaluateApplicability` and seed an applicable policy. No code — correctly documented in handler + skipped test.
- **SPEC-4 domain template + R-12 + the two template-keyed advisory arms — HELD (hollow-layer + §0.3).** There is NO consumption path today: no `DomainTemplate` object/command; `define_from_template` is a hardcoded catalog blueprint that becomes a normal `DefinePwuType`; the lint `CompositionNode` carries no `pwuKind`; `typicallyDecomposed` exists nowhere; the ladder/framework content lives only in docs. Authoring a template now = a governed object with ZERO readers. WHICH kinds are "typically decomposed" / expected depths is a governance judgment (§0.3) — a hardcoded set risks false advisories for legitimately-different orgs (STD-1 irreducibility is scope-relative). **Un-defer design (non-hollow):** (1) add `pwuKind` to the lint `CompositionNode`; (2) `lintComposition`/`checkUnderDecomposition` take an OPTIONAL domain-template param `{ typicallyDecomposedKinds, expectedMinDepthByKind }` (keeps lint domain-AGNOSTIC per D-1); (3) the domain-aware caller (rph-demo) supplies it, sourced from an AUTHORED R-12 template (content projected from the ASPLE fixture — the sanctioned source); (4) wire arm (a) pwuKind-typically-decomposed and arm (b) branch-depth-vs-expected. Build template + reader together so it is never hollow. Blocked on: the R-12 authored-template governance decision.
- **performerMode (automation axis) — HELD (R-9).** Zero code; the vocab EXPLICITLY reserves it — `canonical-vocabulary.json` ExecutionBoundary note: "The automation axis … is a SEPARATE future field, NOT folded into this enum (R-9)." Building it now contradicts a ratified ruling. **Un-defer trigger:** a sponsor ruling on the automation-axis field shape (MANUAL/ASSISTED/AUTOMATED on the PWU-Type or instance), then add it as its own orthogonal field — never conflated with ExecutionBoundary.
- **V-model correspondence axis ("4th plane") — RETIRED as a category error (sponsor 2026-07-21), NOT held.** The prior "deferred 4th plane" framing conflated content with platform. ASPLE is a *subject* — a PWA a user asks the authoring agent to create — so the V-model (like JTBD/UCD/FDD/TDD) is domain SDLC *framework vocabulary* that may appear INSIDE that authored content, not a feature the platform's authoring surface must render. The three planes are platform-UNIVERSAL (all professional work has composition, artifact dependencies, a lifecycle); left↔right V-model correspondence is domain-specific (Fixture B "get bloodwork" has none), so elevating it to a platform plane/overlay over-fits the platform to one domain — the exact platform/content leak D-3/INV-3 forbid. Any verification-to-specification correspondence an author wants is already expressible as ordinary PWU Types wired through the **artifact hand-off plane** (the verification node consumes the artifact the spec node produces), with the assurance floor's Reasoning Review doing the actual verifying. DS-001 itself supports this: **D-1** already places "correspondence axis" inside the pluggable **domain template** (content); only **D-1a**'s "4th plane" clause mislabeled it platform. **Action:** nothing to build and nothing to hold; DS-001 D-1a's "deferred 4th plane" clause should be struck/annotated to match D-1 (correspondence = domain-template content, never a platform rendering plane).
- **Runtime attestation-capture (R-10 runtime) — HELD (no capture model; same root as the ValidatePwa limb).** Substrate PARTLY exists: a live PWU-execution runtime (ExecutionPlan/ExecutionStep + `changePwuState`) to hook into, and a directly reusable reviewed-version-vs-current staleness guard (`markPwuReady`, `pwu.ts`). Genuinely unbuilt: any counterparty-ATTESTATION object/command/event (none exists); an Undertaking-level execution loop (only `CreateUndertaking`); and dereferencing a cited attestation id (currently unchecked — and the reference fixture cites an object never created, so validating it needs the assessment-object lifecycle — the SAME §16-item-9 root as the ValidatePwa limb). **Un-defer design:** model an `ATTESTATION` aggregate + `CaptureAttestation` command/event carrying counterparty identity, attested policy ids, reviewed semanticVersion, timestamp; capture it at Undertaking time over the delegated boundary; reuse the `markPwuReady` staleness pattern; then the delegated rail's "substituted by counterparty attestation" becomes backed by a real captured record rather than a derived constant. A mini-wave (DS-001 §326 defers it).
- **Live calibration run (EP-TST-8 prompt-regression) — HELD (manual; cost + no-fabrication).** `agy` v1.1.5 IS available and `playwright.live.config.ts` exists, so the live author-from-intent path is runnable. But a live run consumes the sponsor's model quota (a cost action requiring confirmation), is nondeterministic, and the reasoning-quality/consistency dimensions require HUMAN scoring that must not be auto-certified as passed. **Recipe (record, run on request):** `cd apps/rph-demo && RPH_DEMO_MODE=live bun run e2e:live` for the live authoring smoke; then score the resulting graph with `scoreAgainstOracle` (Fixture A oracle) and manually judge the EP-TST-8 five dimensions (structured output · reasoning quality · business decisions · tool selection · consistency). This is the versioned prompt-regression evaluation set for DWP-01/DWP-04/F-11; it stays a reasoned manual deferral (mock CI cannot evaluate reasoning-quality — `JAN-ENGC-001 §1.4/§6.2`).

### 20.1 Category-error audit (2026-07-21) — platform/content boundary sweep

Prompted by the V-model retirement, swept the platform layers (contracts / application / authoring / projections) for the root pattern: **an ASPLE-internal or domain-specific concept promoted to a JPWB platform feature** (a D-2 / D-3 / INV-3 leak). Result:

- **Wave + post-wave code is CLEAN.** DWP-01…07, the F-7 closure, and the F-11 / Fixture-C / F-13 builds introduced no such leak. The platform code (ExecutionBoundary, handlers, `leafKind`, the delegated rail, the `ProjectedOracle`/`scoreAgainstOracle` mechanism) is domain-agnostic; the ASPLE-specific values live only in the explicitly-labeled calibration fixture (`PROJECTED_APLE_ORACLE`), the correct home for a subject/oracle. `engine.ts` is the exemplar of doing it right — "PWA-AGNOSTIC mechanism: the Product Realization PWA … loaded as versioned DATA and INJECTED." The **V-model "4th plane" was the sole genuine category error, doc-only (never in code)** — retired above, no code to unwind.
- **Pre-existing, same-family (not wave-introduced) — RECORDED, folds into other work:**
  - `packages/rph-authoring/src/catalog.ts` `PWU_TYPE_CATALOG` hardcodes the Product-Realization §7 work areas as the `define_from_template` blueprint **inside the domain-agnostic authoring layer**, duplicating the `rph-product-realization-pwa` domain pack and sourcing domain content from a constant rather than a pluggable D-1 template. **This IS the SPEC-4/R-12 gap** — the correct fix (both `define_from_template` and the two deferred advisory arms sourcing from ONE authored pluggable domain template) is folded into the SPEC-4 hold above; do NOT add a hardcoded stopgap.
  - `packages/rph-projections/src/compatibility-view.ts` `KIND_TO_MILESTONE` hardcodes PR pwuKinds → DOC-006 legacy phase labels in a projection; milder (graceful "unknown kinds → INTAKE" fallback; a legacy bridge). Un-leak: make the kind→milestone map domain-supplied. Low urgency.
- **General rule of thumb (adopted):** do not promote an ASPLE-internal framework (V-model, JTBD, UCD, FDD, TDD) or Product-Realization specifics to a platform feature. Domain content lives in the pluggable domain template / domain pack (D-1) or a labeled fixture; the platform stays agnostic and INJECTS it (D-2).

*End of JAN-PRPWA-DR-001 v0.3.0 — wave complete; post-wave follow-up + category-error audit dispositioned 2026-07-21.*
