...
| **Status** | `WAVE COMPLETE (2026-07-20). DWP-01…07 all DELIVERED + CONFORMANT (commits af3825c8, ceae3bd7, 82e93ca1, 09a806ba, e3e32b97, 2aaf50db, 5ae6c61a on branch sonar/jpwb-remediation-2026-07-20; not pushed). Full gate green: check-types 21/21, all unit suites, lint 0, boundary 0, e2e 28/28. D-C→Option 1 shipped; Option 3 systemic F-7 closure remains BACKLOGGED (§15) — the recommended next increment. Sub-decisions ruled + disclosed: RC-4 (DWP-04→05 e2e), D-E (agent advisory surface), D-F (delegated RR-slot label), oracle-format (assertion-set).` |
...
- **BACKLOG (Option 3, post-wave hardening; sponsor-directed 2026-07-20):** *Assurance-policy-reference validation as a domain invariant* — move `requiredAssurancePolicyIds` **and** `attestedAssurancePolicyIds` ACTIVE/non-floor validation from the broker (a UX pre-check) into the domain handler write boundary (C-5), closing the `F-7` bypass so no non-broker command dispatch can persist invalid/DRAFT/floor policy references. Cross-cutting (touches `pwa-authoring.ts` + adds an `rph-application → ASSURANCE_POLICY status` read); its own effort/wave; connects to the hollow-governed-layer theme (`F-13`).
...
- **BACKLOG (sponsor to schedule):** Option 3 — move ALL assurance-policy-reference validation (`requiredAssurancePolicyIds` + `attestedAssurancePolicyIds`) into the domain handler to close the F-7 broker/UI-only bypass. NOT pulled into this wave: it gates `DefinePwuType` and the seed/reference-PWA authors policy references before activation, so it needs its own increment (seed-ordering + retain-existing semantics). This is the recommended next increment.
