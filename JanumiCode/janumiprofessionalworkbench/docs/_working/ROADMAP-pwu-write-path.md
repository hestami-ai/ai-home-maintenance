...
| **W-4.6** | `BeginPwuRecomposition`, `CompletePwuRecomposition` | events must be AUTHORED | split from W-4.5: unlike `PwuBaselined`, no PWU-side recomposition event exists. Their arrows are reachable by nothing today, so W-7's urgency does not apply equally. See REG-F-082 — their binding rows currently name the wrong machine |
