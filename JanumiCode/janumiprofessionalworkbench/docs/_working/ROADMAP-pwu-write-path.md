...
| **W-5.5** ✅ **UNBLOCKED 2026-08-16 (REG-D-043)** | `UnblockPwu` — the deferred half of W-5. Arrows **DERIVED from §8.2 reversed**: `BLOCKED → {SHAPING, PLANNED, EXECUTING}`, `ESCALATED → EVIDENCE_PENDING`. **`BLOCKED → READY` deliberately NOT added** — not a ratified in-arrow, and it is the discriminating case. | W-5 | **The act is now ruled, so the shape is authorable (REG-D-029).** THREE things this must solve, all grounded: (1) the origin is NOT on `PwuBlocked` — its `workLifecycleState` is set to `BLOCKED` itself, while the GENERIC `PwuStateChanged` carries `previousState`; fork is fold-the-event-prefix (`readAggregateEvents` + `replayPwuAxes`, no new plumbing, three handlers already do it) vs record `blockedFrom` (contract change; historical events lack it, so fail closed per AX-8). (2) `PwuLifecycleCommandSpec.target` is a SINGLE state — a dynamic target reaches every lifecycle command. (3) The arrow is reachable by the generic setter unless guarded (cf. REG-F-072). **PREDICTED RED:** `block-escalate.test.ts` CONTROL 4 — restructure it to COLLECT all four outcomes so it discriminates, since its loop currently throws on the first and never reaches READY. —— ✅ **AMENDED 2026-08-16 (REG-F-193) — COLLISION 1 CLOSED, and the resolution is a UNION rather than the re-key.** The GUARD becomes `ownerOfArrow(from, to) ?? ownerOfTarget(to)`: arrow lookup first for precision, the existing **eleven**-row `PWU_SEMANTIC_LIFECYCLE_COMMANDS` retained as a FAIL-CLOSED BACKSTOP. Refused set = the union of both keys, so **widening is zero by construction** (and measured: 49 == 49, symmetric difference empty both ways). ⚠ **A PURE ARROW KEY WAS REFUTED BECAUSE IT KILLS A CONTROL:** `generic-setter-scope.test.ts:233-248` is the sole pin on ownership running LAST, its subject is `PROPOSED→READY`, and that arrow is not among the 49 — so under an arrow key the ordering mutant becomes unkillable and no ledger mutant backs it up. The union keeps the target lookup live, so CONTROL 2 stays falsifiable. **FOUR PARTS, in this order.** (1) `blockedFrom` on `PwuBlocked` (AX-6; historical events lack it → fail closed per AX-8). (2) `PwuLifecycleCommandSpec` gains an ARROW-SHAPED declaration — for the eleven existing commands arrows are `sourceStates × {target}`, unchanged; the recovery command declares its four pairs explicitly, which is also what subsumes the "dynamic target" problem. (3) The machine arrows + `UnblockPwu` + the union guard, landed TOGETHER. (4) The gates: `verif/lifecycle-arrow-declarations.test.ts` DISJOINT → **arrow-level** (forced under any design, measured: it reddens on exactly `PLANNED`/`EXECUTING`/`EVIDENCE_PENDING`); `pwu-fold-drive-sites.test.ts` `ownedEvents()` → iterate `PWU_LIFECYCLE_COMMAND_SPECS` **or `PwuUnblocked` ships undriven, which is REG-F-084’s defect built a THIRD time**; and CONTROL 4 must assert the **CODE per arrow**. ⚠⚠ **THE PREDICTED RED IS NOT "two of four flip" — that held only if the arrows landed WITHOUT the guard. Landed together, all four stay REJECTED and only the CODE changes, so a status-only CONTROL 4 is GREEN BEFORE AND AFTER the transition it exists to notice.** |
