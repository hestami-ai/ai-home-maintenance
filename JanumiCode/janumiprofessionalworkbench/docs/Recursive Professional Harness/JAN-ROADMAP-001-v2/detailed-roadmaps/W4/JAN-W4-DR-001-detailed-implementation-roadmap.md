...
| Baseline promotion-to-AUTHORITATIVE in the UI (engine supports it; the demo route stubs it) | 4-007 | **DEF-W4-003 — ⚠ DISCHARGED, recorded 2026-08-20 (REG-F-200); the sentence below is retained as the historical statement and is NO LONGER TRUE.** The promotion UI exists: `apps/rph-demo/src/routes/baselines/+page.server.ts` drives CANDIDATE→UNDER_REVIEW→APPROVED→AUTHORITATIVE through `canPromoteBaseline`'s real gate, and the route's own header records the closure it never wrote back here — *"This header used to say promotion was out of scope for this authoring surface"*. Historical text: the engine's `promoteBaseline` is fully wired (WIRE-4/3b); the UI needs the promotion-decision orchestration form |
