...
Plus the bounded, disclosed deferrals per wave (rule-DSL authoring, ~~execution/inspector drill-down UI~~, PWA profiles/fork/export, versioned compatibility derivation, the reshape/falsification loop) — **⚠ AUDITED 2026-08-20 (REG-F-200): 9 of the 21 `DEF-*` deferrals no longer describe the tree. The execution drill-down named in this sentence is BUILT** (steps, attempts, runtime binding, recovery and provenance all render at `apps/rph-demo/src/routes/undertakings/[id]/+page.svelte`; delivered under JAN-EXECPLAN DWP-01..05, DR-004 DWP-06 and JAN-EXECREM), and the genuine residue there is execution-plan AUTHORING, not drill-down. Current status for every deferral is indexed at `docs/tracking/census/f200-deferrals.ndjson` and gated in `verif/tracker-ingest.test.ts`; the per-wave gate packages below are DATED EXIT RECORDS and are deliberately left as written. — all recorded in their gate packages with the underlying capability tested at the engine layer.
