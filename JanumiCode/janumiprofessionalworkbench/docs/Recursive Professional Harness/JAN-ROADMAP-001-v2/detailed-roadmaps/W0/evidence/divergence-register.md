...
- **authority:** coding agent. **status:** ~~ACTIVE~~ **DISCHARGED (recorded 2026-08-20, REG-F-200)** — delivered at BOTH named targets and never written back. `W1-DR-001` §4 is literally titled *"Current-state findings and evidence — per-WP conformance classification"* and grades all eight W1 master WPs (CONFORMANT / PARTIAL / HOLLOW, with code grounding and a "closure needed" column); `W2-DR-001` §2 carries the same for W2 by explicit reference to `evidence/persistence-current-state.md`, whose §"Per-WP verdicts" scores all seven. The stronger per-INVARIANT form the requirement names now exists as gated production data rather than prose: `packages/rph-domain/src/conformance-manifest.ts` (a rule resolving to UNACCOUNTED fails the suite) and `packages/rph-domain/src/enforcement-register.ts`.
- ⚠ **WHY THIS ROW WENT STALE, since it is the general case and not a clerical slip.** This file keeps its DIV and DEC rows current — DIV-W0-001 carries a dated RESOLVED, both DEC rows read EFFECTIVE — and left the one DEF row untouched. Corpus-wide the same asymmetry holds: 9 of 21 `DEF-*` deferrals had drifted, because the artifact meant to carry live deferral status, `JAN-ROADMAP-001-C-living-control-registers.yaml`, declares a `deferral` record contract with a required `status` field and then holds `deferrals: []`. Every deferral therefore lives only in DATED per-wave gate packages, which are historical exit records and must NOT be rewritten — so the question *"is this deferral open today?"* had no artifact to answer it. The deferrals are now INDEXED (`docs/tracking/census/f200-deferrals.ndjson`, gated in `verif/tracker-ingest.test.ts`), which is the same move this programme already made once: an index over the sources, never a rewrite of them.
