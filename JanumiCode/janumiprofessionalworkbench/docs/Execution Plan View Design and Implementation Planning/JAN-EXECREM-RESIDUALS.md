...
| # | Severity | Statement |
|---|---|---|
| **N-10** | MINOR | **[CLOSED 2026-07-26 — fixtures normalised to the corpus nouns; NO gate added, deliberately.]** The recommended ruling was taken: `file-system` and `network` are the corpus's own words, quoted verbatim in RPH-EXE-004's ratified statement, so adopting them is a **derivation**, not a house preference. Every `fs.read`/`shell.exec` literal in the fixtures is rewritten. **NO enforcement was added, and that is the point:** §22.1's ruling left the capability value domain deliberately **open** (a closed enum would forbid capabilities the corpus never enumerated), so a test banning dotted identifiers would contradict a standing ruling to tidy test data. One case keeps **both** dialects on purpose — `capbind-subtypes-contract.test.ts`'s openness assertion — because rewriting it would quietly convert a claim about the schema into a claim about house style. **Statement kept verbatim:** | Fixtures carry both `file-system`/`network` (verbatim from RPH-EXE-004's ratified statement) and `fs.read`/`shell.exec` (a dotted convention from nowhere). Tidiness while nothing consumed capabilities; a real problem once a broker must map them — and the dotted form encodes a **verb granularity the target platform cannot enforce**. Recommended ruling: the corpus's nouns. |
