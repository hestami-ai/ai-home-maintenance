...
|---|---|---|---|---|
| N-1 | **RPH-EXE-003** | "Starting execution with a runtime binding still in REQUESTED is rejected." | ~~`bindingPermitsExecution`~~ | **CLOSED 2026-07-25 by `JAN-EXEBIND` WP-B1.** `StartExecutionStep` resolved no binding at all, so a step started freely against a REQUESTED, DENIED or REVOKED one. Now `ENFORCED` and observed through `Engine.dispatch`, with §15.3's `authorizedRuntimeBindingIds` wired as a **separate** limb — the field WP-14 persisted and nothing read. 7 declared mutants, all RED. |
