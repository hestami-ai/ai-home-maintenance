...
| **M1** | Canonical domain model (17 objects) | All 17 object types as Zod+JSON-Schema; every closed enum; the ~30 named-but-undefined helper sub-schemas. | Every object validates min+full, rejects unknown props; all DOC-006 per-object fixtures validate (RPH-FIX-002). | M0 | ✅ (DISCHARGED: RPH-FIX-002 landed in M13 replay conformance — packages/rph-engine/src/replay.ts) |
...
| **M9** | Decomposition/recomposition + obligation·constraint·assumption | Obligation-conservation gate; recomposition conflict resolution; assumption reification; **dynamic decomposition** (D3). | RPH-DEC-001..007, RPH-CNS-001..004, RPH-ASM-001..006; **P2/P3** (the doc ids are Property P2/P3 §25 + the RPH-DEC/CNS/ASM tests — the earlier "CT-2/CT-4" placeholders don't exist in the specs). | M8 | ✅ (kernel + adversarial-reviewed; DISCHARGED IN PART: decomposition/recomposition commands wired at packages/rph-application/src/handlers/decomposition.ts. RESIDUE: the REVISE path performs none of DOC-003 DEC-2/3/4 — REG-005 REG-F-006, OPEN AND UNSCHEDULED) |
| **M10** | Controller, governance & baseline promotion | Controller decision sequence; ControlAction superset; authority-verified decisions (version-bound); immutable baseline gate. | RPH-GOV-001..007, RPH-BAS-001..007; **P5/P7** (CT-5/6/8/9 are placeholder names; real ids are the RPH-GOV/RPH-BAS tests). | M9 | ✅ (executable kernel + adversarial-reviewed; DISCHARGED: governance + baseline command handlers landed — packages/rph-application/src/handlers/governance.ts and the baseline handler family) |
| **M11** | Execution model, runtime bindings & restart recovery | Static + governed execution plans; runtime-binding authorization; **Maker/implementation execution**; reconcile uncertain side-effects on restart. | RPH-EXE-001..009, RPH-PER-011..014; RPH-E2E-006 (restart, no duplicate side-effects); **P1/P6** (exec≠assurance is P1; P6 idempotency). | M10 | ~~✅ (execution kernel + adversarial-reviewed; DISCHARGED: handler registry at packages/rph-application/src/handlers/registry.ts; execution command handlers + sub-type tightening landed under JAN-EXECREM and JAN-CAPBIND)~~ **PARTIAL as to `RPH-E2E-006` — SUBSUMED BY `JAN-SLICE-SWP-03`. The execution kernel half stands; the RESTART-RECOVERY half did not. `e2e-006-restart-mid-execution.slice.test.ts` drives a REAL restart (file-backed store closed and reopened under a new engine) and finds NO reconciliation act: no Reconcile/Resume/Recover command among the 106, and `externalOperationId` / `reconciliationState` are declared FIELDS of `executionAttempt` in the ratified M11 vocab that the contracts never generated. "Reconcile uncertain side-effects on restart" in the Scope column above was never built. The disposition of this rule is now derived by the Slice ledger, not by this row.** |
| **M12** | Executable conformance suite (7 layers) | Full DOC-008 taxonomy + property P1–P8 + builders/doubles + mutation testing on critical handlers; CI gate. | DOC-008 DoD (all 17 items); CI fails on any single invariant violation. | M2,M4,M5,M6,M7,M9,M10,M11 | ✅ core (property P1–P8 + 7-layer taxonomy accounting-gate + adversarial-reviewed; DISCHARGED IN PART: mutation testing landed under JAN-VERIF — scripts/mutants/, 137 distinct mutations gated; builders/doubles landed as per-family fixtures. RESIDUE: closing the PER/E2E/FIX conformance ids was never tracked to completion — UNSCHEDULED, carried in JAN-ENFCOV-DS-001 §9) |
| **M13** | Reference Undertaking replay (FSM) | Seed/Replay/Conformance modes over the 72-step trace; intent→Architecture-Baseline proven; root PWU legitimately incomplete. | DOC-006 §33 (15 criteria) + CT-1..10; RPH-FIX-001..006; RPH-E2E-001; DOC-009 §44. | M12 | ~~✅ core (rph-engine facade + event-history replay conformance over the 72-step trace: RPH-FIX-001..006 + P1/P5/P6; live-command-drive handlers deferred)~~ **PARTIAL as to `RPH-E2E-001` — SUBSUMED BY `JAN-SLICE-SWP-02`. `e2e-001-intent-to-architecture.slice.test.ts` asserts the five ratified OUTCOME clauses and explicitly NOT the flow ORDER, which `REG-D-047` ruled is an ORDER and which the §26 order limb in `replay-conformance.test.ts` shows the engine VIOLATES at seven acts (23 of 198 precedences). This row's ✅ read as certifying the whole rule. The disposition is now derived by the Slice ledger.** |
...
- **2026-07-11 — M14 CORE COMPLETE ✅ (`a0d46b1`).** Engine seam (tested): `rph-engine/src/graph-view.ts`
  ...
  **STALE — STRUCK 2026-08-31 (`JAN-SLICE-SWP-04`). This progress log's last entry is 2026-07-12 and this line
  has been its live summary ever since. MEASURED TODAY: 12 package directories under `packages/` plus
  `apps/rph-demo`; **495 test files, 5,909 tests**. The count is not the point — the point is that a
  hand-authored progress figure went 7 weeks without anything able to notice it had drifted by a factor of 17,
  which is finding F-7 ("the progress substrate is stale and multiplying") in one line. Progress over the Slice
  programme is now DERIVED: see `docs/tracking/slices/LEDGER.md`, generated by `scripts/slice-ledger.ts` and
  gated by `verif/slice-ledger.test.ts` against staleness. ⚠ NOTHING DERIVES THE OTHER FOURTEEN MILESTONES'
  STATUS, so the rest of this table remains hand-authored and carries the same risk this line just demonstrated.**
