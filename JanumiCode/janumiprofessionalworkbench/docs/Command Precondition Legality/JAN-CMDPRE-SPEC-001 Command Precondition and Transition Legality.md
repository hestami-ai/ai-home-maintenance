...
|---|---|---|
| Execution PLAN-level target sets (`ApproveExecutionPlan`, `CancelExecutionPlan`, `CompleteExecutionPlan`, `FailExecutionPlan`, `SupersedeExecutionPlan`, `ApplyTacticalChange`) | DWP-05 marks the Cancel/Fail/Supersede/Complete exposures `knowledge_status: PARTIAL`; the target sets await empirical establishment | F-1, F-6 |
| pwa-authoring publication-lifecycle target sets (`SubmitPwaForReview`, `ValidatePwa`, `DeprecatePwa`, `RetirePwa`, `DeletePwa`, `EditPwa`, `EditPwuType`, `RemovePwuType`, `AppendConversationEntries`) | Same continuation; `ValidatePwa` additionally carries a latent precondition-before-guard ordering issue (filed as a PILOT finding) | F-1, F-6 |
...
| `EditAssurancePolicy`, `SubmitEvidenceForAssessment` | `commitState`/hand-rolled sites that do not advance a status field; their re-issue harm (a duplicate received-evidence fact) needs the reader-precondition variant, not a `fromStates` set — DS-001 critique B4 ground | F-6; DWP-08 |
