...
The **state-machine catalog (§2) is complete**: all 26 aggregate transition machines in `transitions.data.ts`. The **per-command contract catalog (§3) is complete at classification grain for all 84 registered handlers** — every command is classified.

> **AS-OF NOTE (added by the JAN-CMDPRE post-build review, 2026-07-24).** §3 and §3.1 record the **design-time (pre-remediation) baseline** — the classification of each site *as the DWP series found it*, i.e. the remediation SURFACE the DWPs then acted on (§3.1 is titled exactly that). They are **not** a running picture of the current code and were deliberately **not** re-classified after each increment shipped. Because DWP-04/05/06/08 are now DELIVERED (see `JAN-CMDPRE-DR-001` §16 and this document's own §5.2/§5.3/§5.4 and §7), a site §3/§3.1 marks `NONE` / `GUARD_ONLY_ACCIDENTAL` / "would-append-second-event" (e.g. `SupersedeBaseline`, `RevokeDecision`, the AssurancePolicy and execution plan-level and pwa-publication families, the commitState edits) has since been hardened in code and now carries an EXPLICIT_FROMSTATES set or a PREDICATE. **The authoritative CURRENT state of each site is: the shipped handler, its named `dwpNN-precondition-coverage` kill test, §5.2–§5.4 + §7 here, and the DR-001 traceability matrix — in that order — NOT §3/§3.1.** Reading §3 row 67 (`SupersedeBaseline = NONE`) as "still un-hardened" is a misread of a historical baseline; the delivered `precondition: fromStates('AUTHORITATIVE')` is at `governance.ts` and is kill-tested. Any future reclassification of §3/§3.1 to the delivered grain must be regenerated from the handlers, not hand-edited.

**Full remediation contracts (the authored target set + fixture obligation) are specified exhaustively for the governance and assurance families — DWP-04's ground — and for the families the JAN-CMDPRE series has already hardened (intent, evidence/assumption, decomposition/recomposition, runtime-binding, execution step-level).** The **execution PLAN-level re-issue *target* semantics and the pwa-authoring publication-lifecycle remainder** were classified here at the design baseline (their pre-remediation mechanism is enumerated in §3.1) and their authored target sets were deferred to the continuation forks (F-1, F-6) and the Deliberately Unspecified table (§7) at the time of writing — subsequently DELIVERED in DWP-05 (§5.3) — because `JAN-CMDPRE-DR-001` DWP-05 recorded those exposures as `knowledge_status: PARTIAL` and specifying their target before establishment would have been inventing a settled fact the program had not yet established (JPWB-CON-000 AX-8: fail closed; file, do not invent). This is minimum-scope-plus-hardened-families exhaustive, with the (then) genuinely-unestablished remainder tabled — not the whole thinned to touch more.
...
> **AS-OF: design-time (pre-remediation) baseline — see the §1.4 AS-OF NOTE.** This catalog and §3.1 record each site's classification *as the DWP series found it*. Sites marked `NONE` / `GUARD_ONLY_ACCIDENTAL` that DWP-04/05/06/08 have since hardened now carry an EXPLICIT_FROMSTATES set or PREDICATE in code; the CURRENT state is the shipped handler + its `dwpNN-precondition-coverage` kill test + §5.2–§5.4/§7 + the DR-001 traceability matrix, not this table.

Every registered command handler (`packages/rph-application/src/handlers/registry.ts`), classified by its precondition mechanism **at the design baseline** (pre-remediation; see the AS-OF note above). The classes:
...
### 3.1 Remediation surface — the un-hardened and accidentally-protected sites *(as of the design baseline)*

> **AS-OF: pre-remediation.** Every site below is stated *as the DWP series found it*. The governance, AssurancePolicy, execution plan-level, pwa-publication, and commitState-edit sites listed here as `NONE` / `GUARD_ONLY_ACCIDENTAL` / "would-append-second-event" have since been DELIVERED (DWP-04/05/06/08) and now carry a precondition in code with a named kill test — see §5.2–§5.4, §7, and the DR-001 §16 traceability matrix. What genuinely remains un-hardened is the F-6 PWU-lifecycle family (`advancePwuLifecycle`: BeginPwuShaping/MarkPwuReady/ChallengePwu/ReshapePwu/InvalidatePwu/SupersedePwu), which stays GUARD_ONLY_ACCIDENTAL by design (§5.4).
