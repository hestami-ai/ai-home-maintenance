<!--
  TaskDecompositionPipelineCard — Wave 8 composite card for a recursive
  task-decomposition run (Phase 6.1a). Mirrors
  ComponentDecompositionPipelineCard for tasks: per-pass status,
...
<script lang="ts">
  ...
  interface PassEntry {
    ...
    termination_reason?: 'fixed_point' | 'depth_cap' | 'budget_cap' | 'fanout_cap' | 'human_pruned_all' | 'diverging' | 'dedup_offline';
  }
  ...
  function terminationChipText(): string {
    ...
    switch (last.termination_reason) {
      ...
      case 'diverging': return 'diverging — early terminate';
      case 'dedup_offline': return 'dedup offline';
    }
...
<div class="pipeline-card pipeline-task" data-record-id={record.id} data-phase-id={record.phase_id}>
  <button class="header" onclick={() => (collapsed = !collapsed)}>
    <span class="toggle">{collapsed ? '▶' : '▼'}</span>
    <span class="icon">◇</span>
    <span class="label">Task Decomposition</span>
    <span class="meta">
    ...
    {#if content.final_leaf_count !== undefined}
      <span class="leaf-count">{content.final_leaf_count} atomic units</span>
    {/if}
  </button>
    ...
    <div class="body">
      ...
      {#if rootNodes.length > 0}
        <section class="roots-section">
          <div class="section-label">Task trees ({rootNodes.length} root{rootNodes.length === 1 ? '' : 's'})</div>
          {#each rootNodes as rn (rn.id)}
            <TaskDecompositionNodeCard record={rn} />
          {/each}
        </section>
      {/if}
      ...
      {#if isTerminated}
        <section class="totals-section">
          ...
          <dl class="totals">
            <dt>Atomic-unit leaves</dt><dd>{content.final_leaf_count ?? 0}</dd>
...
<style>
  ...
  .icon { color: var(--vscode-charts-orange, #d18616); }
